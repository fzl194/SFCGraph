...
```
三层图谱构建规范/
├── README.md / VERSION / CHANGELOG.md / 演进机制.md   治理（全局）
...
4. [conventions/命名规范-建议](conventions/命名规范-建议.md)（通用命名）
5. 构建某层时，读该层 `{layer}/` 下全部（agent → SKILL → 字段 → check）
6. 实际跑构建时，读 [scripts/README.md](scripts/README.md)（共享导出脚本）
