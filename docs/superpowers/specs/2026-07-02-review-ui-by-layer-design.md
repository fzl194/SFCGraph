...
| 点击内嵌 chip | `go(sid)`(跳转) |
| 点击 L2 卡片标题区 | 切换其编排下层 L3 chip 列的展开/折叠(只响应 click,不响应 Enter/Space——避免与键盘焦点冲突) |
...
| 键盘 1 / 2 / 3 | 主 card verdict(通过 / 驳回 / 待补);**textarea focus 时 no-op**(避免吞掉输入的数字);FOCUS 与之无关 |
| 键盘 Tab | 在三面板 + 主区焦点环循环切换(FOCUS 顺序:atom → compound → feature → main → atom,见 §5.1) |
| 浏览器 Ctrl+F / Ctrl+P | **不监听**(浏览器原生行为,直接穿透到主区文本) |
| 左面板定位 | 靠面板顶部简化版筛选框(见 §9.1) |

### 3.6 键盘与焦点协调(关键)

- **FOCUS 变量(数据环)**:表示"当前主区焦点属哪个面板",驱动 ←/→ 行为。面板的 DOM focus 用浏览器原生 :focus,二者**解耦**。
- **Tab 切换**:每次按下,在数据环 `atom → compound → feature → main → atom` 循环移一步,同时调用对应 DOM 元素 `.focus()`(左面板的 input 或 主区的 verdict 按钮)。**注意**:Tab 浏览器原生行为在 input/textarea 内被吞掉(默认 shift tab 退出),不冲突。
- **textarea focus 时**(note 输入框被点):
  - 1/2/3 键**全部 no-op**(避免吞用户输入数字)
  - Ctrl+Enter 才提交 verdict(spec 不强制要求,UI 提供"提交"按钮)
- **L2 卡片标题区**:仅响应 `click` 事件切换折叠。**不**响应 keydown(避免与"全主区 verdict 键"耦合,或与 Tab 焦点环冲突)。
- **Ctrl+F 浏览器原生**:前端不拦截,浏览器在全主区文本中搜索。
- **Ctrl+P 打印**:前端不拦截(用户的浏览器默认行为)。

### 3.7 L1 主 card 内嵌区总高约束(防 L1 区视觉爆炸)

- L1 主 card 内的"编排下层"(L2 卡片列表)**不受 L3 折叠策略保护**(L2 自身默认折叠其 L3,但 L2 卡片本身仍全展开)。
- L2 卡片数 > `CHIP_COLLAPSE_AT_LIGHT = 8` 时,主 card 内嵌区采用**两级折叠**:
  - **轻折叠(8 < L2 数 ≤ 15)**:仅显示前 3 个 L2 卡片 + "展开更多 (剩余 N 个)" 按钮
  - **重折叠(L2 数 > 15)**:仅显示前 3 个 L2 卡片 + "展开更多 (剩余 N 个,点击展开后再次折叠需点 L2 标题)" 提示
- 阈值常量集中在 `index.html <script>` 顶部(同 `CHIP_COLLAPSE_AT = 15`),便于调优。
...
5. 点 feature 2-00001 → 主区显示编排 + 内嵌 compound 卡片(默认折叠)+ L1 内嵌区若 > 8 个 L2 卡片则仅显示前 3 + 展开更多
6. 点 2-00001 内嵌的 1-00001 卡片标题区 → 展开 L3 chip 列(只 click 响应,不响应 Enter/Space)
...
11. 键盘 1 / 2 / 3 → 主 card verdict;点 note textarea → 1/2/3 应 no-op(不吞输入数字)
12. 终端跑 python review-ui/_diag.py → 无报错(确认 build_tree / build_reuse 仍存在)
13. 点面板内筛选框,输入"计费" → 该面板仅显示命中项;长名"端到端用户跟踪"+ id 在窄面板(110px)下应 ellipsis 截断不溢出
```
...
| 风险 | 对策 |
|---|---|
| 嵌入卡片层级过深导致主区滚动过长 | 3 层硬上限 + L2 卡片**默认折叠**其编排下层(§3.4)+ L2 卡片列表本身按 §3.7 两级折叠(>8 轻折叠 / >15 重折叠) |
| 复用面 / L3 chip 数量爆炸(atom 0-00019 被 60 feature 用) | 阈值常量 `CHIP_COLLAPSE_AT = 15`(§5.1);L2 卡片数额外受 §3.7 两级折叠保护 |
...
| 窄面板筛选框溢出(110px 宽度塞 input + 计数) | input 设 `min-width: 60%(≈66px)`、宽度自适应;超长 panel item 名 + id 用 `text-overflow: ellipsis` 截断(单行) |
