...
7. **版本双模解析**（填规范空白）：图谱节点 = 版本无关 id（一个 id 一个节点，即使多版本）；物化节点内容时按 **① 默认版本（每 nf 配置，缺省=该 nf 最新） / ② 指定版本（请求 `?version=` 或前端版本选择器）** 解析。边连接版本无关 id，不涉及版本。
...
```
Object = {
  id:           str           # 版本无关逻辑ID（NF 3段 / 跨NF 2段）——图谱节点身份
  type:         str           # frontmatter type
  layer:        str           # 由 type 经注册表派生（Command/ConfigObject/.../Business）
  scope:        "nf" | "cross"
  nf:           str | None    # NF隔离类有
  version:      str | None    # 本实例版本（frontmatter version）
  versions:     [str]         # 该 id 下所有可用版本（索引期聚合，多版本时>1）
...
```
Edge = { from: id, relation: str, to: id }      # id 全部版本无关
```

- **来源**：md 底部 `## 边` 章节，每行 `- {relation}: [[{to_id}]]`（command 规范约定）。
- 平台只**解析**已构建的 `## 边`（边的**派生**由上游"三层图谱构建规范"builder 完成，平台 v1 不重新派生）。
- `to_id` = 版本无关逻辑ID，直接按 id 查 `objects_by_id` 命中（无需路径推断/文件名匹配，见 §6.5）。
...
command 规范要求"引用不带版本、解析时动态选版本"，但未定义选法。本平台设计两模式：

- **图谱节点 = 版本无关 id**：同一 id（如 `UDG@MMLCommand@ADD URR`）即使有多个版本目录，在图谱里**只算一个节点**；`versions[]` 记录其全部可用版本。
- **默认版本**：每 nf 一个默认版本，来自配置 `default_versions: {UDG: 20.15.2, UNC: 20.15.2}`；未配置则取该 nf 下**最新版本**（语义化点分段数值排序）。
- **指定版本**：请求带 `?version=X`（或前端版本选择器）且 X 在该 id 的 `versions[]` 中 → 用 X。
- **解析顺序**：物化节点内容（`GET /objects/{id}` 等）时，**指定版本优先，否则默认版本**。
- **边与版本无关**：边连接版本无关 id，跨版本引用天然成立（业务层引用下层不受版本绑死）。
- 索引：按 id 聚合 `versions[]`；按 (id, version) 存具体内容；邻接表按 id 建。

### 6.5 边 target 解析（极简）

`## 边` 里的 `[[{to_id}]]` 中 `to_id` **就是版本无关逻辑ID**，直接在 `objects_by_id` 查找命中（无需路径推断、无需文件名 sanitized 匹配）。未命中 → 记悬空边告警（不阻断）。这比"解析 markdown 路径链接"简单得多，也是采用 `## 边` wikilink 的好处。
...
- 遍历资产库所有 md → 解析为 Object → 按 id 聚合 `versions[]` → 解析 `## 边` → 产出 Edge 列表。
- 邻接表按**版本无关 id** 建：`out_edges[id] = [Edge]`、`in_edges[id] = [Edge]`（反链）。
- 索引结构：`objects_by_id`（id → {versions, default_resolved, ...}）、`objects_by_type`、`content_by_(id,version)`、`out_edges`、`in_edges`。
...
读端点全部 `GET`、只读；另含上传端点（`import`）。前缀 `/api/v1`。**无 `gid`**——平台只有一份统一资产库。`{id}` 为版本无关逻辑ID；`?version=` 可选（指定版本，否则默认版本，见 §6.4）。
...
| `GET /objects` | 列对象（节点=版本无关id） | `?type=&q=&nf=&domain=&page=&size=` → `[{id, type, nf|domain, versions, name}]` |
| `GET /objects/{id}` | **单对象**（默认/指定版本内容） | `?version=` → `{...Object, versions[], out_edges[]}` |
| `GET /objects/{id}/neighbors` | **单跳** | `?hops=1&version=` → `{center, out[], in[]}` |
| `GET /objects/{id}/md` | 原始 md（默认/指定版本） | `?version=` → `text/markdown`（`raw_md`） |
| `GET /subgraph` | N 跳子图 | `?center=&hops=&type=&version=` → `{nodes[], edges[]}` |
| `GET /default-versions` | 默认版本配置（只读） | `{UDG: 20.15.2, UNC: 20.15.2}`（配置文件来源） |
...
- **顶栏**：导入/导出 + **版本选择器**（按 nf 选默认/指定版本，驱动 `?version=`）+ 统计概览。
...
- **单元**：frontmatter 解析器、`## 边` wikilink 解析器、归类器（id+frontmatter → 路径+文件名=逻辑ID）、版本聚合（id → versions[]）、默认版本解析（配置/最新）、类型注册表查找/扩展合并、邻接表/BFS。
...
3. **版本双模**：同 id 多版本时，`?version=` 指定生效；不指定按默认版本解析；`versions[]` 正确列出。
...
| 项 | 说明 | v1 处置 |
|---|---|---|
| 版本解析细节 | "默认版本/指定版本"为平台设计（规范留白） | 默认=每nf配置或缺省最新；指定=`?version=`/前端选择器。**请用户确认此语义** |
| 样例 bundle 来源 | 用 command builder 产出 | 开发期制作为测试夹具 |
| 层目录大小写 | command 规范用 PascalCase（Command/...） | 平台沿用 PascalCase 层目录 |
| 多版本同 id 节点数 | 一个 id 一个节点 vs 每版本一节点 | v1：一个 id 一个节点（版本无关），内容按版本解析 |
