...
7. 工作区重名：`graph_id` 与已存在工作区冲突时，默认**拒绝并返回冲突提示**；支持 `?force=true` 覆盖（覆盖前对原工作区自动做一次导出快照备份，避免误删）。
...
#### 6.3.1 扩展类型声明文件格式（bundle 携带）

bundle 的 `types/` 目录下，**每个扩展类型一个文件** `types/{TypeName}.yaml`，复用 §6.3 的字段 schema：

```yaml
# types/MyCustomType.yaml
scope: nf                       # nf | cross
frontmatter_required: [id, type, nf, version]
frontmatter_optional: [foo, bar]
edge_resolvers: [edge_explicit] # 引用已注册的边解析插件名
```

- 导入时遍历 `types/*.yaml`，逐条合并进注册表。
- `edge_resolvers` 引用的插件名必须在平台已注册插件中存在；引用未注册插件 → 导入告警（该类型退化为仅用 `edge_explicit`）。
- 同名类型冲突：bundle 扩展覆盖平台默认，并在导入报告中记录。
...
- v1 规则：同一逻辑ID 若多版本共存，解析为**最新版本**，并在导入时记录告警。
- **排序陷阱**：纯字符串排序对语义化版本号有误（`20.9.10` 会排在 `20.15.2` 之后）。实现时优先**按点分段做数值比较**（语义化），失败再退化为字符串比较；告警中列出识别到的全部版本及排序结果，便于发现异常。
...
| 多版本同逻辑ID | 解析规则 | 语义化排序取最新 + 告警；精细选择留二期（§6.4） |
