...
7. **版本解析（默认/指定，填规范空白）**：图谱节点 = **(id, version)，每版本一独立节点**；`## 边` 的版本无关引用 `[[id]]` **默认指向目标网元最新版本节点**，**显式切换**（`?version=`/前端版本选择器）指向指定版本；**默认版本 = 该网元最新版本**（语义化排序，无配置文件）。
...
- **默认版本 = 该网元最新版本**（语义化点分段数值排序取最大；**无配置文件**）。
- **引用（边）解析**：`## 边` 里的 `[[id]]`（不带版本）**默认指向目标 id 的最新版本节点**；**显式版本切换**（API `?version=X` 或前端版本选择器）则指向 X 版本节点。
- **单对象查询**：`GET /objects/{id}` 不带版本 → 返回**最新版本**节点内容 + `versions[]`；带 `?version=X` → 该版本节点。
...
| 项 | 说明 | v1 处置 |
|---|---|---|
| 版本解析 | 规范留白，已按用户拍板 | 节点=(id,version)每版本一节点；边默认指目标最新版本；显式 `?version=`/选择器切换；默认版本=网元最新（无配置） |
