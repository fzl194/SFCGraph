...
- [ ] **Step 1: 加测试断言（test_feature.py 已有的 build_feature_node 测试旁）**

```python
def test_source_evidence_ids_via_feature_step(tmp_path, monkeypatch):
    """feature step 应把 find_overview_md 返回的全部 md path 写入 source_evidence_ids（不只概述）."""
    # 构造一个特性目录: 概述 + 激活 + 原理
    # （集成测试：跑 feature step，断言 source_evidence_ids 含全部 md）
    # 见 FeatureGraph/tests 既有集成测试模式
    pass  # 实现时按既有 test_seed/test_overview 的 fixture 模式补
```

- [ ] **Step 2: 修改 feature step — 单概述分支 source_evidence_ids 用 doc_assets 全部 path**

`FeatureGraph/builder/steps/feature.py` 单概述 yes 分支，当前：
```python
node["source_evidence_ids"] = all_evidence if all_evidence else [primary_overview]
```
改为用 doc_assets 全部 path（不只 overview_paths）：
```python
# source_evidence_ids = 该特性全部 md（来自 find_overview_md 的 doc_assets），不只概述
node["source_evidence_ids"] = [fp for fp, _ in doc_assets] if doc_assets else [primary_overview]
```
注意：`doc_assets` 是 `find_overview_md` 返回的第二项（含全部 md），在 yes 分支已可用。

- [ ] **Step 3: 多概述父节点 source_evidence_ids 也用全部 md**

`_build_multi_overview_nodes` 里父节点 `build_multi_overview_parent` 的 `overview_paths` 参数，改为传入 doc_assets 全部 path（不只 overview_paths）：
```python
# feature step 多概述分支
...
# _build_multi_overview_nodes 内父节点 source_evidence_ids 用 all_doc_paths
```
...
```typescript
// 在 command-graph 路由后加
{
  path: '/feature',
  component: () => import('./feature_graph/FeatureListPage.vue'),
},
{
...
```
注意：FeatureListPage 路由用 `:nf?/:version?` 可选参数（首页 `/feature` 显示 overview 风格或跳转；`/feature/UDG/20.15.2` 显示列表）。实际：`/feature` → FeatureOverview，`/feature/:nf/:version` → FeatureListPage。拆两个路由：

```typescript
{ path: '/feature', name: 'feature-overview', component: () => import('./feature_graph/FeatureOverview.vue') },
{ path: '/feature/:nf/:version', name: 'feature-list', component: () => import('./feature_graph/FeatureListPage.vue') },
{ path: '/feature/:nf/:version/feature/:code', name: 'feature-detail', component: () => import('./feature_graph/FeatureDetail.vue') },
{ path: '/feature/:nf/:version/license/:code', name: 'license-detail', component: () => import('./feature_graph/LicenseDetail.vue') },
```

- [ ] **Step 2: Commit**

```bash
git add platform-next/frontend/src/router.ts
git commit -m "feat(platform-next/frontend): router.ts加/feature三级路由"
```
...
// loadAll 里: 加载 featureDocs 后, 默认 switchDoc(0)
```
