...
1. **用户必须已确认参数表**：必须已通过 GATE-4（用户在 Phase 4 明确确认了参数表）。如果用户未确认参数，**STOP**，回到 Phase 4 等待用户确认。没有例外。
2. **方案和参数必须已确定**：必须有 Phase 1 输出的匹配结果（场景/方案/5 维度决策点）和 Phase 3 输出的完整参数表。缺少任何一个，**STOP**，回到对应阶段补全。
