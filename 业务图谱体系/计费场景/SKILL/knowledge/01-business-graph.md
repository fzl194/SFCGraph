...
### 0.2 计费场景知识来源
- `01计费场景业务图谱.md`（757行，旧业务图谱）
- `unified-knowledge/计费场景统一知识库.md`（3232行，统一知识库 K001-K261）
- `SKILL/knowledge/计费场景业务图谱.md`（624行）
- `SKILL/knowledge/业务感知业务图谱.md`（1628行，业务感知全景）
- `SKILL/knowledge/协议知识-Ga-Gy-DCC.md`（680行）
- `SKILL/knowledge/协议知识-N40-PFCP-Gx.md`（742行）
- `feature-knowledge/cross-feature-analysis.md`（580行）
...
- 接口：Ga（离线）、Gy/DCC（在线）、N40/Nchf（融合）、N4/PFCP（SMF↔UPF）、Gx/N7（PCF↔PCEF/PGW）
...
| `status` | `active` |
| `source_evidence_ids` | `EV-KB-001`(K001-K004, K012), `EV-KB-002`(K015), `EV-BS-001`, `EV-FK-Offline-UDG`, `EV-FK-Content-UDG`, `EV-FK-Offline-UNC` |
...
| `status` | `active` |
| `source_evidence_ids` | `EV-KB-001`(K028-K048, K037, K043-K045), `EV-KB-002`(K015), `EV-FK-Online`, `EV-FK-Content-UDG`, `EV-FK-Event` |
...
| `status` | `active` |
| `source_evidence_ids` | `EV-KB-001`(K001, K101-K121, K201, K202, K105), `EV-FK-Converged-UDG`, `EV-FK-Converged-UNC`, `EV-FK-Content-UDG` |
...
| `status` | `active` |
| `source_evidence_ids` | `EV-KB-001`(K013, K020-K022, K136-K146, K214), `EV-BS-001`, `EV-FK-SA-Basic`, `EV-FK-Content-UDG`, `EV-FK-PCC-UDG`, `EV-FK-PCC-UNC`, `EV-FK-Content-UNC` |
...
| `core_mechanism_combo` | `URR.METERINGTYPE(VOLUME/DURATION/EVENT) → QCT(配额计费时间,CTP/QCT两种模式) → 在线:GSU携带CC-Time/CC-Total-Octets → 离线:仅累计上报` |
| `status` | `active` |
| `source_evidence_ids` | `EV-KB-001`(K014, K024, K126-K128, K140), `EV-BS-001`, `EV-FK-Content-UDG`, `EV-FK-Duration`, `EV-FK-Volume`, `EV-FK-Event` |
...
| `status` | `active` |
| `source_evidence_ids` | `EV-KB-001`(K037, K110, K214), `EV-BS-001`, `EV-FK-Online`, `EV-FK-Converged-UDG`, `EV-FK-Converged-UNC` |
...
| `status` | `active` |
| `source_evidence_ids` | `EV-KB-001`(K216), `EV-BS-001`, `EV-FK-Content-UDG`, `EV-FK-PCC-UDG` |
...
|---------------|---------------|-------------------|-------------|-----------------|---------------------|--------------|---------------------|------------------|----------|----------------------|
| `DP-CH-01` | `L1_business` | `Namespace` | `NS-CH-01` | 计费方式选择 | 当前计费场景采用哪种计费方式 | `["离线计费","在线计费","融合计费"]` | 进入计费场景时 | 决定URR的USAGERPTMODE、是否需要CCT模板、配额控制机制、CHF/OCS/CG交互模式；选择CS-CH-01/02/03 | `active` | `EV-BS-001, EV-KB-001(K001-K004), EV-CFA` |
| `DP-CH-02` | `L1_business` | `Namespace` | `NS-CH-01` | 配置网元范围选择 | 本次计费配置需要覆盖哪些网元 | `["UPF+SMF","UPF only","SMF only"]` | 进入计费配置生成或核查任务时 | 决定生成UPF执行链、SMF编排链，还是双侧协同链；影响跨网元一致性核查范围 | `active` | `EV-BS-001, EV-CFA` |
| `DP-CH-03` | `L1_business` | `Namespace` | `NS-CH-01` | 匹配层次选择 | 业务流通过哪层匹配 | `["L34层匹配","L7-URL匹配","L7-协议匹配","L34+L7混合匹配"]` | 规划识别条件时 | 决定是否需要L7FILTER和PROTBINDFLOWF，影响配置对象链长度和Rule类型选择 | `active` | `EV-FK-SA-Basic, EV-FK-Content-UDG, EV-CFA` |
| `DP-CH-04` | `L1_business` | `Namespace` | `NS-CH-01` | 配额耗尽动作选择 | 在线/融合计费配额耗尽后如何处理 | `["BLOCK","REDIRECT","FORWARD"]` | 在线/融合计费配额耗尽 | 决定是否配置QUOTAEXHAUSTACT/REDIRECT或放行；影响用户体验切换路径 | `active` | `EV-FK-Online, EV-FK-Converged-UDG, EV-CFA` |
| `DP-CH-05` | `L1_business` | `Namespace` | `NS-CH-01` | 计费粒度选择 | 计费策略作用于哪个粒度 | `["会话级(普通计费)","业务级(内容计费)","QoS flow级(漫游计费)"]` | 确定计费方式后 | 决定是否需要SA识别和RG差异化；影响URR绑定方式和Feature选择 | `active` | `EV-KB-001(K013-K027), EV-CFA` |
| `DP-CH-06` | `L1_business` | `Namespace` | `NS-CH-01` | 计量方式选择 | 业务使用量按什么维度计量 | `["VOLUME(流量)","DURATION(时长)","EVENT(事件)"]` | 确定计费粒度后 | 决定URR.METERINGTYPE参数和Feature选择(GWFD-020302/020303/020306)；事件计费仅限在线SCUR | `active` | `EV-KB-001(K013-K027), EV-FK-Duration, EV-FK-Volume, EV-FK-Event, EV-CFA` |
| `DP-CH-07` | `L1_business` | `Namespace` | `NS-CH-01` | 兜底策略选择 | 未命中规则流量如何计费 | `["默认URR组(DFTURRGRPNAME)","异常费率(SIGURRGRPNAME)","SPECTRAFURRGRP全局兜底"]` | 规划兜底策略时 | 决定DFTURRGRPNAME和DFTSIGURRGNAME绑定方案；影响计费完整性 | `active` | `EV-FK-Content-UDG, EV-CFA` |
| `DP-CH-08` | `L1_business` | `Namespace` | `NS-CH-01` | 跨网元一致性策略 | CP和UP参数如何对齐 | `["双产品协同(UPF+SMF)","仅UPF本地规则","SMF动态下发"]` | 双产品部署或动态PCC场景 | 决定配置路径（预定义/动态/本地规则）、USERPROFILE/RULENAME/URRID跨侧一致性要求 | `active` | `EV-FK-PCC-UDG, EV-FK-PCC-UNC, EV-CFA` |
...
|-----------|-------------|-------------|------------------------|--------------------|--------------|---------------------|----------|----------------------|
| `BR-CH-01` | SA基础License开启断言 | `scope_rule` | `explicit` | `principle` | UDG侧LKV3G5SABS01+LKV3G5BCBC01+LKV3G5PCCB01、UNC侧LKV3W9SPCC11+LKV3W9BCC12必须为ENABLE | 业务感知计费能力不可用 | `active` | `EV-FK-SA-Basic, EV-FK-Content-UNC, EV-CFA` |
| `BR-CH-02` | 配置链逐层一致性校验 | `acceptance_rule` | `explicit` | `config` | RULEBINDING→RULE→PCCPOLICYGRP→URRGROUP→URR→FLOWFILTER→FILTER各层名称、绑定关系与规划值一致 | 配置链断裂导致计费不生效 | `active` | `EV-FK-Content-UDG, EV-CFA` |
| `BR-CH-03` | PFCP Usage Report一致性校验 | `acceptance_rule` | `explicit` | `config` | Usage Report中URR ID与UPF配置一致；UR-SEQN唯一 | 计费流量无法正确关联和上报 | `active` | `EV-FK-Online, EV-FK-Converged-UDG, EV-CFA` |
| `BR-CH-04` | CP/UP URR一致性诊断 | `diagnosis_rule` | `explicit` | `config` | SMF侧URR与UPF侧URR的URRID、RG、USAGERPTMODE、METERINGTYPE必须一致 | ALM-81026/81054告警，计费异常 | `active` | `EV-FK-SA-Basic, EV-FK-Content-UDG, EV-CFA` |
| `BR-CH-05` | URRID全局唯一约束 | `scope_rule` | `explicit` | `config` | URRID在所有URR对象中唯一 | PFCP Session Report无法关联 | `active` | `EV-FK-Content-UDG, EV-CFA` |
| `BR-CH-06` | 默认URR组必须配置 | `design_rule` | `explicit` | `design` | DFTURRGRPNAME和DFTSIGURRGNAME必须同时配置 | 未命中RULE的流量不会计费 | `active` | `EV-FK-Content-UDG, EV-CFA` |
| `BR-CH-07` | RG值跨侧一致性约束 | `design_rule` | `explicit` | `config` | UDG.ADD URR.RG = UNC.ADD URR.RG | 报表与实际计费不一致 | `active` | `EV-FK-Content-UDG, EV-FK-Content-UNC, EV-CFA` |
| `BR-CH-08` | REFRESHSRV时序约束 | `runtime_check_rule` | `explicit` | `ops` | REFRESHSRV必须在所有ADD/SET完成后最后执行；PROTBINDFLOWF配置后需等待60秒再执行REFRESHSRV | 部分配置不生效 | `active` | `EV-FK-SA-Basic, EV-CFA` |
| `BR-CH-09` | 配额降速优先级覆盖 | `design_rule` | `explicit` | `design` | 在线配额耗尽后的降速规则必须使用最高优先级，且覆盖原保障规则的匹配范围 | 部分流量降速不彻底 | `active` | `EV-FK-Online, EV-FK-Converged-UDG, EV-CFA` |
| `BR-CH-10` | SMF融合计费三联前置约束 | `scope_rule` | `explicit` | `config` | 融合计费需满足CHGMODE=NchfMode、CHARGECTRL或USRPROFCHARGE/APNCHARGECTRL使能、CHFINIT=SENDREQ三条件 | 不发送Initial或Initial不携带预期RG | `active` | `EV-FK-Converged-UNC, EV-FK-Converged-UDG, EV-CFA` |
| `BR-CH-11` | PROTBINDFLOWF协议匹配约束 | `selection_rule` | `explicit` | `config` | PROTOCOLNAME必须与目标网站实际协议一致（https网站必须配https）；执行后60s才生效 | L7匹配失败，流量不会被识别为指定业务 | `active` | `EV-FK-SA-Basic, EV-FK-Content-UDG, EV-CFA` |
| `BR-CH-12` | License前置门控 | `scope_rule` | `explicit` | `principle` | 内容计费需UDG侧LKV3G5BCBC01+UNC侧LKV3W9BCC12双License同时开启；融合计费需WSFD-011206 License | 配置命令成功但功能不生效，难以排查 | `active` | `EV-FK-Content-UDG, EV-FK-Content-UNC, EV-CFA` |
| `BR-CH-13` | Credit Pooling共享配额池约束 | `scope_rule` | `explicit` | `design` | Credit Pooling共享配额池内URR的METERINGTYPE必须一致；Pool容量上限由OCS/CHF控制，不可本地越界配置 | 配额统计混乱，多业务共享配额溢出 | `active` | `EV-FK-Online, EV-CFA` |
| `BR-CH-14` | 费率切换时序约束 | `runtime_check_rule` | `explicit` | `ops` | FESTIVAL/WEEKDAY/TARIFFGROUP必须在费率切换周期开始前配置完成；费率切换命令组必须在URR三件套之后、REFRESHSRV之前执行 | 费率切换不生效或计费错误 | `active` | `EV-FK-Offline-UNC, EV-FK-Converged-UNC, EV-CFA` |
| `BR-CH-15` | MEC多UPF独立配额约束 | `scope_rule` | `explicit` | `design` | MEC场景下多UPF独立配额要求各UPF的URR ID全局唯一且配额不互相干扰；需要独立的CHF会话和Usage Report上报通道 | 多UPF间配额冲突或重复计费 | `active` | `EV-FK-Converged-UNC, EV-CFA` |
| `BR-CH-16` | 互操作计费连续性约束 | `design_rule` | `explicit` | `principle` | 5GS↔EPS互操作切换时，目标系统必须继承源系统的计费上下文(RG/URR ID/配额余额)；切换前后Usage Report必须连续不中断 | 切换期间计费中断或话单丢失 | `active` | `EV-FK-Converged-UNC, EV-CFA` |

> **rule_type说明**：与带宽场景对齐Schema标准枚举。`scope_rule`=范围约束，`acceptance_rule`=验收规则，`design_rule`=设计规则，`diagnosis_rule`=诊断规则，`selection_rule`=选择规则，`runtime_check_rule`=运行时检查规则。
> **扩展场景规则（BR-CH-13~16）**：源自R03完整性审查H-06，覆盖Credit Pooling、费率切换、MEC、互操作4个扩展场景。
...
|----------------------|------------------------|--------------------|--------------|-----------------------|
| `SO-CH-01` | 报文解析 | L34协议解析和L7 URL解析，是计费匹配的前置能力 | FILTER, L7FILTER | `EV-FK-SA-Basic, EV-FK-Content-UDG, EV-CFA` |
| `SO-CH-02` | 协议识别 | 识别协议/应用（如http/https/IM协议组），用于计费分流 | PROTBINDFLOWF | `EV-FK-SA-Basic, EV-FK-Content-UDG, EV-CFA` |
| `SO-CH-03` | 过滤条件 | 将业务描述翻译为可匹配的FILTER/FLOWFILTER/FLOWFILTERGRP组合 | FILTER, FLOWFILTER, FLOWFILTERGRP, FLTBINDFLOWF | `EV-FK-Content-UDG, EV-FK-SA-Basic, EV-CFA` |
| `SO-CH-04` | 规则语义 | 将匹配条件与计费动作绑定为RULE，计费场景POLICYTYPE固定为PCC | RULE(POLICYTYPE=PCC) | `EV-FK-PCC-UDG, EV-FK-Content-UDG, EV-CFA` |
| `SO-CH-05` | 策略语义 | 计费场景固定为PCC策略类型，封装URRGROUP为可被RULE引用的策略组 | PCCPOLICYGRP | `EV-FK-PCC-UDG, EV-CFA` |
| `SO-CH-06` | 优先级语义 | 多规则同时命中时按PRIORITY裁决（越小越高），特定业务优先于兜底 | RULE.PRIORITY | `EV-FK-PCC-UDG, EV-FK-Content-UDG, EV-CFA` |
| `SO-CH-07` | 绑定语义 | UserProfile承载规则绑定(RULEBINDING)和默认计费组(URRGRPBINDING) | USERPROFILE, RULEBINDING, URRGRPBINDING | `EV-FK-Content-UDG, EV-FK-PCC-UDG, EV-CFA` |
| `SO-CH-08` | 计费语义 | 差异化计费(RG)、免费业务(FREE RG)、默认计费(兜底)；核心三件套URR→URRGROUP→PCCPOLICYGRP | URR, URRGROUP, PCCPOLICYGRP | `EV-FK-Content-UDG, EV-CFA` |
| `SO-CH-09` | 配额语义 | 在线/融合计费的配额控制(GSU/Requested-Unit)、阈值触发和耗尽动作(Final-Unit-Action) | CCT, QUOTAEXHAUSTACT, DCCTEMPLATE | `EV-FK-Online, EV-FK-Converged-UDG, EV-FK-Converged-UNC, EV-CFA` |
| `SO-CH-10` | 计费协议栈-Ga/Gy ★ | Ga接口(离线,GTP')、Gy接口(在线,DCC/Diameter Credit Control)、DCC会话(CCR-I/U/T)、MSCC多业务信用控制 | CG配置对象, OCS配置对象, DIAMCONNGRP | `EV-PK-Ga-Gy-DCC` |
| `SO-CH-11` | 计费协议栈-N40/PFCF ★ | N40接口(融合,Nchf/HTTP2)、PFCP协议(N4,Usage Report)、Gx接口(4G,PCRF↔PCEF/PGW) | HTTPLEGRP/HTTPLE/SBIAPLE, TNFINS/TNFGRP, SELECTCHFGBYCC | `EV-PK-N40-PFCP-Gx` |
| `SO-CH-12` | 核查与诊断语义 | 配置链逐层LST回查、PFCP Usage Report验证、EMS_CtfErrorRpt信令跟踪、告警诊断(ALM-81026/81054/100530) | LST全链路命令集, PFCP Session Report, EMS_CtfErrorRpt | `EV-FK-SA-Basic, EV-CFA` |
