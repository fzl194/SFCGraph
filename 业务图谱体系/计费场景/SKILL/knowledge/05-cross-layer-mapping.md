...
|------|---------|------|---------|---------|
| 1 | `BR-CH-01`（License前置断言） | `refined_by` | `FR-CH-01`（SA特征License依赖） | BR"License必须ENABLE"→FR"GWFD-110101依赖LKV3G5SABS01" |
| 2 | `BR-CH-02`（配置链逐层一致性） | `refined_by` | `FR-CH-03`（PCC策略组绑定完整性） | BR"全链名称一致"→FR"PCCPOLICYGRP必须包含至少1个URRGROUP引用" |
| 3 | `BR-CH-08`（REFRESHSRV时序约束） | `refined_by` | `TR-CH-03`（PROTBINDFLOWF等待60秒） | BR"60秒后刷新"→TR"T-003配置PROTBINDFLOWF后延迟执行后续任务" |
| 4 | `BR-CH-06`（默认URR组必须配置） | `refined_by` | `TR-CH-04`（URRGRPBINDING必填） | BR"DFTURRGRPNAME+DFTSIGURRGNAME"→TR"T-104必须绑定默认URR组" |
| 5 | `BR-CH-11`（PROTBINDFLOWF协议约束） | `refined_by` | `CR-CH-11`（PROTOCOLNAME与协议一致） | BR"https配https"→CR"ADD PROTBINDFLOWF.PROTOCOLNAME ∈ {http,https}" |

> **完整覆盖**：以上5条覆盖了BR→FR、BR→TR、TR→CR三层refined_by关系。其余BR（如BR-CH-03/04/05/07/09/10/12）暂无直接下层规则精化，作为独立高层约束存在。
...
- [x] 所有Feature的decomposes_to指向真实存在的ConfigTask（27个）
- [x] 所有ConfigTask的invokes指向真实存在的MMLCommand（87个）
- [x] 所有MMLCommand的operates_on指向真实存在的ConfigObject（55个）
- [x] 所有ConfigTask的targets指向真实存在的SemanticObject（12个）或ConfigObject
- [x] 所有DP的selects指向真实存在的CS（7个）
- [x] 路径A~F端到端完整：BD→NS→CS→Feature→Task→Command→Object
- [x] 无 §13 禁止关系
