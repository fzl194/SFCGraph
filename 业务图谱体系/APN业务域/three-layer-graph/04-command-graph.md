...
> **★版本状态**：APN 命令层重建版 v2 — **37 / 37 特性命令层全部已从产品文档抽取并入 §1**（100% 源自激活/参考信息/特性概述 + MML 命令手册原文，零编造）。其中簇B/D/E-core/A/C（31 特性）§1-§8 内联详表；簇F（§1.8，27 命令）+ 簇E 尾 GRE/IPSec-UNC（§1.9，复用型）命令表已并入，全参数/ConfigObject/CommandRule/脚本见各 draft（`draft/04-cluster-*.md`，16 文件）。旧版由 git 可恢复（`git show HEAD:.../04-command-graph.md`）。
...
### 1.8 簇F 网元选择 + 接入控制（WSFD-107010 / WSFD-010202 / WSFD-106003 / GWFD-010151，27 命令）

> **⚠️文档缺口**：WSFD-107010 UPF选择仅 1 篇特性概述（无激活/参考信息），命令清单来自概述表1「UPF选择原则」嵌入式 MML 手册引用（18 命令），激活脚本为概述推导（§8 标注）。命令/参数 100% 手册原文，零编造。全参数表/ConfigObject(24)/CommandRule(9)/脚本见 `draft/04-cluster-F-网元接入控制.md`。

| `command_id` | `command_name` | `verb`/`object` | `command_summary` | 关键参数（数） | `source_evidence_ids` |
|--------------|----------------|---------------|-------------------|----------|----------------------|
| CMD-UNC-107010-01 | ADD PNFPROFILE | ADD/PNFPROFILE | ★UPF选择核心·NF概述（NFTYPE含NfUPF；PRIORITY/CAPACITY 仅 PRIORITYFLAG=ENABLE 生效；高危） | NFINSTANCEID,NFTYPE,FQDN,IPV4/V6ADDR,CAPACITY,PRIORITY,LOAD,LOCALITY（16） | EV-FK-107010 |
| CMD-UNC-107010-02 | ADD PNFDNN | ADD/PNFDNN | 对端DNN（必选·用户DNN；PRISWITCH/CAPSWITCH） | INDEX,NFINSTANCEID,DNN,PNFNSINDEX,PRISWITCH,PRIORITY,CAPSWITCH,CAPACITY（9） | EV-FK-107010 |
| CMD-UNC-107010-03 | ADD PNFNS | ADD/PNFNS | 对端切片（必选·用户切片；SST+SD） | INDEX,NFINSTANCEID,SST,SD（5） | EV-FK-107010 |
| CMD-UNC-107010-04 | ADD PNFDNAI | ADD/PNFDNAI | 对端DNAI（必选·DNAI） | INDEX,NFINSTANCEID,DNAI,PNFDNNINDEX,PRISWITCH...（9） | EV-FK-107010 |
| CMD-UNC-107010-05 | ADD PNFUPFINFO | ADD/PNFUPFINFO | 对端UPF信息（EPS互通 IWKEPSIND） | NFINSTANCEID,IWKEPSIND,PDUSESSIONTYPE（3） | EV-FK-107010 |
| CMD-UNC-107010-06 | ADD PNFSMFSERAREA | ADD/PNFSMFSERAREA | 对端SMF服务区（必选·位置区；"*"支持所有） | NFINSTANCEID,SMFSERVINGAREA,PNFNSINDEX,PRISWITCH...（7） | EV-FK-107010 |
| CMD-UNC-107010-07 | ADD PNFTAI | ADD/PNFTAI | 对端TAI（必选·位置区TAI；"*"支持所有） | NFINSTANCEID,TAI,PNFNSINDEX,PRISWITCH...（8） | EV-FK-107010 |
| CMD-UNC-107010-08 | ADD PNFTAIRANGE | ADD/PNFTAIRANGE | 对端TAI范围（QUERYTYPE=START_END/PATTERN） | INDEX,NFINSTANCEID,MCC,MNC,QUERYTYPE,RANGESTART/END...（13） | EV-FK-107010 |
| CMD-UNC-107010-09 | ADD UPNODE | ADD/UPNODE | ★UPF节点特征（28参数；LOCATION=Local/Central/CentralOnly；高危：SUPULCLBPONLY/LOCAL不能作I-UPF） | NFINSTANCENAME,LOCATION,UPFUNCTION,LOCK,ADDRALLOCMODE,SUPULCLBPONLY...（28） | EV-FK-107010 |
| CMD-UNC-107010-10 | ADD UPAREA | ADD/UPAREA | UPF服务区（AREATYPE=S1TAI/N2TAI/LAI） | AREANAME,AREATYPE（2） | EV-FK-107010 |
| CMD-UNC-107010-11 | ADD UPAREABINDN2TAI | ADD/UPAREABINDN2TAI | 服务区绑5G TAI范围（各范围不允许交集） | AREANAME,N2BEGINTAI,N2ENDTAI（3） | EV-FK-107010 |
| CMD-UNC-107010-12 | ADD LOCBINDAREA | ADD/LOCBINDAREA | 位置绑优先服务区（LOCALITYFLAG=ENABLE生效） | LOCALITY,AREANAME（2） | EV-FK-107010 |
| CMD-UNC-107010-13 | ADD UPBINDS11 | ADD/UPBINDS11 | SGW-U绑S11（PRIORITY 0~255） | NFINSTANCENAME,S11IPTYPE,S11IPV4/V6,PRIORITY（5） | EV-FK-107010 |
| CMD-UNC-107010-14 | ADD UPBINDGNGP | ADD/UPBINDGNGP | GGSN/PGW-U绑GnGp/S5S8 | NFINSTANCENAME,GNGPIPTYPE,GNGPIPV4/V6,PRIORITY（5） | EV-FK-107010 |
| CMD-UNC-107010-15 | SET UPSELECTPRI | SET/UPSELECTPRI | UPF选择策略次序（三级不可相同） | FIRSTPRIORITY,SECONDPRIORITY,THIRDPRIORITY（3） | EV-FK-107010 |
| CMD-UNC-107010-16 | SET UPSELECTFLAG | SET/UPSELECTFLAG | ★UPF选择条件开关（15开关；PRIORITYFLAG=ENABLE后PNF系列优先级生效） | PRIORITYFLAG,LOCALITYFLAG,N3UPFAPNFLAG,ULISGWFLAG,AMBRUPFFLAG,LOADFLTFLAG...（15） | EV-FK-107010 |
| CMD-UNC-107010-17 | SET APNUPSELPLY | SET/APNUPSELPLY | APN粒度UP选择策略（覆盖全局） | APN,PSAPOSPRIFLAG,ULISGWFLAG,PRIORITYFLAG,COMBINEPRISTG...（8） | EV-FK-107010 |
| CMD-UNC-107010-18 | SET UPLOADBALANCE | SET/UPLOADBALANCE | UP负载均衡（LIGHT/HEAVYLOAD门限） | LOADCTRLFLG,LIGHTLOAD,HEAVYLOAD,OVERLOADCTRLFLG（4） | EV-FK-107010 |
| CMD-UNC-010202-01 | ADD AREADNS | ADD/AREADNS | ★位置区域DNS域名（DNTYPE 14种；ZONESW=YES时ZONENAME必填） | DNTYPE,LAC,LACRANGE,RAC,TAC,TACRANGE,UUT,ZONESW,ZONENAME,ZONELOC（11） | EV-FK-010202 |
| CMD-UNC-010202-02 | ADD IPV4DNSH | ADD/IPV4DNSH | IPv4 DNS Hostfile（一主机名最多64 IP） | HSINDEX,HOSTNAME,ADDRSECTION,IPV4ADDR1~8,PRIORITY1~8,WEIGHT1~8（7类） | EV-FK-010202 |
| CMD-UNC-010202-03 | ADD DNSN | ADD/DNSN | DNS NAPTR（FQDN→网元接口；ENTITY 8类+INTYPE 13类约束） | FQDN,HSINDEX,ENTITY,INTYPE,S5PROTOCOL,S8PROTOCOL,SMF,PRIORITY,WEIGHT（13） | EV-FK-010202 |
| CMD-UNC-106003-01 | ADD NGMMSUBDATA | ADD/NGMMSUBDATA | ★AMF本地移动性管理（★本地优先于签约；匹配优先级 IMSI>IMSI_PREFIX>FOREIGN/HOME>ALL） | SUBRANGE,IMSIPRE,IMSI,RATRESTRICT,CORERESTRICT,RFSPINDEX,UEAMBRULK/DLK（17） | EV-FK-106003 |
| CMD-UNC-106003-02 | SET NGMMPROCTRL | SET/NGMMPROCTRL | 5G MM流程控制（PROT 14流程；60+拒绝原因值；高危） | PROT,AUSFDISC*/AIR*/AUTH*/UDM*/ACCRST...（60+） | EV-FK-106003 |
| CMD-UNC-106003-03 | ADD GBARD | ADD/GBARD | Gb模式接入限制（SGSN/GERAN；CARDTYPE SIM/USIM；3控制方式） | SUBRANGE,IMSIPRE,BEGIMSI,ENDIMSI,APNNI,CARDTYPE,ARD,CTRLTYPE,CAUSE（10） | EV-FK-106003 |
| CMD-UNC-106003-04 | ADD IUARD | ADD/IUARD | Iu模式接入限制（SGSN/UTRAN；同GBARD+CSI） | 同GBARD（10） | EV-FK-106003 |
| CMD-UNC-106003-05 | ADD S1ARD | ADD/S1ARD | S1模式接入限制（MME/E-UTRAN；★无CARDTYPE） | SUBRANGE,IMSIPRE,BEGIMSI,ENDIMSI,APNNI,ARD,CTRLTYPE,CAUSE（9） | EV-FK-106003 |
| CMD-UDG-010151-01 | SET APNQOSATTR | SET/APNQOSATTR | ★APN级QoS U面带宽流控（CARSHAPESWUL/DL三态+CARSHAPE二选；★与C面接入权限机制不同，非C-U对称） | APN,CARSHAPESWUL,CARSHAPEUL,CARSHAPESWDL,CARSHAPEDL（5） | EV-FK-010151 |

> 簇F 关键 CommandRule（9条，详见 draft §4）：CR-107010-01 UPF三轮筛选次序（必选→优选→负载）/ CR-107010-02 PNF系列PRIORITY依赖PRIORITYFLAG / CR-107010-04 ADD UPNODE高危影响I-UPF / CR-010202-01 AREADNS DNTYPE决定条件参数 / CR-106003-01 AMF本地优先签约 / CR-106003-02 卡类型控制仅GERAN/UTRAN依赖鉴权 / CR-010151-01 CARSHAPE依赖CARSHAPESW。

### 1.9 簇E 尾 GRE + IPSec-UNC（IPFD-015002 / IPFD-016000，复用型）

> **GRE（IPFD-015002）**：4 命令（ADD/MOD/RMV/LST GRETUNNEL），**ADD GRETUNNEL 复用簇E IPSec draft CMD-UDG-015004-19（18参数，手册同源同 id `_00841729`）**，`used_by_features` 扩展含 015002。独有：CR-015002-01 GRE 最多两层嵌套（第三层 Down，critical）/ 源地址互斥 / GRE Key 两端一致。UDG 仅调测文档，UNC 有完整激活。全参数/CR 见 `draft/04-cluster-E-GRE-015002.md`。
>
> **IPSec-UNC（IPFD-016000）**：经逐文件比对，UNC `平台服务管理/IPSEC功能管理/` 手册树与 UDG **文件 ID 逐一完全相同**（25 命令手册 `_80432524`/`_26032189` 等全部相同），激活文档（普通IPv4 `_61317238`、国密 `_03728909`）命令/参数/脚本/手册路径逐字节一致 → **U+C 共享统一 IPSec 命令模块**。UNC 独有命令 0；25 命令全部 `used_by_features` 加 016000 引用 UDG 015004 权威参数（不重复维护）。UNC 独有 1 条 CR（CR-016000-UNC-09：不支持 GREv6-over-IPSecv6 等 5 类场景，源自 UNC 概述）。★**反向补全 UDG 缺口**：UDG 015004 标⚠️的 ADD IFIPV6ADDRESSIPSEC，UNC 手册已定位 `_21521206`，可反查补全 UDG。详见 `draft/04-cluster-E-IPSec-016000-UNC.md`。

| `command_id` | `command_name` | 复用/独有 | 说明 |
|--------------|----------------|----------|------|
| CMD-UDG-015002-01 | ADD/MOD/RMV/LST GRETUNNEL | 复用 CMD-UDG-015004-19 | 18参数同表；GRE 独有 REDUNDANCYEN（010107静态冗余复用）/ 两层嵌套约束 |
| CMD-UNC-016000-* | IPSec 命令族 25 条 | 复用 CMD-UDG-015004-* | U+C 同名同参同手册 id；UNC 独有 CR-016000-UNC-09（5类不支持场景） |
