...
| 输入 | 内容 | 来源 |
|---|---|---|
| ① 已抽 yaml | atom task + atom 挂的 rule/DP | `assert/.../tasks/task-0-*.yaml` + `task_rules/rule-*.yaml` + `decision_points/dp-*.yaml` |
...
**产物**：`assert/UDG/20.15.2/command-review-index.json`（187 个审查卡，key=atom 短 id）
每卡汇总：atom+rule/DP、命令真相参数（精确解析）、已绑 vs 缺口、用该命令的特性清单。供**批次规划/统计**用。

### 3.2 结构化证据包（审查主输入）★
**脚本**：`task-build-skill/scripts/build_command_evidence.py`
**产物**：`assert/UDG/20.15.2/command-evidence/{atom短id}.md`（187 个，每命令一个）
...
- **批次进度**：`assert/UDG/20.15.2/command-review-tracker.md`（187 atom 逐行，标 ✅done/⏳，含命令名/参数数/有无 rule·DP）。
