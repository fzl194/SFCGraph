# 命令证据包：MOD RULE
> 原始命令 md：`output/UNC 20.15.2 产品文档(裸机容器) 05/OM参考/命令/UNC MML命令/业务服务管理/会话管理/计费和策略的业务管理/业务模板/规则/修改规则（MOD RULE）_09897202.md`
> 用该命令的特性数：2
...
| PRIORITY | 全局优先级 | 对端协商 | optional | 无 | 整数类型，取值范围为1～4294967295。值越小优先级越高。 |
...
| RULERANGE | 规则生效范围 | global_planned | conditional | 无 | 枚举类型。 |
| NWDAFEVENTS | NWDAF数据分析事件 | global_planned | optional | 无 | 枚举类型。 |
| IPERULEDELYSW | 智能规则传递开关 | global_planned | conditional | 无 | 枚举类型。 |
...
## ④ 自动比对
- 命令真相参数（8）：['FLOWFILTERNAME', 'IPERULEDELYSW', 'NWDAFEVENTS', 'POLICYNAME', 'POLICYTYPE', 'PRIORITY', 'RULENAME', 'RULERANGE']
