# 设置节点检测参数（SET NODECHKPARA）

- [命令功能](#ZH-CN_MMLREF_0000001095265008__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001095265008__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001095265008__1.3.3)
- [使用实例](#ZH-CN_MMLREF_0000001095265008__1.3.4)
- [输出结果说明](#ZH-CN_MMLREF_0000001095265008__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000001095265008)
...
## [参数说明](#ZH-CN_MMLREF_0000001095265008)
...
## [使用实例](#ZH-CN_MMLREF_0000001095265008)
...
## [输出结果说明](#ZH-CN_MMLREF_0000001095265008)

命令执行正常，会返回命令执行成功的提示信息。输出结果说明如 [表1](#ZH-CN_MMLREF_0000001095265008__table819976173217) 所示。

该命令执行异常，会返回对应的错误码。常见错误码如 [表2](#ZH-CN_MMLREF_0000001095265008__table46145803712) 所示。

*表1 输出项说明*

| 输出项名称 | 输出项解释 |
| --- | --- |
| 资源类型 | 用于返回本次设置的资源类型。<br>取值范围：<br>“虚拟机” |
| 参数类型 | 用于返回本次设置的参数类型。<br>取值范围：<br>- “节点内存检测”<br>- “节点CPU过载检测”<br>- “inotify实例检测”<br>- “inotify监控检测”<br>- “节点信号量集检测”<br>- “节点文件句柄检测”<br>- “网卡检测”<br>- “IP路由检测”<br>- “关键文件检测”<br>- “分区过载检测”<br>- “进程数检测”<br>- “inode使用率检测”<br>- “容器存储空间检测”<br>- “IP互串冲突预防”<br>- “智算单元资源检测” |
| 详细信息 | 用于返回本次设置的具体执行结果。 |

*表2 错误码列表*

| 错误码 | 错误码解释 | 原因分析 | 处理建议 |
| --- | --- | --- | --- |
| 130001 | 没有符合条件的节点 | 所要设置的节点或者网元ID不存在，无相应节点检测参数信息。 | 按照<br>[**DSP NODE**](节点查询（DSP NODE）_71678755.md)<br>命令查询的所有节点信息，选择确实存在的节点，设置其相应的节点检测类型对应的检测参数。 |
| 130002 | 操作失败 | 内部错误。 | 请联系<br>华为<br>技术支持。 |
| 130004 | 部分成功 | 设置部分节点检测参数失败。 | 请联系<br>华为<br>技术支持。 |
| 130060 | 访问ETCD失败 | ETCD故障导致设置参数失败。 | 请联系<br>华为<br>技术支持。 |
| 130009 | 参数不合法 | 设置的参数值不在规定范围之内。 | 按照规定的范围进行参数值的设置。 |
| 130010 | 资源类型与实际不匹配 | 所要设置的硬件资源类型与当前环境实际的硬件资源类型不匹配。 | 按照实际的资源类型进行设置。 |
| 130011 | 操作太频繁，请稍后重试 | 当参数类型设置为网卡检测、IP路由检测、关键文件检测、分区过载检测、进程数检测、inode使用率检测、容器存储空间检测或IP互串冲突预防时，5分钟内只能设置一次。 | 等待5分钟后再进行设置。 |
