# 显示NETCONFC与其他组件的消息交互信息（DSP NCCDIAGLOG）

- [命令功能](#ZH-CN_CONCEPT_0259103509__1.3.1.1)
- [注意事项](#ZH-CN_CONCEPT_0259103509__1.3.2.1)
- [操作用户权限](#ZH-CN_CONCEPT_0259103509__1.3.3.1)
- [参数说明](#ZH-CN_CONCEPT_0259103509__1.3.4.1)
- [使用实例](#ZH-CN_CONCEPT_0259103509__1.3.5.1)
- [输出结果说明](#ZH-CN_CONCEPT_0259103509__1.3.6.1)

#### [命令功能](#ZH-CN_CONCEPT_0259103509)

该命令用于显示NETCONFC与其他组件的消息交互信息。

#### [注意事项](#ZH-CN_CONCEPT_0259103509)

该命令执行后立即生效。

#### [操作用户权限](#ZH-CN_CONCEPT_0259103509)

G_1，管理员级别命令组

#### [参数说明](#ZH-CN_CONCEPT_0259103509)
...
#### [输出结果说明](#ZH-CN_CONCEPT_0259103509)

| 输出项名称 | 输出项解释 |
| --- | --- |
| 输出字符串 | 该参数用于表示诊断信息数据输出字符串。<br>- Time：操作的时间点。<br>- Event：建连的事件。<br>- FromAppName：Schema操作的业务名称。<br>- ChannelID：NETCONFC分配的通道ID。<br>- OriChannelID：业务携带的通道ID。<br>- SvrSessionID：NETCONF会话ID。<br>- Terminal：目的地址。<br>- MsgID：RPC报文的message id。<br>- Dir：报文的传送方向。取值范围：枚举类型。NCC->NCS：从NETCONFC发送至NETCONFS。NCS->NCC：从NETCONFS发送至NETCONFC。<br>- Method：操作类型。取值范围：枚举类型。POST：创建。DELETE：删除。PUT：修改。GET：查询。ACTION：维护。<br>- Operation：Schema操作的URI和BODY简要信息。 |
