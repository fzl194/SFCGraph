# 显示同步到业务进程的通道信息（DSP PAECHANNELSYNC）

- [命令功能](#ZH-CN_CONCEPT_0192520035__1.3.1.1)
- [注意事项](#ZH-CN_CONCEPT_0192520035__1.3.2.1)
- [操作用户权限](#ZH-CN_CONCEPT_0192520035__1.3.3.1)
- [参数说明](#ZH-CN_CONCEPT_0192520035__1.3.4.1)
- [使用实例](#ZH-CN_CONCEPT_0192520035__1.3.5.1)
- [输出结果说明](#ZH-CN_CONCEPT_0192520035__1.3.6.1)

#### [命令功能](#ZH-CN_CONCEPT_0192520035)

该命令用于显示同步到业务进程的通道信息。

#### [注意事项](#ZH-CN_CONCEPT_0192520035)

该命令执行后立即生效。

#### [操作用户权限](#ZH-CN_CONCEPT_0192520035)

G_1，管理员级别命令组；G_2，操作员级别命令组；G_3，用户级别命令组

#### [参数说明](#ZH-CN_CONCEPT_0192520035)
...
#### [输出结果说明](#ZH-CN_CONCEPT_0192520035)

| 输出项名称 | 输出项解释 |
| --- | --- |
| 通道ID | 全局通道ID。 |
| 通道名称 | 通道名称。 |
| 通道类型 | 通道类型分为：NULL、默认、透传、转发链、硬件加速、直连、服务和低速。 |
| TP | 目标端口。 |
| 优先级为0的队列ID | 优先级为0的队列ID。 |
| 优先级为1的队列ID | 优先级为1的队列ID。 |
| 优先级为2的队列ID | 优先级为2的队列ID。 |
| 优先级为3的队列ID | 优先级为3的队列ID。 |
| 优先级为4的队列ID | 优先级为4的队列ID。 |
| 优先级为5的队列ID | 优先级为5的队列ID。 |
| 优先级为6的队列ID | 优先级为6的队列ID。 |
| 优先级为7的队列ID | 优先级为7的队列ID。 |
