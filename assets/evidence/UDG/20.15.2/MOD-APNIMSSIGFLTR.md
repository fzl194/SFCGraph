# 修改APN的IMS分类器（MOD APNIMSSIGFLTR）

- [命令功能](#ZH-CN_CONCEPT_0182837826__1.3.1.1)
- [注意事项](#ZH-CN_CONCEPT_0182837826__1.3.2.1)
- [操作用户权限](#ZH-CN_CONCEPT_0182837826__1.3.3.1)
- [参数说明](#ZH-CN_CONCEPT_0182837826__1.3.4.1)
- [使用实例](#ZH-CN_CONCEPT_0182837826__1.3.5.1)

#### [命令功能](#ZH-CN_CONCEPT_0182837826)

**适用NF：PGW-U、UPF**

![](修改APN的IMS分类器（MOD APNIMSSIGFLTR）_82837826.assets/notice_3.0-zh-cn.png)

本命令属于高危命令，请确认修改规则后合法的IMS信令报文都能命中其中一条规则。

该命令用于修改APN下的IMS信令专用上下文的Filter。使用VoLTE语音业务时，需要使用该命令修改IMS信令专用上下文的Filter来设置报文的过滤规则。

#### [注意事项](#ZH-CN_CONCEPT_0182837826)

- 该命令执行后只对新激活用户生效。
- 执行该命令时，如果需要修改APN参数，只能修改为做IMS业务的APN，且规则修改后要保证所有合法IMS业务能够命中。如果配置不当，将会影响IMS语音信令的转发，造成语音不通。

#### [操作用户权限](#ZH-CN_CONCEPT_0182837826)

G_1，管理员级别命令组；G_2，操作员级别命令组

#### [参数说明](#ZH-CN_CONCEPT_0182837826)
...
| IPVERSION | IP地址版本 | 可选必选说明：可选参数<br>参数含义：该参数用于指定过滤规则是IPv4类型还是IPv6类型。<br>数据来源：本端规划<br>取值范围：枚举类型。<br>- IPV4：IPv4地址类型。<br>- IPV6：IPv6地址类型。<br>默认值：无<br>配置原则：无 |
...
| PROTOCOL | 协议类型 | 可选必选说明：条件必选参数<br>前提条件：该参数在“PROSPECTYPE”配置为“SPECIFY”时为必选参数。<br>参数含义：该参数用于指定协议号。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围为1～255。协议类型tcp、udp，对应的协议号为6、17。<br>默认值：无<br>配置原则：当指定了端口的时候，取值只能是6或者17。 |

#### [使用实例](#ZH-CN_CONCEPT_0182837826)

根据网络规划，需要在APN下修改一个Filter：

```
MOD APNIMSSIGFLTR:APN="huawei.com",PRIORITY=8,IPVERSION=IPV4,SRCIPV4SPECTYPE=SPECIFY,SRCIPV4ADDR="10.23.23.23",SRCIPV4MASKLEN=32,SRCPORTSPECTYPE=SPECIFY,SRCPORTSTART=1,SRCPORTEND=65535,DSTIPV4SPECTYPE=SPECIFY,DSTIPV4ADDR="10.5.5.5",DSTIPV4MASKLEN=1,DSTPORTSPECTYPE=SPECIFY,DSTPORTSTART=50,DSTPORTEND=80,PROSPECTYPE=SPECIFY,PROTOCOL=6;
```
