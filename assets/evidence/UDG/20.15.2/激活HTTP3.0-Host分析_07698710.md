...
- [操作场景](#ZH-CN_OPI_0000001407698710__1.3.1)
- [必备事项](#ZH-CN_OPI_0000001407698710__1.3.2)
- [操作步骤](#ZH-CN_OPI_0000001407698710__1.3.3)
- [任务示例](#ZH-CN_OPI_0000001407698710__1.3.4)

## [操作场景](#ZH-CN_OPI_0000001407698710)

在 UDG 上激活HTTP3.0 Host分析功能。当用户访问的业务采用HTTP3.0协议承载时，需要部署HTTP3.0 Host分析特性，从而实现对业务实施计费与策略控制。
...
## [必备事项](#ZH-CN_OPI_0000001407698710)

前提条件

- 请仔细阅读[特性概述](特性概述_58218397.md)。
- UDG完成与周边网元的对接。
...
| [**ADD URR**](../../../../../OM参考/命令/UDG MML命令/用户面服务管理/业务控制策略/计费控制/使用率上报规则/增加URR（ADD URR）_82837629.md) | URR标识（URRID） | 12002 | 全网规划 | 使用量上报规则信息，要求<br>UDG<br>和CG两端的URRID取值要一致。 |
...
| [**ADD L7FILTER**](../../../../../OM参考/命令/UDG MML命令/用户面服务管理/业务匹配策略/业务过滤器管理/七层规则管理/七层过滤器/增加七层过滤器（ADD L7FILTER）_82837397.md) | URL（URL） | graph.facebook.com | 本端规划 | 配置7层过滤条件。 |
...
| [**ADD PROTBINDFLOWF**](../../../../../OM参考/命令/UDG MML命令/用户面服务管理/业务匹配策略/业务过滤器管理/流过滤器管理/流过滤器的协议绑定/增加流过滤器协议绑定关系（ADD PROTBINDFLOWF）_82837370.md) | 协议名称（PROTOCOLNAME） | Quic | 本端规划 | 绑定的七层协议。 |
...
## [操作步骤](#ZH-CN_OPI_0000001407698710)
...
6. 配置内容计费规则绑定用户模板。
  ...
  > **说明**
  > 如果进行HTTP3.0 Host分析功能，需解析SNI或者证书字段以获取Host，再基于此Host进行计费和业务控制。

## [任务示例](#ZH-CN_OPI_0000001407698710)

任务描述

开启 UDG 的HTTP3.0 Host分析功能， UDG 将访问quic业务且URL为graph.facebook.com的用户业务报文记入使用量上报规则名称urr_red，进行离线计费。
...
```
ADD URR:URRNAME="urr_red",URRID=12002,USAGERPTMODE=OFFLINE;
...
```
ADD L7FILTER:L7FILTERNAME="l7-test",SUBL7FLTNAME="sl7-test",URL="graph.facebook.com";
ADD FLOWFILTER:FLOWFILTERNAME="fg_red";
ADD PROTBINDFLOWF:FLOWFILTERNAME="fg_red",PROTOCOLNAME="quic",L7FILTERNAME="l7-test";
```
