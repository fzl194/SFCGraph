...
- [操作场景](#ZH-CN_OPI_0124082592__1.3.1)
- [必备事项](#ZH-CN_OPI_0124082592__1.3.2)
- [操作步骤](#ZH-CN_OPI_0124082592__1.3.3)
- [任务示例](#ZH-CN_OPI_0124082592__1.3.4)

## [操作场景](#ZH-CN_OPI_0124082592)

在 UDG 上激活HTTP2.0 Host识别功能。当用户访问的业务采用HTTP2.0协议承载时，需要部署HTTP2.0 Host识别特性，对业务实施计费与策略控制。

此配置实例以离线计费为例，进行七层业务的内容计费配置。

> **说明**
> 适用于PGW-U、UPF。

## [必备事项](#ZH-CN_OPI_0124082592)

前提条件

- 请仔细阅读[GWFD-110201 HTTP2.0 Host识别特性概述](GWFD-110201 HTTP2.0 Host识别特性概述_24082586.md)。
- UDG完成与周边网元的对接。
...
| --- | --- | --- | --- | --- |
| [**ADD URR**](../../../../../OM参考/命令/UDG MML命令/用户面服务管理/业务控制策略/计费控制/使用率上报规则/增加URR（ADD URR）_82837629.md) | 使用量上报规则名称（URRNAME） | urr_red | 本端规划 | 使用量上报规则信息，要求<br>UDG<br>和UNC两端的URRID取值要一致。 |
| [**ADD URR**](../../../../../OM参考/命令/UDG MML命令/用户面服务管理/业务控制策略/计费控制/使用率上报规则/增加URR（ADD URR）_82837629.md) | URR标识（URRID） | 12000 | 全网规划 | 使用量上报规则信息，要求<br>UDG<br>和UNC两端的URRID取值要一致。 |
| [**ADD URR**](../../../../../OM参考/命令/UDG MML命令/用户面服务管理/业务控制策略/计费控制/使用率上报规则/增加URR（ADD URR）_82837629.md) | 使用量上报模式（USAGERPTMODE） | OFFLINE | 全网规划 | 使用量上报规则信息，要求<br>UDG<br>和UNC两端的URRID取值要一致。 |
...
| [**ADD L7FILTER**](../../../../../OM参考/命令/UDG MML命令/用户面服务管理/业务匹配策略/业务过滤器管理/七层规则管理/七层过滤器/增加七层过滤器（ADD L7FILTER）_82837397.md) | 七层过滤器名称（L7FILTERNAME） | l7-test | 本端规划 | 配置7层过滤条件。 |
| [**ADD L7FILTER**](../../../../../OM参考/命令/UDG MML命令/用户面服务管理/业务匹配策略/业务过滤器管理/七层规则管理/七层过滤器/增加七层过滤器（ADD L7FILTER）_82837397.md) | 子七层过滤器名称（SUBL7FLTNAME） | sl7-test | 本端规划 | 配置7层过滤条件。 |
| [**ADD L7FILTER**](../../../../../OM参考/命令/UDG MML命令/用户面服务管理/业务匹配策略/业务过滤器管理/七层规则管理/七层过滤器/增加七层过滤器（ADD L7FILTER）_82837397.md) | URL（URL） | www.huawei.com | 本端规划 | 配置7层过滤条件。 |
...
| [**ADD PROTBINDFLOWF**](../../../../../OM参考/命令/UDG MML命令/用户面服务管理/业务匹配策略/业务过滤器管理/流过滤器管理/流过滤器的协议绑定/增加流过滤器协议绑定关系（ADD PROTBINDFLOWF）_82837370.md) | 协议名称（PROTOCOLNAME） | https | 本端规划 | 绑定的七层协议。 |
...
6. 配置内容计费规则绑定用户模板。
  ...
  > **说明**
  > 如果进行HTTPS Host识别功能，需解析SNI或者证书字段以获取Host，再基于此Host进行计费和业务控制。

## [任务示例](#ZH-CN_OPI_0124082592)

任务描述

开启 UDG 的HTTP2.0 Host识别功能， UDG 将访问https业务且URL为www.huawei.com的用户业务报文记入使用量上报规则名称urr_red，进行离线计费。
...
```
ADD URR:URRNAME="urr_red",URRID=12000,USAGERPTMODE=OFFLINE;
...
```
ADD L7FILTER:L7FILTERNAME="l7-test",SUBL7FLTNAME="sl7-test",URL="www.huawei.com";
ADD FLOWFILTER:FLOWFILTERNAME="fg_red";
ADD PROTBINDFLOWF:FLOWFILTERNAME="fg_red",PROTOCOLNAME="https",L7FILTERNAME="l7-test";
```
