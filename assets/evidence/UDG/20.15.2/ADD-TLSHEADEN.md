...
- [命令功能](#ZH-CN_CONCEPT_0182837521__1.3.1.1)
- [注意事项](#ZH-CN_CONCEPT_0182837521__1.3.2.1)
- [操作用户权限](#ZH-CN_CONCEPT_0182837521__1.3.3.1)
- [参数说明](#ZH-CN_CONCEPT_0182837521__1.3.4.1)
- [使用实例](#ZH-CN_CONCEPT_0182837521__1.3.5.1)

#### [命令功能](#ZH-CN_CONCEPT_0182837521)

**适用NF：PGW-U、UPF**

该命令用于添加HTTPS头增强的相关配置。配置头增强功能，可以将用户相关信息插入协议头部，传递给Web服务器，Web服务器获取到这些信息以后可以用于结算、折扣、提示等，便于灵活开展业务。

#### [注意事项](#ZH-CN_CONCEPT_0182837521)

- 该命令执行后立即生效。
- 整机最多支持100个不同名称的HTTPS头增强的实例配置。
- 整机最多支持500个TLSHeadEn前缀字段的命令配置。
- 整机最多支持对32个不同tls-type（TLSDynHdr）的HTTPS头增强插入项启动防欺诈检测。
- 单个报文最多支持16个不同前缀字段名称的头增强插入防欺诈检测。
- 新增一个tls-type（TLSDynHdr）用于防欺诈检测时，该插入项的防欺诈检测功能将在配置之后30秒内生效。
- 如果使用HTTPS头增强功能，所有报文都需要进行SA处理，对系统进行SA处理的性能造成影响。
- HTTPS头增强防欺诈检测开启后，系统会增加报文内容的检测和修正处理，因此会增加系统处理开销，需要根据现网实际使用情况决定是否启动HTTPS头增强插入字段的防欺诈检测。
- 当需要配置密钥时，需要同时输入确认密钥，两次输入一致才能配置成功。
- 同一头增强策略下最大支持40个不同datatype。
...
#### [操作用户权限](#ZH-CN_CONCEPT_0182837521)

G_1，管理员级别命令组；G_2，操作员级别命令组

#### [参数说明](#ZH-CN_CONCEPT_0182837521)
...
| REMOVEDMCC | 需去除的移动国家码 | 可选必选说明：条件可选参数<br>前提条件：该参数在“DATATYPE”配置为“MSISDN1”、“MSISDN2” 或 “MSISDN3”时为可选参数。<br>参数含义：该参数用于指定头增强动作插入MSISDN字段需要去除的国家码信息。<br>数据来源：本端规划<br>取值范围：字符串类型，1~4位数字。<br>默认值：无<br>配置原则：该字符串为空时，代表无需去除用户的MSISDN信息中国家码。 |
...
| SALTSW | 是否添加盐值 | 可选必选说明：条件可选参数<br>前提条件：该参数在“ENCRYALGORI”配置为“MD5” 或 “SHA256”时为可选参数。<br>参数含义：该参数用于配置是否添加盐值。<br>数据来源：本端规划<br>取值范围：枚举类型。<br>- ENABLE：使能。<br>- DISABLE：不使能，有安全风险，不建议使用。<br>默认值：ENABLE<br>配置原则：为了保证更好的安全性，建议配置为ENABLE。 |
...
| CIPHERTXTENCODE | 头增强密文编码方式 | 可选必选说明：条件可选参数<br>前提条件：该参数在“ENCRYALGORI”配置为“AES128”、“AES256”、“MD5”、“SHA256”、“RC4”、“AES128_CTR”、“AES128_GCM”、“AES256_CTR” 或 “AES256_GCM”时为可选参数。<br>参数含义：该参数用于设置头增强密文的编码方式。<br>数据来源：本端规划<br>取值范围：枚举类型。<br>- INHERIT：使用BASE64和BIT475控制是否对头增强密文做特殊编码。<br>- NONE：不对头增强密文做特殊编码。<br>- BASE64：对头增强密文做BASE64编码。<br>- ASCII：对头增强密文做ASCII编码。<br>默认值：INHERIT<br>配置原则：<br>- 如果运营商想要对头增强加密结果进行BASE64编码，则配置为BASE64。<br>- 如果运营商想对头增强加密结果进行ASCII编码，则配置为ASCII。 |
...
#### [使用实例](#ZH-CN_CONCEPT_0182837521)

假如运营商希望头增强支持插入MSISDN，type为10000，防欺诈功能不开启，选择AES128 GCM加密方式，密钥为XXXXXX：
