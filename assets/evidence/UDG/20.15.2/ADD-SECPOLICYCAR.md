# 增加安全防攻击CAR规则（ADD SECPOLICYCAR）

- [命令功能](#ZH-CN_CONCEPT_0000001600601201__1.3.1.1)
- [注意事项](#ZH-CN_CONCEPT_0000001600601201__1.3.2.1)
- [操作用户权限](#ZH-CN_CONCEPT_0000001600601201__1.3.3.1)
- [参数说明](#ZH-CN_CONCEPT_0000001600601201__1.3.4.1)
- [使用实例](#ZH-CN_CONCEPT_0000001600601201__1.3.5.1)
- [参考信息](#ZH-CN_CONCEPT_0000001600601201__1.3.6.1)

#### [命令功能](#ZH-CN_CONCEPT_0000001600601201)

该命令用来创建报文CAR动作规则。

#### [注意事项](#ZH-CN_CONCEPT_0000001600601201)

- 该命令执行后立即生效。
- 该命令最大记录数为15360。
- 该命令仅适用于非NP卡场景和NP卡非加速模式场景

#### [操作用户权限](#ZH-CN_CONCEPT_0000001600601201)

G_1，管理员级别命令组；G_2，操作员级别命令组

#### [参数说明](#ZH-CN_CONCEPT_0000001600601201)
...
| SECPOLICYCIR | 安全策略承诺信息速率 | 可选必选说明：可选参数<br>参数含义：策略承诺信息速率。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围为0～1000000。<br>默认值：无<br>配置原则：默认值依赖于SECPOLICYTYPEID字段的配置。如果不设置该参数，则CIR默认值由选择的安全策略编号确定。 |
| SECPOLICYCBS4SH | 安全策略承诺突发尺寸 | 可选必选说明：可选参数<br>参数含义：策略承诺突发尺寸。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围为0～9000000。<br>默认值：无<br>配置原则：默认值依赖于SECPOLICYTYPEID字段的配置。如果不设置该参数，则CBS默认值由选择的安全策略编号确定。 |
| SECMINPKTLEN | 安全最小包补偿长度 | 可选必选说明：可选参数<br>参数含义：最小包补偿长度。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围为64～9600。<br>默认值：128 |
...
#### [使用实例](#ZH-CN_CONCEPT_0000001600601201)

增加一个安全CAR规则：

```
ADD SECPOLICYCAR: SECPOLICYID=1, SECPOLICYTYPE=Tcpip, SECPOLICYCIR=2333, SUBTCPIPTYPE=FRAGMENT;
```

#### [参考信息](#ZH-CN_CONCEPT_0000001600601201)
