# 增加APN的终端接入eDRX模式属性（ADD APNEDRXATTR）

- [命令功能](#ZH-CN_MMLREF_0000001532381428__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001532381428__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001532381428__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000001532381428__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000001532381428)

**适用NF：SMF**

该命令用于增加指定APN的终端接入的eDRX模式。

## [注意事项](#ZH-CN_MMLREF_0000001532381428)
...
- 当APN对应的APNEDRXATTR命令记录不存在时，EDRXSW、PKTCNTPRIORITY、DLBUFFPKTCNT和EXTDLBUFFTIME参数的默认值为SET GLBEDRXATTR命令中同名参数的值。

- 最多可输入20000条记录。

#### [操作用户权限](#ZH-CN_MMLREF_0000001532381428)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0000001532381428)
...
| EDRXSW | 支持eDRX模式开关 | 可选必选说明：可选参数<br>参数含义：该参数用于指定是否使能NR和NR RedCap终端接入时的eDRX模式。<br>数据来源：本端规划<br>取值范围：<br>- “INHERIT（继承）”：继承SET GLBEDRXATTR命令。<br>- “DISABLE（不使能）”：关闭终端接入的eDRX模式。<br>- “ENABLE（使能）”：打开终端接入的eDRX模式。<br>默认值：INHERIT<br>配置原则：<br>如果该参数的值不为“INHERIT”，且该参数的值与SET GLBEDRXATTR命令中的EDRXSW参数不同时，以该参数的值为准。 |
...
## [使用实例](#ZH-CN_MMLREF_0000001532381428)

假如用户需要增加对APN为“apn1”的终端接入的eDRX模式，打开eDRX模式，下行包缓存数获取优先级为从DLBUFFPKTCNT参数获取，给UPF携带下行包缓存数为100，额外下行包缓存时间为10秒时，则使用该实例：

```
ADD APNEDRXATTR: APN="apn1", EDRXSW=ENABLE, PKTCNTPRIORITY=DLBUFFPKTCNT, DLBUFFPKTCNT=100, EXTDLBUFFTIME=10;
```
