# 增加5G语音部署配置（ADD NGVOICEDEPLOY）

- [命令功能](#ZH-CN_MMLREF_0000001186340330__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001186340330__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001186340330__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000001186340330__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000001186340330)

**适用NF：AMF**

该命令用于增加UE使用5G网络接入时的IMS VoPS语音部署策略。

## [注意事项](#ZH-CN_MMLREF_0000001186340330)

- 该命令执行后立即生效。

- 最多可输入1024条记录。

#### [操作用户权限](#ZH-CN_MMLREF_0000001186340330)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0000001186340330)
...
| AMFHOMO | AMF是否支持IMS语音 | 可选必选说明：可选参数<br>参数含义：该参数用于指定AMF侧的所有跟踪区是否都支持PS域IMS语音能力。<br>数据来源：全网规划<br>取值范围：<br>- “NOT_SUPPORT（不支持）”：不支持<br>- “SUPPORT（支持）”：支持<br>- “UNSPECIFIED（未指定）”：表示接入网络的PS域IMS语音能力未知，AMF需要向NG-RAN侧发UE RADIO CAPABILITY CHECK REQUEST消息获取UE的IMS VoPS支持能力。最终根据UE能力和“DCVOPS（Data Centric类型终端支持VoPS）”参数的配置来决策PS域IMS语音能力。<br>默认值：SUPPORT<br>配置原则：无 |
| DCVOPS | Data Centric类型终端支持VoPS | 可选必选说明：可选参数<br>参数含义：该参数用于指定Data Centric类型终端是否都支持IMS语音能力。<br>数据来源：全网规划<br>取值范围：<br>- “NOT_SUPPORT（不支持）”：不支持<br>- “SUPPORT（支持）”：支持<br>默认值：SUPPORT<br>配置原则：<br>当运营商不允许Data Centric终端使用5G语音功能时，设置为NOT_SUPPORT; 当运营商允许Data Centric终端使用5G语音功能时，设置为SUPPORT。 |
...
## [使用实例](#ZH-CN_MMLREF_0000001186340330)

运营商A允许本网用户支持IMS语音业务，Data Centric类型终端支使用IMS VoPS，只有用户签约携带了语音DNN为“IMS”，并且检查用户和IMS DNN都支持45G互操作，才可以使用IMS语音服务，执行如下命令：

```
ADD NGVOICEDEPLOY: SUBRANGE=HOME_USER, AMFHOMO=SUPPORT, DCVOPS=SUPPORT, UES1MODE=ON, VOICEDNN="IMS", IWKEPSIND=ON;
```
