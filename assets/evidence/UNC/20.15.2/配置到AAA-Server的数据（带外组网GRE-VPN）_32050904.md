# 配置到AAA Server的数据（带外组网GRE VPN）

- [操作场景](#ZH-CN_OPI_0232050904__1.3.1)
- [必备事项](#ZH-CN_OPI_0232050904__1.3.2)
- [操作步骤](#ZH-CN_OPI_0232050904__1.3.3)
- [验证方法](#ZH-CN_OPI_0232050904__1.3.4)
- [任务示例](#ZH-CN_OPI_0232050904__1.3.5)

## [操作场景](#ZH-CN_OPI_0232050904)

UNC 和AAA Server（计费/鉴权AAA服务器）的互通采用带外组网方式，为RADIUS信令报文和数据报文配置不同的出接口。 UNC 到计费/鉴权AAA服务器采用缺省路由，当有多个计费/鉴权AAA服务器时，可减少路由配置的工作量，并便于管理。为了提高互通的安全性，在 UNC 和企业网所在网段的防火墙之间建立GRE VPN隧道。隧道绑定的VPN和计费/鉴权AAA服务器绑定的VPN不同。

> **说明**
> 适用于 GGSN、 PGW-C、SMF。

## [必备事项](#ZH-CN_OPI_0232050904)
...
| **[ADD APNRDSSVRGRP](../../../../../../OM参考/命令/UNC MML命令/业务服务管理/会话管理/RADIUS管理/连接管理/APN Radius服务器/设置APN Radius服务器组（ADD APNRDSSVRGRP）_09896735.md)** | APN名称（APN） | apn1 | 已配置数据中获取 | 已通过<br>**[ADD LOGICINF](../../../../../../OM参考/命令/UNC MML命令/业务服务管理/接口管理/计费和策略接口管理/逻辑接口/增加逻辑接口（ADD LOGICINF）_09896723.md)**<br>配置，可以使用<br>**[LST LOGICINF](../../../../../../OM参考/命令/UNC MML命令/业务服务管理/接口管理/计费和策略接口管理/逻辑接口/查询逻辑接口（LST LOGICINF）_09896726.md)**<br>命令进行查询。 |
...
## [验证方法](#ZH-CN_OPI_0232050904)

具体步骤请参见 [调测到AAA Server的数据](../调测到AAA Server的数据_32027181.md) 。

## [任务示例](#ZH-CN_OPI_0232050904)

任务描述

本实例中使用的是 UNC 与计费/鉴权AAA服务器通过带外GRE VPN组网方式对接时的配置范例。操作员在 UNC 上进行数据配置实现以下要求：

- UNC使用缺省路由与计费/鉴权AAA服务器建立起正常的IP连接。
- UNC使用缺省路由与PDN之间通过GRE VPN隧道建立起正常的IP连接。
...
```
ADD RDSSVR: RDSSVRGRPNAME="isprg",SERVERTYPE=AUTHENTICATION,IPVERSION=IPV4,SERVERIPV4="10.168.10.1",PORT=1812,VPNINSTANCE="vpn_aaa",CIPHERKEY="*****",CIPHERKEYCNFM="*****";
```

```
ADD RDSSVR: RDSSVRGRPNAME="isprg",SERVERTYPE=ACCOUNTING,IPVERSION=IPV4,SERVERIPV4="10.168.10.1",PORT=1813,VPNINSTANCE="vpn_aaa",CIPHERKEY="*****",CIPHERKEYCNFM="*****";
```
