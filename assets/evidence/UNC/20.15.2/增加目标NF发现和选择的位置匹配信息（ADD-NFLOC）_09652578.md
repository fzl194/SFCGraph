# 增加目标NF发现和选择的位置匹配信息（ADD NFLOC）

- [命令功能](#ZH-CN_MMLREF_0209652578__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0209652578__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0209652578__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0209652578__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0209652578)

![](增加目标NF发现和选择的位置匹配信息（ADD NFLOC）_09652578.assets/notice_3.0-zh-cn_2.png)

执行该命令，如果参数设置不合理，可能影响AMF优选本DC SMF。

**适用NF：AMF、SMF**

该命令用于为目标NF的发现和选择增加位置匹配相关的信息，即在目标NF的发现和选择过程中，可以基于NF部署的位置做精确或者优先匹配。

## [注意事项](#ZH-CN_MMLREF_0209652578)

- 该命令执行后立即生效。

- 最多可输入16条记录。
- SMF仅支持通过该指令实现Preferred-Locality发现目标UDM；暂不支持通过该指令实现Preferred-Locality优选PCF等其它目标NF。
- 对于同一个NF，在发现同一种目标NF的时候，只能使用Locality或Preferred Locality其中一种；在同时配置的情况下，以Locality的优先级为高。
- 对于同一个NF的一种位置类型，如果指定了“NF_ALL”类型的目标NF，则不允许再指定具体的目标NF类型；反之，如果已经指定了具体的目标NF类型，则不允许再配置“NF_ALL”类型的目标NF。

#### [操作用户权限](#ZH-CN_MMLREF_0209652578)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0209652578)
...
## [使用实例](#ZH-CN_MMLREF_0209652578)

由于网络调整，运营商期望数据中心“DC1”内的AMF为新建的PDU Session选择位于“DC2”数据中心内的SMF，执行如下命令：

```
ADD NFLOC: NFTYPE=NfAMF, TGTNFTYPE=SMF, LOCTYPE=PREFERRED_LOCALITY, PREFERLOC="DC2", DESC="prefer the SMFs of DC2";
```
