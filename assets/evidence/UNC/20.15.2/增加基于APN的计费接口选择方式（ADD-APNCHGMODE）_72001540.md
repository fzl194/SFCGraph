# 增加基于APN的计费接口选择方式（ADD APNCHGMODE）

- [命令功能](#ZH-CN_MMLREF_0272001540__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0272001540__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0272001540__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0272001540__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0272001540)
...
#### [操作用户权限](#ZH-CN_MMLREF_0272001540)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0272001540)
...
| BY5GCNRI | 按5GC无限制接入标识选择计费接口 | 可选必选说明：该参数在"TMACCTYPE"配置为"UENON5G_RAT4G"、"UE5G_RAT4G"时为条件可选参数。<br>参数含义：该参数用于配置是否按5GC无限制接入标识的取值来选择计费接口。“5GC无限制接入标识”是指S11、S5/S8、S2b接口的“5GC Not Restricted Indication Selection”参数。<br>数据来源：本端规划<br>取值范围：<br>- False（否）<br>- True（是）<br>默认值：False<br>配置原则：<br>本参数为TRUE时，当对端携带“5GC Not Restricted Indication Selection”参数为1时，选择Nchf接口；当对端携带“5GC Not Restricted Indication Selection”参数为0，或者对端未携带“5GC Not Restricted Indication Selection”参数时，选择GaGy接口。<br>本参数为FALSE且BY5GSIWKI参数为FALSE时，根据FORCED参数选择计费接口，需要配置FORCED参数。 |

## [使用实例](#ZH-CN_MMLREF_0272001540)

添加基于APN的计费接口选择方式，“APN名称”为“HUAWEI.COM”，指定5G终端4G接入的计费接口为Nchf。

```
ADD APNCHGMODE: APN="huawei.com",TMACCTYPE=UE5G_RAT4G, BY5GSIWKI=False, FORCED=NchfMode;
```
