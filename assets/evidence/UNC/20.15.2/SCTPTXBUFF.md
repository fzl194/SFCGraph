# 查询SCTP发送缓冲区(DSP SCTPTXBUFF)

- [命令功能](#ZH-CN_CONCEPT_0000001195432714__1.4.1.1)
- [注意事项](#ZH-CN_CONCEPT_0000001195432714__1.4.2.1)
- [操作用户权限](#ZH-CN_CONCEPT_0000001195432714__1.4.3.1)
- [参数说明](#ZH-CN_CONCEPT_0000001195432714__1.4.4.1)
- [使用实例](#ZH-CN_CONCEPT_0000001195432714__1.4.5.1)
- [输出结果说明](#ZH-CN_CONCEPT_0000001195432714__1.4.6.1)

#### [命令功能](#ZH-CN_CONCEPT_0000001195432714)

**适用NF：SGSN、MME、AMF**

此命令用于查询SCTP发送缓冲区信息。

#### [注意事项](#ZH-CN_CONCEPT_0000001195432714)

此命令执行后立即生效。

#### [操作用户权限](#ZH-CN_CONCEPT_0000001195432714)

G_1，管理员级别命令组；G_2，操作员级别命令组；G_3，用户级别命令组

#### [参数说明](#ZH-CN_CONCEPT_0000001195432714)

| 参数标识 | 参数名称 | 参数说明 |
| --- | --- | --- |
| SRT | 查询方式 | 可选必选说明：可选参数<br>参数含义：该参数用于指定查询方式。<br>取值范围：<br>- PROC（按进程查询）<br>- INTYPE（按接口类型查询）<br>默认值：“PROC（按进程查询）” |
| RUNAME | RU名称 | 可选必选说明：条件可选/必选参数<br>参数含义：该参数用于指定待查询的RU名称。该参数可以通过<br>[DSP RU](../../../../../平台服务管理/单体服务公共功能管理/系统管理/资源管理/RU管理/显示资源单元信息（DSP RU）_59103857.md)<br>命令查询。<br>取值范围：1~63位字符串<br>默认值 ：无<br>配置原则：“查询方式”参数设置为“PROC（按进程查询）”时，该参数为可选参数；“查询方式”参数设置为“INTYPE（按接口类型查询）”时，该参数为必选参数。 |
| PN | 进程号 | 可选必选说明：条件可选/必选参数<br>参数含义：该参数用于指定查询的SGP的进程号。<br>取值范围： 0~11<br>默认值 ：无<br>配置原则：“查询方式”参数设置为“PROC（按进程查询）”时，该参数为可选参数；“查询方式”参数设置为“INTYPE（按接口类型查询）”时，该参数为必选参数。 |
| IFTYPE | 接口类型 | 可选必选说明：条件必选参数<br>参数含义：该参数用于指定查询的接口类型。<br>前提条件：“查询方式”参数设置为“INTYPE（按接口类型查询）”时，该参数必选。<br>取值范围：<br>- S1（S1）<br>- N2（N2）<br>默认值 ：S1<br>配置原则：无 |

#### [使用实例](#ZH-CN_CONCEPT_0000001195432714)

查询N2接口SCTP发送缓冲区，可以用如下命令：

DSP SCTPTXBUFF: SRT=INTYPE, RUNAME="LINK_SP_RU_0064", PN=0, IFTYPE=N2;

```
%%DSP SCTPTXBUFF: SRT=INTYPE, RUNAME="LINK_SP_RU_0064", PN=0, IFTYPE=N2;%%
RETCODE = 0  操作成功

操作结果如下
------------
偶联ID  对端IP地址1   对端端口号  私有发送缓冲区总大小  私有发送缓冲区占用大小  扩展发送缓冲区总大小  扩展发送缓冲区占用大小  发送缓冲区占用率  

198     192.168.0.10  16514       20000                 116                     0                     0                       1%                
200     192.168.1.11  16926       20000                 116                     0                     0                       1%                
(结果个数 = 2)

---    END 
```

查询所有SGP进程上的SCTP发送缓冲区，可以用如下命令：

DSP SCTPTXBUFF: SRT=PROC;

```
%%DSP SCTPTXBUFF: SRT=PROC;%%
RETCODE = 0  操作成功

操作结果如下
------------
RU名称           进程号  S1共享发送缓冲区占用块  N2共享发送缓冲区占用块  迷你端共享区总块  迷你端共享区占用率  

LINK_SP_RU_0064  0       0                       0                       30000000          0%                  
LINK_SP_RU_0065  1       0                       0                       30000000          0%                  
LINK_SP_RU_0064  1       0                       0                       30000000          0%                  
LINK_SP_RU_0065  0       0                       0                       30000000          0%                  
(结果个数 = 4)

---    END 
```

#### [输出结果说明](#ZH-CN_CONCEPT_0000001195432714)

| 输出项名称 | 输出项解释 |
| --- | --- |
| RU名称 | 查询出的SPU资源单元名。<br>取值范围：1~63位字符串 |
| 进程号 | 查询出的发送缓冲区所在的SGP进程的进程号。 |
| S1共享发送缓冲区占用块 | 查询出的SGP进程上的S1共享发送缓冲区占用块。 |
| N2共享发送缓冲区占用块 | 查询出的SGP进程上的N2共享发送缓冲区占用块。 |
| 迷你端共享区总块 | 查询出的SGP进程上的迷你端共享区总块。 |
| 迷你端共享区占用率 | 查询出的SGP进程上的迷你端共享区占用率。 |
| 偶联ID | 查询出的SCTP链路的偶联ID。 |
| 对端IP地址1 | 查询出的SCTP链路中gNodeB/eNodeB的IP地址1。 |
| 对端IP地址2 | 查询出的SCTP链路中gNodeB/eNodeB的IP地址2。 |
| 对端端口号 | 查询出的SCTP链路中gNodeB/eNodeB的端口号。 |
| 私有发送缓冲区总大小 | 查询出的SCTP链路的私有发送缓冲区总大小。 |
| 私有发送缓冲区占用大小 | 查询出的SCTP链路的私有发送缓冲区占用大小。 |
| 扩展发送缓冲区总大小 | 查询出的SCTP链路的扩展发送缓冲区总大小。 |
| 扩展发送缓冲区占用大小 | 查询出的SCTP链路的扩展发送缓冲区占用大小。 |
| 发送缓冲区占用率 | 查询出的SCTP链路的发送缓冲区占用率。 |
