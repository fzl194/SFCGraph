# 导出节点信息（EXP NODEINFO）

- [命令功能](#ZH-CN_MMLREF_0294730443__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0294730443__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0294730443__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0294730443__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0294730443)

该命令根据指定网元ID和类型导出相应的节点信息。当命令执行成功后可通过DSP NODEINFO来显示导出的节点信息。

## [注意事项](#ZH-CN_MMLREF_0294730443)

1.该命令执行后立即生效。 2.该命令仅在虚机场景下支持

#### [操作用户权限](#ZH-CN_MMLREF_0294730443)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0294730443)

| 参数标识 | 参数名称 | 参数说明 |
| --- | --- | --- |
| MEID | 网元ID | 可选必选说明：可选参数<br>参数含义：该参数用于指定网元来查询节点信息；若不输入，则表示当前控制节点所在网元；网元ID可以通过LST ME命令查询出来。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围是0~65535。<br>默认值：无<br>配置原则：无 |
| TYPE | 类型 | 可选必选说明：必选参数<br>参数含义：该参数用于指定需要查询信息的类型。<br>数据来源：本端规划<br>取值范围：<br>- VF_DRIVER（VF驱动）<br>默认值：无<br>配置原则：无 |

## [使用实例](#ZH-CN_MMLREF_0294730443)

查询网元的VF驱动版本。

```
%%EXP NODEINFO: MEID=0, TYPE=VF_DRIVER;%%
RETCODE = 0  操作成功

进度报文
--------
上报类型  =  导出节点信息
状态  =  进行中    
进度  =  40%    
会话号  =  172.19.1.3_575  
(结果个数 = 1)

---    END

%%EXP NODEINFO: MEID=0, TYPE=VF_DRIVER;%%
RETCODE = 0  操作成功

进度报文
--------
上报类型  =  导出节点信息
状态  =  成功    
会话号  =  172.19.1.3_575  
(结果个数 = 1)

共有2个报告
---    END
```
