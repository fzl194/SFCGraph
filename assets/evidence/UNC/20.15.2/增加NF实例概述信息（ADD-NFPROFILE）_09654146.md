# 增加NF实例概述信息（ADD NFPROFILE）

- [命令功能](#ZH-CN_MMLREF_0209654146__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0209654146__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0209654146__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0209654146__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0209654146)
...
## [注意事项](#ZH-CN_MMLREF_0209654146)

- 该命令执行后立即生效。

- 最多可输入16条记录。

#### [操作用户权限](#ZH-CN_MMLREF_0209654146)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0209654146)
...
| CAPACITY | 容量 | 可选必选说明：可选参数<br>参数含义：本参数用于指定NF的相对权重（与其他同类型NF实例相比）。特别地，如果NF容量的绝对值不超过本参数的取值范围，那么本参数可以直接取用容量绝对值。例如AMF可接入的用户数是50000，那么该AMF的容量就可以用50000表示。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围是0~65535。值越大表示容量越大。<br>默认值：100<br>配置原则：无 |
...
## [使用实例](#ZH-CN_MMLREF_0209654146)

新增1个AMF的NF实例，将其初始状态置为注册态，该NF实例除了要满足本网内的业务外，还要考虑外网（跨PLMN）NF对该NF实例的访问。该NF支持SMF、AUSF、UDM对其进行访问，且将该NF实例优先级设置为1，容量设置为100，脚本如下：

```
ADD NFPROFILE: NFINSTANCENAME="AMF_Instance_0", NFTYPE=NfAMF, NFSTATUS=Registered, FQDN="amf1.cluster1.net2.amf.5gc.mnc012.mcc345.3gppnetwork.org", INTERPLMNFQDN="amf1.cluster1.net2.amf.5gc.mnc012.mcc345.3gppnetwork.org", CAPACITY=100, PRIORITY=1, ALLOWEDNFTYPES=AllowNfAUSF-1&AllowNfSMF-1&AllowNfUDM-1;
```
