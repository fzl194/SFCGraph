# 修改5GC QoS到TOS/DSCP的映射规则（MOD 5GCREMARK）

- [命令功能](#ZH-CN_MMLREF_0209652540__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0209652540__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0209652540__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0209652540__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0209652540)

**适用NF：SMF**

该命令用来修改5G用户QoS参数到IP报头中的DSCP（区别服务编码点）/TOS（服务类型）的映射规则，用户的数据将根据映射得到的DSCP（区别服务编码点）/TOS（服务类型）中的参数值进行转发。

## [注意事项](#ZH-CN_MMLREF_0209652540)

命令执行后只对新接入用户生效。

#### [操作用户权限](#ZH-CN_MMLREF_0209652540)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0209652540)
...
| REMARKTYPE | 标记类型 | 可选必选说明：可选参数<br>参数含义：该参数用于表示映射到DSCP或者TOS。<br>数据来源：全网规划<br>取值范围：<br>- DSCP（映射到Dscp）<br>- TOS（映射到TOS）<br>默认值：无<br>配置原则：无 |
...
## [使用实例](#ZH-CN_MMLREF_0209652540)

修改QoS Profile名称为“profile”，5QI值为5，ARP的优先级别为5的5GC Remark配置，修改标记类型类型为“DSCP”，参数DSCP为“BE”：

```
MOD 5GCREMARK:QOSPROFILENAME="profile",FIVEQI=5,ARPPL=5,REMARKTYPE=DSCP,DSCP=BE; 
```
