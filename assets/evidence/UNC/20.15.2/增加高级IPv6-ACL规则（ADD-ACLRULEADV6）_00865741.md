# 增加高级IPv6 ACL规则（ADD ACLRULEADV6）

- [命令功能](#ZH-CN_CONCEPT_0000001600865741__1.3.1.1)
- [注意事项](#ZH-CN_CONCEPT_0000001600865741__1.3.2.1)
- [操作用户权限](#ZH-CN_CONCEPT_0000001600865741__1.3.3.1)
- [参数说明](#ZH-CN_CONCEPT_0000001600865741__1.3.4.1)
- [使用实例](#ZH-CN_CONCEPT_0000001600865741__1.3.5.1)

#### [命令功能](#ZH-CN_CONCEPT_0000001600865741)

该命令用于创建高级IPv6 ACL规则。高级IPv6 ACL可以基于五元组（源目的IPv6，源目的端口号，协议号）进行报文匹配。主要应用场景为路由策略以及端口流量策略等。

#### [注意事项](#ZH-CN_CONCEPT_0000001600865741)
...
#### [操作用户权限](#ZH-CN_CONCEPT_0000001600865741)

G_1，管理员级别命令组；G_2，操作员级别命令组

#### [参数说明](#ZH-CN_CONCEPT_0000001600865741)
...
| ACLPROTOCOLTYPE | 协议选项类型 | 可选必选说明：可选参数<br>参数含义：该参数用于指定IPv6 ACL规则协议选项的类型。<br>数据来源：本端规划<br>取值范围：枚举类型。<br>- Number：数字。<br>- WellKnow：知名协议号。<br>默认值：Number<br>配置原则：根据需要设置的IPv6报文协议选项类型设置该参数。 |
| ACLPROTOCOL | 协议类型值 | 可选必选说明：条件必选参数<br>前提条件：该参数在“ACLPROTOCOLTYPE”配置为“Number”时为必选参数。<br>参数含义：该参数用于指定IPv6 ACL规则协议类型。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围为0～255。知名协议号IPv6（0）：此规则应用于所有IPv6报文。 TCP（6）：此规则应用于所有TCP报文。 UDP（17）：此规则应用于所有UDP报文。 ICMPv6（58）：此规则应用于所有ICMPv6报文。<br>默认值：无<br>配置原则：根据需要匹配的IPV6报文协议类型设置该参数。 |
...
#### [使用实例](#ZH-CN_CONCEPT_0000001600865741)

用户使用流分类，需要过滤源IP地址为2001:db8::1:1，目的地址为2001:db8::1:2的报文，可以配置高级IPv6 ACL规则：

```
ADD ACLRULEADV6:ACLNAME="3000",ACLRULENAME="rule_5",ACLACTION=Deny,ACLPROTOCOLTYPE=Number,ACLPROTOCOL=0,ACLSOURCEIP="2001:db8::1:1",ACLSRCWILD=20,ACLDESTIP="2001:db8::1:2",ACLDESTWILD=20;
```
