...
> ⚠ 源文档勘误：步5「配置将某类业务/应用的流量不统计到会话级流量」操作步骤标题与数据规划表均用 **MOD PCCPOLICYGRP**（PCCPOLICYGRPNM 标"已配置数据中获取"，暗示 PCCPOLICYGRP 已存在），但任务示例脚本同一步骤写成 **ADD PCCPOLICYGRP**（`ADD PCCPOLICYGRP:PCCPOLICYGRPNM="pccpolicygrp_01", FUPSESSIONEXC=ENABLE;`）。ADD=新建 / MOD=修改已存在，二者语义矛盾。处置：FUP 作 PCC 增量时 PCCPOLICYGRP 已在 PCC 基本功能步骤 8 建过→走 MOD 修改增量参数 FUPSESSIONEXC；首次部署 PCCPOLICYGRP 未建→走 ADD 新建并带 FUPSESSIONEXC。两命令 wiki 均有 FUPSESSIONEXC 参数。feature wiki [2-00007](task/UNC/20.15.2/2-00007.md) 已脚注处置此差异。
