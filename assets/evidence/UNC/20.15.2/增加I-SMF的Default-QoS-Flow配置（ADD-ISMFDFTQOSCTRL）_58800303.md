# 增加I-SMF的Default QoS Flow配置（ADD ISMFDFTQOSCTRL）

- [命令功能](#ZH-CN_MMLREF_0000001258800303__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001258800303__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001258800303__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000001258800303__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000001258800303)
...
## [注意事项](#ZH-CN_MMLREF_0000001258800303)

- 命令执行后只对新接入用户生效。

- 如果需要绑定"ALLOWED5QI"或"ALLOWEDARPS"参数，请确保系统中已存在这两条参数的配置记录，否则不能绑定。
- 最多可输入2048条记录。
- 如需配置MFBR及GFBR，需要保证MFBR大于等于GFBR，否则不能添加。

#### [操作用户权限](#ZH-CN_MMLREF_0000001258800303)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0000001258800303)
...
| AMBRUL | 上行Session AMBR(千比特/秒) | 可选必选说明：可选参数<br>参数含义：该参数用于指定组成QoS控制的上行Session AMBR。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围是1~65000000。本参数默认配置为10000000kbps。<br>默认值：无<br>配置原则：无 |
| AMBRDL | 下行Session AMBR(千比特/秒) | 可选必选说明：可选参数<br>参数含义：该参数用于指定组成QoS控制的下行Session AMBR。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围是1~65000000。本参数默认配置为10000000kbps。<br>默认值：无<br>配置原则：无 |
...
| MFBRDL | 下行最大速率 (千比特/秒) | 可选必选说明：可选参数<br>参数含义：该参数用于指定组成QoS控制的下行最大速率 (千比特/秒)。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围是1~65000000。本参数不配置时则为无效值0，表示不进行限速或管控。<br>默认值：无<br>配置原则：无 |
...
## [使用实例](#ZH-CN_MMLREF_0000001258800303)

在A和B两个运营商共建网络场景下，B运营商只负责建H-SMF，这样如果用户移动离开该H-SMF的覆盖范围，可能存在需要借用A运营商I-SMF的情况。A运营商出于最大化保障本网用户对资源的优先使用权，可以在本网I-SMF上设置针对外网用户的QoS使用限制，来达成上述目的。 例如A运营商决定限制B运营商5QI为1的用户接入，同时限制B运营商用户缺省承载的AMBR不超过1000千比特/秒，对于AMBR超过1000千比特/秒的用户，按I-SMF本地配置的值修正B运营商用户的QoS，配置命令如下： 1. 添加SMF允许的5QI列表： 2. 添加SMF允许的ARP列表： 3. 添加I-SMF的Default QoS Flow参数：
...
ADD ISMFDFTQOSCTRL:MCC="460",MNC="00",CTRLTYPE=GLOBAL_LEVEL,AMBRUL=1000,AMBRDL=1000,QOSACTION=DEGRADE,ALLOWED5QIS=1,BINDALLOWED5QIS=ENABLE,ALLOWEDARPS=65535,BINDALLOWEDARPS=DISABLE;
```
