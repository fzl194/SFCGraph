# 增加UPF逻辑接口（ADD UPLOGICINTF）

- [命令功能](#ZH-CN_MMLREF_0296805379__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0296805379__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0296805379__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0296805379__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0296805379)

**适用NF：SMF、PGW-C、SGW-C、GGSN**

该命令用于增加UPF逻辑接口。当UPF不上报U面逻辑接口信息时，SMF可以用配置的逻辑接口信息筛选UPF。

## [注意事项](#ZH-CN_MMLREF_0296805379)

- 该命令执行后立即生效。

- 同一UPF的同一逻辑接口类型下NETWORKINSTNAME只能配置一个。

- 最多可输入20480条记录。

#### [操作用户权限](#ZH-CN_MMLREF_0296805379)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0296805379)
...
| IPVERSION | IP地址类型 | 可选必选说明：必选参数<br>参数含义：该参数用于标识逻辑接口支持的IP地址类型。<br>数据来源：全网规划<br>取值范围：<br>- IPV4V6（IPv4v6）<br>- IPV4（IPv4）<br>- IPV6（IPv6）<br>默认值：无<br>配置原则：无 |

## [使用实例](#ZH-CN_MMLREF_0296805379)

增加UP逻辑接口，UPF实例标识为upf_instance_1，逻辑接口类型为N3，网络实例名称为huawei.com，IP地址类型为IPv4。

```
ADD UPLOGICINTF: UPFINSTANCEID="upf_instance_1", ITFTYPE=N3, NETWORKINSTNAME="huawei.com", IPVERSION=IPV4;
```
