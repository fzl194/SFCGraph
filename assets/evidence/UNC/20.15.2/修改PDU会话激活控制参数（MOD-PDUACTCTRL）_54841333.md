# 修改PDU会话激活控制参数（MOD PDUACTCTRL）

- [命令功能](#ZH-CN_MMLREF_0000002154841333__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000002154841333__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000002154841333__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000002154841333__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000002154841333)

**适用NF：AMF**

该命令用于修改基于DNN配置PDU会话激活处理策略。

## [注意事项](#ZH-CN_MMLREF_0000002154841333)

该命令执行后立即生效。

#### [操作用户权限](#ZH-CN_MMLREF_0000002154841333)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0000002154841333)
...
| POLICY | 处理策略 | 可选必选说明：可选参数<br>参数含义：该参数用于控制整体策略，是直接拒绝激活，还是在签约数据匹配失败后进行纠正处理。<br>数据来源：全网规划<br>取值范围：<br>- REJECT（直接拒绝）<br>- MATCH_FAIL_NOTCORRECT（签约数据匹配失败后不执行纠正策略）<br>- MATCH_FAIL_CORRECT（签约数据匹配失败后执行纠正策略）<br>默认值：无<br>配置原则：无 |
...
## [使用实例](#ZH-CN_MMLREF_0000002154841333)

修改使用DNN为“huawei.com”，IMSI为“123456789012345”的用户PDU会话激活流程中签约数据DNN匹配失败后拒绝接入网络，执行如下命令：

```
MOD PDUACTCTRL:SUBRANGE=SPECIFIC_IMSI,IMSI="123456789012345",DNNTYPE=SPECIAL_DNN,DNN="huawei.com",POLICY=MATCH_FAIL_NOTCORRECT;
```
