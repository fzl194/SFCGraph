# 增加S-GW选择策略(ADD SGWSELPLCY)

- [命令功能](#ZH-CN_MMLREF_0000001126145974__1.3.1.1)
- [注意事项](#ZH-CN_MMLREF_0000001126145974__1.3.2.1)
- [本地用户权限](#ZH-CN_MMLREF_0000001126145974__1.3.3.1)
- [网管用户权限](#ZH-CN_MMLREF_0000001126145974__1.3.4.1)
- [参数说明](#ZH-CN_MMLREF_0000001126145974__1.3.5.1)
- [使用实例](#ZH-CN_MMLREF_0000001126145974__1.3.6.1)

#### [命令功能](#ZH-CN_MMLREF_0000001126145974)
...
#### [注意事项](#ZH-CN_MMLREF_0000001126145974)
...
- 该表最大记录数为128。
- 本命令启用后，“ADD SGWDNS”以及“ADD AREADNS”命令将不对本命令控制的用户群生效。
- “用户范围”（包含指定IMSI/MSISDN前缀）、“IMSI前缀”或者“MSISDN前缀”、“选择方法”的组合唯一确认一种S-GW选择策略。若上述组合均相同，指定的S-GW查询域名不能重复。
- 本命令中“用户范围”为“MSISDN_PREFIX(指定MSISDN前缀)”的记录优先级高于“用户范围”为“IMSI_PREFIX(指定IMSI前缀)”的记录，例如用户123030000000001，使用MSISDN为30808，系统中配置如[表1](#ZH-CN_MMLREF_0000001126145974__tab1)所示，优先匹配“MSISDN前缀”为30808的记录2。
  *表1 示例*
...
- 当“用户范围”设置为“IMSI_PREFIX（指定IMSI前缀）”时，如果IMSI前缀的组合同时匹配到多条记录，“IMSI前缀”长度不相等时，使用“IMSI前缀”最长匹配的记录。例如用户123030000000001，在[表2](#ZH-CN_MMLREF_0000001126145974__tab2)所示场景下，匹配到记录2。
  *表2 示例*
...
- 当“用户范围”设置为“MSISDN_PREFIX(指定MSISDN前缀)”时，如果MSISDN前缀的组合同时匹配到多条记录，“MSISDN前缀”长度不相等时，使用“MSISDN前缀”最长匹配的记录。例如用户MSISDN为3080888，在[表3](#ZH-CN_MMLREF_0000001126145974__tab3)所示场景下，匹配到记录2。
  *表3 示例1*
...
#### [本地用户权限](#ZH-CN_MMLREF_0000001126145974)

manage-ug；system-ug

#### [网管用户权限](#ZH-CN_MMLREF_0000001126145974)

G_1，管理员级别命令组；G_2，操作员级别命令组

#### [参数说明](#ZH-CN_MMLREF_0000001126145974)

| 参数标识 | 参数名称 | 参数说明 |
| --- | --- | --- |
| SUBRANGE | 用户范围 | 可选必选说明：必选参数<br>参数含义：该参数用于指定用户范围。<br>数据来源：整网规划<br>取值范围：<br>- “IMSI_PREFIX(指定IMSI前缀)”<br>- “MSISDN_PREFIX(指定MSISDN前缀)”<br>- “SPECIFIC_IMSI(特定IMSI)”<br>- “SPECIFIC_MSISDN(特定MSISDN)”<br>默认值：无<br>配置原则：<br>S-GW选择策略优先级从高到低为：“SPECIFIC_MSISDN (特定MSISDN)”，“SPECIFIC_IMSI (特定IMSI)”， “MSISDN_PREFIX(指定MSISDN前缀)”，“IMSI_PREFIX(指定IMSI前缀)” |
...
| SLCTM | 选择方法 | 可选必选说明：必选参数<br>参数含义：该参数用于指定选择方法。<br>数据来源：整网规划<br>取值范围：<br>- “S-GW_QNAME(指定S-GW的查询域名)”<br>- “S-GW_IP (指定S-GW的IP地址)”<br>默认值：无<br>配置原则：<br>S-GW选择策略优先级从高到低为：“S-GW_IP (指定S-GW的IP地址)”，“S-GW_QNAME(指定S-GW的查询域名)”<br>说明：当本参数取值配置为“S-GW_IP (指定S-GW的IP地址)”时，在异系统切换或重选场景下，该功能受软参"BYTE_EX_B76" BIT6控制，具体请参考该软参含义。 |
...
#### [使用实例](#ZH-CN_MMLREF_0000001126145974)

拨测MSISDN为13515000001的用户选择IP地址为"192.168.183.1"的S-GW，以 UNC 本地A记录解析为例，进行如下配置:

1. 配置MSISDN=“13515000001”指定S-GW的查询域名ADD SGWSELPLCY: SUBRANGE=MSISDN_PREFIX, MSISDNPRE="13515000001", SLCTM=S-GW_QNAME, QNAME="TOPON.ETH1.RTYBG.EPC.MNC003.MCC460.3GPPNETWORK.ORG";
2. 配置域名“TOPON.ETH1.RTYBG.EPC.MNC003.MCC460.3GPPNETWORK.ORG”的查询方式为A查询ADD DNSQ: SUBRANGE=All, DNSUF="TOPON.ETH1.RTYBG.EPC.MNC003.MCC460.3GPPNETWORK.ORG", DNSQUERYMODE=AAAA/A, GROUPID=0;
3. 配置域名“TOPON.ETH1.RTYBG.EPC.MNC003.MCC460.3GPPNETWORK.ORG”解析的IP地址ADD IPV4DNSH: HSINDEX=1000, HOSTNAME="TOPON.ETH1.RTYBG.EPC.MNC003.MCC460.3GPPNETWORK.ORG", ADDRSECTION=SECTION1, IPV4ADDR1="192.168.183.1";

拨测MSISDN为13515000001的用户选择IP地址为"192.168.179.1"的S-GW，以 UNC 本地进行NAPTR查询配置，进行如下配置:

1. 配置MSISDN=“13515000001”指定S-GW的查询域名ADD SGWSELPLCY: SUBRANGE=MSISDN_PREFIX, MSISDNPRE="13515000001", SLCTM=S-GW_QNAME, QNAME="TEST.TAC-LB42.TAC-HB40.TAC.EPC.MNC003.MCC460.3GPPNETWORK.ORG";
2. 配置域名“TEST.TAC-LB42.TAC-HB40.TAC.EPC.MNC003.MCC460.3GPPNETWORK.ORG”的查询方式为NAPTR查询ADD DNSN: FQDN="TEST.TAC-LB42.TAC-HB40.TAC.EPC.MNC003.MCC460.3GPPNETWORK.ORG", HSINDEX=1000, ENTITY=SGW, INTYPE=S11;
3. 配置域名“TOPON.ETH1.SHANGHAI.EPC.MNC003.MCC460.3GPPNETWORK.ORG”解析的IP地址ADD IPV4DNSH: HSINDEX=1000, HOSTNAME="TOPON.ETH1.SHANGHAI.EPC.MNC003.MCC460.3GPPNETWORK.ORG", ADDRSECTION=SECTION1, IPV4ADDR1="192.168.179.1";
