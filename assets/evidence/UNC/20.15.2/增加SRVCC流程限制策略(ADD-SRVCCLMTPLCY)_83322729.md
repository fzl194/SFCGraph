# 增加SRVCC流程限制策略(ADD SRVCCLMTPLCY)

- [命令功能](#ZH-CN_MMLREF_0000001783322729__1.3.1.1)
- [注意事项](#ZH-CN_MMLREF_0000001783322729__1.3.2.1)
- [本地用户权限](#ZH-CN_MMLREF_0000001783322729__1.3.3.1)
- [网管用户权限](#ZH-CN_MMLREF_0000001783322729__1.3.4.1)
- [参数说明](#ZH-CN_MMLREF_0000001783322729__1.3.5.1)
- [使用实例](#ZH-CN_MMLREF_0000001783322729__1.3.6.1)

#### [命令功能](#ZH-CN_MMLREF_0000001783322729)

**适用网元：MME**

该命令用于增加SRVCC流程限制策略。

#### [注意事项](#ZH-CN_MMLREF_0000001783322729)

- 此命令执行后立即生效。
- 增加SRVCC流程限制策略后，下列性能指标可能会减少：[117492512 S1模式SRVCC请求次数](../../../../../../../性能指标/UNC业务性能指标/业务性能指标/SGSN_MME业务性能指标/USN性能指标/S1模式移动管理/S1模式SRVCC/117492512 S1模式SRVCC请求次数_06832078.md)。
- 当SRVCC流程限制策略配置为限制携带SRVCC Operation Possible信元，软参BYTE_EX_B330 BIT2功能不生效；否则软参BYTE_EX_B330 BIT2功能不受影响。
- 此命令最大记录数为1024条。

#### [本地用户权限](#ZH-CN_MMLREF_0000001783322729)

manage-ug；system-ug

#### [网管用户权限](#ZH-CN_MMLREF_0000001783322729)

G_1，管理员级别命令组；G_2，操作员级别命令组

#### [参数说明](#ZH-CN_MMLREF_0000001783322729)
...
| SRVCCCAPCFG | 是否限制携带SRVCC能力 | 可选必选说明：可选参数<br>参数含义：该参数用于指定是否限制SRVCC流程，如果限制则不携带SRVCC Operation Possible信元给eNodeB。<br>数据来源：整网规划<br>取值范围：<br>- NO(否)<br>- YES(是)<br>默认值：NO(否)<br>配置原则：当SRVCC功能支持，但不期望用户发起SRVCC流程，限制在DOWNLINK NAS TRANSPORT、INITIAL CONTEXT SETUP REQUEST、HANDOVER REQUEST消息中携带SRVCC Operation Possible信元给eNodeB，将该参数设置为<br>“YES(是)”<br>。否则配置为<br>“NO(否)”<br>，保持原有实现。 |
| EMGCAPCFG | 紧急呼叫是否限制SRVCC能力 | 可选必选说明：条件可选参数<br>参数含义：该参数用于指定是否限制紧急呼叫SRVCC流程。如果限制则不携带SRVCC Operation Possible信元给eNodeB。<br>前提条件：该参数在<br>“是否限制携带SRVCC能力”<br>参数配置为<br>“YES(是)”<br>后生效。<br>数据来源：整网规划<br>取值范围：<br>- NO(否)<br>- YES(是)<br>默认值：YES(是)<br>配置原则：当SRVCC功能支持时，但不期望用户发起紧急呼叫SRVCC流程，限制在DOWNLINK NAS TRANSPORT、INITIAL CONTEXT SETUP REQUEST、HANDOVER REQUEST消息中携带SRVCC Operation Possible信元给eNodeB，将该参数设置为<br>“YES(是)”<br>。否则配置为<br>“NO(否)”<br>，保持原有实现。 |

#### [使用实例](#ZH-CN_MMLREF_0000001783322729)

增加一条SRVCC流程限制策略配置， “用户范围” 为 “ALL_USER” ， “是否限制携带SRVCC能力” 为 “NO(否)” ：

```
ADD SRVCCLMTPLCY: SUBRANGE=ALL_USER, SRVCCCAPCFG=NO;
```
