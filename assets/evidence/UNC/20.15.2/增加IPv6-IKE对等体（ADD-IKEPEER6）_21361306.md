...
- [命令功能](#ZH-CN_MMLREF_0000001121361306__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001121361306__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001121361306__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000001121361306__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000001121361306)

该命令用于增加IKE对等体。

## [注意事项](#ZH-CN_MMLREF_0000001121361306)
...
#### [操作用户权限](#ZH-CN_MMLREF_0000001121361306)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0000001121361306)
...
| EXCHANGEMODE | 交换模式 | 可选必选说明：可选参数<br>参数含义：交换模式。<br>数据来源：对端协商<br>取值范围：<br>- “Main（主模式）”：主模式<br>- “Aggressive（野蛮模式）”：野蛮模式<br>默认值：Main<br>配置原则：无 |
...
| LOCALIDTYPE | 本地ID类型 | 可选必选说明：可选参数<br>参数含义：本地ID的类型。<br>数据来源：本端规划<br>取值范围：<br>- “IPv6（IPv6）”：IPv6<br>- “Fqdn_ipv6（Fqdn_Ipv6）”：Fqdn_Ipv6<br>- “User_fqdn_ipv6（User_fqdn_ipv6）”：User_fqdn_ipv6<br>- “Dn_ipv6（Dn_ipv6）”：Dn_ipv6<br>- “Ip_disable（IP-disable）”：关闭local ip的IP校验<br>默认值：IPv6<br>配置原则：无 |
| REMOTEID | 远端ID | 可选必选说明：可选参数<br>参数含义：远端ID。<br>数据来源：对端协商<br>取值范围：字符串类型，输入长度范围是1~255。<br>默认值：无<br>配置原则：无 |
| VERSION2 | IKE版本2 | 可选必选说明：可选参数<br>参数含义：IKE版本2。<br>数据来源：对端协商<br>取值范围：<br>- TRUE(TRUE)<br>- FALSE(FALSE)<br>默认值：TRUE<br>配置原则：<br>该参数用于配置ike版本，TRUE代表使用IKE版本2，FALSE代表使用IKE版本1。 |
...
| DPDHASHSEQ | DPD载荷顺序 | 可选必选说明：可选参数<br>参数含义：IKEv1 DPD载荷顺序。<br>数据来源：本端规划<br>取值范围：<br>- “Hash_notify（DPD载荷顺序先hash后notify）”：DPD载荷顺序先hash后notify<br>- “Notify_hash（DPD载荷顺序先notify后hash）”：DPD载荷顺序先notify后hash<br>默认值：Hash_notify<br>配置原则：无 |
| IKEMSGSYNC | 使能IKE消息序列号同步 | 可选必选说明：可选参数<br>参数含义：使能IKE消息序列号同步。<br>数据来源：本端规划<br>取值范围：<br>- Disable（不使能）<br>- Enable（使能）<br>默认值：Disable<br>配置原则：无 |
| IPSECMSGSYNC | 使能IPSEC消息序列号同步 | 可选必选说明：可选参数<br>参数含义：使能IPSEC消息序列号同步。<br>数据来源：本端规划<br>取值范围：<br>- Disable（不使能）<br>- Enable（使能）<br>默认值：Disable<br>配置原则：无 |
...
## [使用实例](#ZH-CN_MMLREF_0000001121361306)

增加名称为“peer1”的IKE对等体，同时指定交换模式为main模式：

```
ADD IKEPEER6:PEERNAME="peer1",EXCHANGEMODE=Main;
```
