# 修改SM原因值映射配置（MOD SMCAUSEMAP）

- [命令功能](#ZH-CN_MMLREF_0209654181__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0209654181__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0209654181__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0209654181__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0209654181)

**适用NF：SMF、PGW-C、SGW-C、GGSN**

该命令用于修改一个原因值映射配置记录。

## [注意事项](#ZH-CN_MMLREF_0209654181)

- 该命令执行后立即生效。

- 通过“CAUSEGRPID”、“CAUSERANGE”、“BGCAUSE”的值来确定一条记录，只能修改“TCAUSE”参数的值。
- 执行该命令，可能会改变通过该映射配置进行原因值控制的异常场景中下发给终端的拒绝原因值，从而导致相应拒绝类话统指标的变化。
- 配置下发的原因值可能会对终端行为产生影响，在配置前评估影响。关于不同原因值的含义及对终端行为的影响请参见协议3GPP TS 24.008、3GPP TS 24.301或3GPP TS24.501。

#### [操作用户权限](#ZH-CN_MMLREF_0209654181)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0209654181)
...
## [使用实例](#ZH-CN_MMLREF_0209654181)

修改原因值映射配置，把CAUSEGRPID（原因值组标识）设置为126，CAUSERANGE（原因值范围）设置为DEFAULT（缺省），TCAUSE（目标原因值）修改为37：

```
MOD SMCAUSEMAP: CAUSEGRPID=126, CAUSERANGE=DEFAULT, TCAUSE=37;
```
