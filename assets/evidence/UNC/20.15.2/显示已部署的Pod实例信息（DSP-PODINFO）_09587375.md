# 显示已部署的Pod实例信息（DSP PODINFO）

- [命令功能](#ZH-CN_MMLREF_0209587375__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0209587375__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0209587375__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0209587375__1.3.5)
- [输出结果说明](#ZH-CN_MMLREF_0209587375__1.3.6)

## [命令功能](#ZH-CN_MMLREF_0209587375)

实例化成功之后，执行此命令，用于显示已部署的Pod实例信息。

## [注意事项](#ZH-CN_MMLREF_0209587375)

无

#### [操作用户权限](#ZH-CN_MMLREF_0209587375)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0209587375)

| 参数标识 | 参数名称 | 参数说明 |
| --- | --- | --- |
| MEID | 网元ID | 可选必选说明：可选参数<br>参数含义：该参数用于标识Pod所在的网元ID。<br>数据来源：本端规划<br>取值范围：字符串类型，输入长度范围是0~40。<br>默认值：无<br>配置原则：无 |

## [使用实例](#ZH-CN_MMLREF_0209587375)
...
## [输出结果说明](#ZH-CN_MMLREF_0209587375)

| 输出项名称 | 输出项解释 |
| --- | --- |
| Pod类型 | 该参数用于表示Pod类型，如sfm-pod，具体类型定义请参考tosca模板中的定义。 |
| Pod标识 | 该参数用于表示Pod标识。 |
| 容器类型 | 该参数用于表示容器类型，如ContainerSm、ContainerAm，具体类型定义请参考tosca模板中的定义。 |
| 网元ID | 该参数用于标识Pod所在的网元ID。 |
| GID | 该参数用于表示Pod的身份标志，由SF-SCF分配。 |
