# 增加网元服务发现功能管理参数（ADD NFDISCMGMTPARA）

- [命令功能](#ZH-CN_MMLREF_0000001094969700__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001094969700__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001094969700__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000001094969700__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000001094969700)

**适用NF：AMF、SMF、NCG、SMSF**

该命令用于增加网元服务发现功能管理参数。以NFType为粒度配置不同的服务发现匹配策略。

## [注意事项](#ZH-CN_MMLREF_0000001094969700)
...
#### [操作用户权限](#ZH-CN_MMLREF_0000001094969700)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0000001094969700)
...
| TAIRANGESELSW | TAI精细化选择开关 | 可选必选说明：该参数在"NFTYPE"配置为"NfAMF"、"NfSMF"时为条件可选参数。<br>参数含义：该参数用于指定服务发现过程中是否开启TAI精细化选择功能。若开关设置为ON时，服务发现会优选TAI区间更精细的网元。若开关设置为OFF时，服务发现不开启TAI精细化选择功能。精细化选择功能仅作用于本地配置命中或者缓存命中的服务发现结果。顺序在优先级选择功能在之后，在本大区locality优选之前。<br>数据来源：本端规划<br>取值范围：<br>- “OFF（OFF）”：功能关闭<br>- “ON（ON）”：功能开启<br>默认值：无<br>配置原则：无 |
...
## [使用实例](#ZH-CN_MMLREF_0000001094969700)

- 服务发现AMF时，开启TAI精细化选择功能。
  ```
  ADD NFDISCMGMTPARA:NFTYPE=NfAMF,TAIRANGESELSW=ON;
  ```
- 服务发现UDM时，开启SUPI精细化选择功能。
  ```
  ADD NFDISCMGMTPARA:NFTYPE=NfUDM,SUPIRANGESELSW=ON,GPSIRANGESELSW=OFF;
  ```
- 服务发现PCF时，同时开启SUPI和GPSI的精细化选择功能。
  ```
  ADD NFDISCMGMTPARA:NFTYPE=NfPCF,SUPIRANGESELSW=ON,GPSIRANGESELSW=ON;
  ```
