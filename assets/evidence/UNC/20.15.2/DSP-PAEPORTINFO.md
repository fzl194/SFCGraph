# 显示PAE端口基本信息（DSP PAEPORTINFO）

- [命令功能](#ZH-CN_CONCEPT_0192520040__1.3.1.1)
- [注意事项](#ZH-CN_CONCEPT_0192520040__1.3.2.1)
- [操作用户权限](#ZH-CN_CONCEPT_0192520040__1.3.3.1)
- [参数说明](#ZH-CN_CONCEPT_0192520040__1.3.4.1)
- [使用实例](#ZH-CN_CONCEPT_0192520040__1.3.5.1)
- [输出结果说明](#ZH-CN_CONCEPT_0192520040__1.3.6.1)

#### [命令功能](#ZH-CN_CONCEPT_0192520040)

该命令用于显示指定微服务的PAE端口信息。流量不通或者端口状态异常场景下，需要确认当前微服务的PAE端口信息以及相关配置是否正常时，可以使用该命令。

#### [注意事项](#ZH-CN_CONCEPT_0192520040)

该命令执行后立即生效。内联口不支持ARP和PING等主机报文应答。

#### [操作用户权限](#ZH-CN_CONCEPT_0192520040)

G_1，管理员级别命令组；G_2，操作员级别命令组；G_3，用户级别命令组

#### [参数说明](#ZH-CN_CONCEPT_0192520040)
...
#### [输出结果说明](#ZH-CN_CONCEPT_0192520040)
...
| 掩码或前缀长度 | 端口的掩码或前缀长度。 |
| MAC地址 | 端口的MAC地址。 |
| MTU（byte） | 端口的最大传输单元，单位为字节。 |
| MTU配置类型 | 端口的最大传输单元配置类型。 |
| 端口状态 | 端口状态。 |
| 命名空间 | 命名空间名称。命名空间是Linux提供的一种资源隔离方案，不同命名空间内的资源只在所属命名空间内有效。通过命名空间隔离可以隔离不同的网络资源，实现不同命名空间内支持配置相同IP而互不冲突。 |
| 绑定模式 | 端口绑定模式。 |
| 队列数 | 队列数。 |
