# 修改去活用户面专有QoS Flow策略（MOD DEACTQFPLCY）

- [命令功能](#ZH-CN_MMLREF_0000001092470772__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001092470772__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001092470772__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000001092470772__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000001092470772)

**适用NF：SMF**

该命令用于修改当SMF接收到AMF发送的去激活用户面请求时，专有QoS Flow的处理策略。

## [注意事项](#ZH-CN_MMLREF_0000001092470772)

该命令执行后立即生效。

#### [操作用户权限](#ZH-CN_MMLREF_0000001092470772)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0000001092470772)
...
| QFPLCY | 专有GBR类型QoS Flow处理策略 | 可选必选说明：可选参数<br>参数含义：该参数用于指定当SMF接收到AMF发送的去激活用户面请求时，专有GBR类型QoS Flow的处理策略。<br>数据来源：本端规划<br>取值范围：<br>- RELEASE（释放专有QoS Flow）<br>- DELAY_RELEASE（延迟释放专有QoS Flow）<br>- NOT_RELEASE（不释放专有QoS Flow）<br>默认值：无<br>配置原则：无 |
| DELAYTIMER | 延迟释放专有GBR类型QoS Flow时长(秒) | 可选必选说明：该参数在"QFPLCY"配置为"DELAY_RELEASE"时为条件可选参数。<br>参数含义：该参数用于指定延迟释放专有GBR类型QoS Flow时长。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围是10~100，单位是秒。<br>默认值：无<br>配置原则：无 |

## [使用实例](#ZH-CN_MMLREF_0000001092470772)

当需要修改SMF收到AMF发起的用户面去活请求携带原因值“radio-connection-with-ue-lost”时能够延迟释放专有QoS Flow的延迟时间为20秒时，执行如下命令：

```
MOD DEACTQFPLCY: NGAPCAUSEGROUP=RADIO_NETWORK, NGAPCAUSEVALUE=21, QFPLCY=DELAY_RELEASE, DELAYTIMER=20;
```
