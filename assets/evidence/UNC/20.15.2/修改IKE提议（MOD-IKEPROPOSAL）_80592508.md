# 修改IKE提议（MOD IKEPROPOSAL）

- [命令功能](#ZH-CN_MMLREF_0000001180592508__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001180592508__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001180592508__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000001180592508__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000001180592508)

![](修改IKE提议（MOD IKEPROPOSAL）_80592508.assets/notice_3.0-zh-cn_2.png)

修改配置，可能是不安全算法，影响加解密安全性。

如果修改后的IKE提议中的配置与对端规划的IKE提议配置不一致，会导致IKE SA协商失败。

该命令用于修改IKE安全提议配置。

## [注意事项](#ZH-CN_MMLREF_0000001180592508)
...
#### [操作用户权限](#ZH-CN_MMLREF_0000001180592508)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0000001180592508)
...
| AUTHMETHOD | 认证方法 | 可选必选说明：可选参数<br>参数含义：认证方法。<br>数据来源：对端协商<br>取值范围：<br>- “Pre_share（预共享）”：预共享密钥方式<br>- “Rsa_signature（RSA数字签名）”：RSA数字签名<br>- “Cert_signature（数字证书签名）”：数字证书签名<br>- “Digital_envelope（数字信封）”：数字信封<br>默认值：无<br>配置原则：无 |
| AUTHALGORITHM | 认证算法 | 可选必选说明：可选参数<br>参数含义：认证算法。<br>数据来源：对端协商<br>取值范围：<br>- “Md5（Md5算法）”：Md5算法<br>- “Sha1（Sha1算法）”：Sha1算法<br>- “Sha2_256（Sha2-256算法）”：Sha2-256算法<br>- “Sha2_384（Sha2-384算法）”：Sha2-384算法<br>- “Sha2_512（Sha2-512算法）”：Sha2-512算法<br>默认值：无<br>配置原则：无 |
| ENCRALGORITHM | 加密算法 | 可选必选说明：可选参数<br>参数含义：加密算法。<br>数据来源：对端协商<br>取值范围：<br>- “Des_cbc（DES算法）”：DES算法<br>- “Aes_cbc_128（128位AES算法）”：128位AES算法<br>- “Aes_cbc_192（192位AES算法）”：192位AES算法<br>- “Aes_cbc_256（256位AES算法）”：256位AES算法<br>- “Alg_3Des_cbc（3DES算法）”：3DES算法<br>- “Aes_gcm_128（128位AES-GCM算法）”：128位AES-GCM算法<br>- “Aes_gcm_256（256位AES-GCM算法）”：256位AES-GCM算法<br>- “Sm4（SM4算法）”：SM4算法<br>默认值：无<br>配置原则：无 |
...
| SADURATION | SA持续长度 (s) | 可选必选说明：可选参数<br>参数含义：SA生命周期。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围是60~604800，单位是秒。<br>默认值：无<br>配置原则：无 |
| SIGHASHALGNEGSW | 签名哈希算法协商开关 | 可选必选说明：该参数在"AUTHMETHOD"配置为"Cert_signature"时为条件可选参数。<br>参数含义：签名哈希算法协商开关。<br>数据来源：对端协商<br>取值范围：<br>- “Disable（未使能）”：未使能<br>- “Enable（使能）”：使能<br>默认值：无<br>配置原则：无 |
| SIGNPADDING | 签名填充方式 | 可选必选说明：该参数在"SIGHASHALGNEGSW"配置为"Enable"时为条件可选参数。<br>参数含义：签名填充方式。<br>数据来源：对端协商<br>取值范围：<br>- “Pkcs（Pkcs填充）”：Pkcs填充<br>- “Pss（Pss填充）”：Pss填充<br>默认值：无<br>配置原则：无 |
...
## [使用实例](#ZH-CN_MMLREF_0000001180592508)

修改IKE安全提议号为1的IKE安全提议的认证方法为pre_share：

```
MOD IKEPROPOSAL:PROPOSALNUMBER=1,AUTHMETHOD=Pre_share;
```
