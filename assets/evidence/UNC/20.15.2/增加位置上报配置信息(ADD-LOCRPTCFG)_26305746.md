# 增加位置上报配置信息(ADD LOCRPTCFG)

- [命令功能](#ZH-CN_MMLREF_0000001126305746__1.3.1.1)
- [注意事项](#ZH-CN_MMLREF_0000001126305746__1.3.2.1)
- [本地用户权限](#ZH-CN_MMLREF_0000001126305746__1.3.3.1)
- [网管用户权限](#ZH-CN_MMLREF_0000001126305746__1.3.4.1)
- [参数说明](#ZH-CN_MMLREF_0000001126305746__1.3.5.1)
- [使用实例](#ZH-CN_MMLREF_0000001126305746__1.3.6.1)

#### [命令功能](#ZH-CN_MMLREF_0000001126305746)

**适用网元：SGSN、MME**

该命令用于增加位置上报配置信息。当需要对特定范围的GGSN、PGW启用“WSFD- 106402 基于实时位置信息的策略控制功能”特性时，需要执行本命令。

#### [注意事项](#ZH-CN_MMLREF_0000001126305746)
...
#### [本地用户权限](#ZH-CN_MMLREF_0000001126305746)

manage-ug；system-ug

#### [网管用户权限](#ZH-CN_MMLREF_0000001126305746)

G_1，管理员级别命令组；G_2，操作员级别命令组

#### [参数说明](#ZH-CN_MMLREF_0000001126305746)
...
| IPT | IP地址类型 | 可选必选说明：条件必选参数<br>参数含义：该参数用于指定GGSN/P-GW信令面IP地址类型。<br>前提条件：只有在<br>“RANGE”<br>配置为<br>“SPECIFIC_GGSN/P-GW（指定IP的GGSN/P-GW）”<br>时，该参数有效。<br>数据来源：整网规划<br>取值范围：<br>- “IPV4(IPV4)”<br>- “IPV6(IPV6)”<br>默认值：IPV4(IPV4) |
...
| LOCRPT | 支持位置上报功能 | 可选必选说明：可选参数<br>参数含义：该参数用于指定GGSN/P-GW是否支持位置上报功能。<br>数据来源：与对端GGSN/P-GW网元协商一致<br>取值范围：<br>- “NO(否)”<br>- “YES(是)”<br>默认值：NO(否)<br>说明：- 当参数设置为“YES(是)”时，“小区位置信息上报功能（S11接口）”特性的相关license授权并开启后，此参数配置才生效（特性编号：WSFD-106403，license部件编码：LKV2LCR01）。 |
| DESC | 描述 | 可选必选说明：可选参数<br>参数含义：该参数用于指定位置上报配置信息的相关描述。<br>数据来源：整网规划<br>取值范围：1～32位字符串<br>默认值：无 |

#### [使用实例](#ZH-CN_MMLREF_0000001126305746)

增加记录，配置用户范围为 “SPECIFIC_GGSN/P-GW（指定IP的GGSN/P-GW）” ，信令面IP地址为 “10.10.10.17” ，掩码为 “255.255.255.255” ，支持实时上报，描述为 “GGSN01” ：

ADD LOCRPTCFG: RANGE=SPECIFIC_GGSN/P-GW, IPT=IPV4, GWIPV4="10.10.10.17", MASKV4="255.255.255.255", LOCRPT=YES, DESC="GGSN01";
