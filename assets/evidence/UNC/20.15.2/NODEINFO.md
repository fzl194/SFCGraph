...
- [命令功能](#ZH-CN_MMLREF_0294730419__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0294730419__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0294730419__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0294730419__1.3.5)
- [输出结果说明](#ZH-CN_MMLREF_0294730419__1.3.6)

## [命令功能](#ZH-CN_MMLREF_0294730419)

该命令根据指定网元ID和类型查询节点的导出信息。若要获取最新的节点信息，请先使用EXP NODEINFO导出信息，再通过该命令查询导出的信息。

## [注意事项](#ZH-CN_MMLREF_0294730419)

- 该命令返回的信息为EXP NODEINFO的导出信息，若不执行EXP NODEINFO，则其返回信息不会发生变化。
- 若执行命令后的返回码为66348，则表示未导出过节点信息，请先使用EXP NODEINFO导出信息。
- 裸机容器场景不支持VF驱动的检查。

#### [操作用户权限](#ZH-CN_MMLREF_0294730419)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0294730419)
...
## [输出结果说明](#ZH-CN_MMLREF_0294730419)

| 输出项名称 | 输出项解释 |
| --- | --- |
| 网元ID | 该参数用于指定网元来查询节点信息；若不输入，则表示当前控制节点所在网元；网元ID可以通过LST ME命令查询出来。 |
| 查询类型 | 该参数用于指定需要查询信息的类型。<br>取值说明：<br>- VF_DRIVER（VF驱动） |
| 节点IP | 该参数用于显示节点的IP地址。 |
| 节点信息 | 改参数用于显示查询到的节点信息。 |
