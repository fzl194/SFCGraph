# 修改签约数据纠正参数(MOD SMSUBDATA)

- [命令功能](#ZH-CN_MMLREF_0000001126145678__1.3.1.1)
- [注意事项](#ZH-CN_MMLREF_0000001126145678__1.3.2.1)
- [本地用户权限](#ZH-CN_MMLREF_0000001126145678__1.3.3.1)
- [网管用户权限](#ZH-CN_MMLREF_0000001126145678__1.3.4.1)
- [参数说明](#ZH-CN_MMLREF_0000001126145678__1.3.5.1)
- [使用实例](#ZH-CN_MMLREF_0000001126145678__1.3.6.1)

#### [命令功能](#ZH-CN_MMLREF_0000001126145678)

**适用网元：SGSN、MME**

该命令用于修改一条签约数据修改的记录，修改IMSI号段、PDP TYPE、APN匹配的用户群的签约数据类型。使用该命令时建议设置软参DWORD_EX26 BIT13的值为1。

#### [注意事项](#ZH-CN_MMLREF_0000001126145678)

- 该命令执行后立即生效。

#### [本地用户权限](#ZH-CN_MMLREF_0000001126145678)

manage-ug；system-ug

#### [网管用户权限](#ZH-CN_MMLREF_0000001126145678)

G_1，管理员级别命令组；G_2，操作员级别命令组

#### [参数说明](#ZH-CN_MMLREF_0000001126145678)
...
| CORRECTAPNNI | 修改签约APNNI | 可选必选说明：可选参数<br>参数含义：该参数用于标识是否要修改签约的APNNI。<br>数据来源：本端规划<br>取值范围：<br>- “YES(是)”：用于增加一条修改签约的APNNI记录。<br>- “NO(否)”<br>默认值：<br>“无”<br>。 |
| NEWAPNNI | 新的签约APNNI | 可选必选说明：条件必选参数<br>参数含义：该参数用于指定新的APN网络标识。<br>前提条件：该参数在<br>“CORRECTAPNNI(修改签约APNNI)”<br>参数设置为<br>“YES(是)”<br>时，才需要配置。<br>数据来源：本端规划<br>取值范围：1～62位字符串<br>默认值：无<br>配置原则：<br>- APN网络标识地址由一个或多个LABEL构成，各LABEL间用“.”间隔。每个LABEL的构成字符只能是字母A～Z或a～z、数字0～9和中划线“-”。<br>- APN网络标识地址不能以“rac”、“lac”、“sgsn”或“rnc”开头，不能以“.gprs”结尾，不能取值为“*”。<br>- 输入的APN NI参数应为类似于XXX.XXX的形式，如“example.com”。如果配置的APN NI不正确，可能因DNS解析失败而导致激活流程失败。 |
| NEWPDPTYPE | 新的签约PDP类型 | 可选必选说明：可选参数<br>参数含义：该参数用于指定新的PDP类型，如果签约的PDP类型为Non-IP类型，本参数不生效。<br>数据来源：整网规划<br>取值范围：<br>- “IPV4(IPv4地址)”<br>- “IPV6(IPv6地址)”<br>- “PPP(PPP)”<br>- “IPV4V6(IPv4v6)”<br>- “IPV4/IPV6(IPv4/IPv6)”<br>- “NOT_MODIFY(不修改)”<br>默认值：无 |
...
#### [使用实例](#ZH-CN_MMLREF_0000001126145678)

修改一条签约数据纠正参数信息，其用户范围为所有用户，匹配条件为签约数据，签约APNNI范围为所有APNNI，PDP协议类型为PPP协议，不修改签约APNNI， 不修改新的PDP类型：

MOD SMSUBDATA: SUBRANGE=ALL_USER, TYPE=SUBSCRIBED_PARAMETER, APNNIRANGE=APNNI_ALL, PDPTYPE=PPP;
