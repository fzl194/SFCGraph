# 增加PLMN-CG配置参数（ADD CHGPLMNCG）

- [命令功能](#ZH-CN_MMLREF_0000001172225067__1.3.1.1)
- [注意事项](#ZH-CN_MMLREF_0000001172225067__1.3.2.1)
- [本地用户权限](#ZH-CN_MMLREF_0000001172225067__1.3.3.1)
- [网管用户权限](#ZH-CN_MMLREF_0000001172225067__1.3.4.1)
- [参数说明](#ZH-CN_MMLREF_0000001172225067__1.3.5.1)
- [使用实例](#ZH-CN_MMLREF_0000001172225067__1.3.6.1)

#### [命令功能](#ZH-CN_MMLREF_0000001172225067)
...
#### [注意事项](#ZH-CN_MMLREF_0000001172225067)
...
- 不同命令配置的CG优先级从高到低的次序是：[**ADD CHGPLMNCG**](增加PLMN-CG配置参数（ADD CHGPLMNCG）_72225067.md)，[**ADD CHGBEHA**](../计费行为参数配置/增加计费行为参数(ADD CHGBEHA)_26145366.md)，[**SET CHGCHAR**](../计费属性参数配置/设置计费属性参数(SET CHGCHAR)_26145368.md)，[**ADD CHGCG**](../计费控制/增加CG配置参数（ADD CHGCG）_72225055.md)。
- 增加数据时，需要参照[**SET CHGCDR**](../计费控制/设置计费CDR参数（SET CHGCDR）_26145372.md)中的参数“EXTRANGE”。如果“EXTRANGE”设置为“禁止非PLMNCG”时，新增记录和表中各记录之间的CGIP1，CGIP2，CGIP3，CGIP4，CGIP5要么完全相同，要么完全不同，否则赋值报错。如果“EXTRANGE”设置为“允许非PLMNCG”时，则无需判断。
...
#### [本地用户权限](#ZH-CN_MMLREF_0000001172225067)

manage-ug；system-ug

#### [网管用户权限](#ZH-CN_MMLREF_0000001172225067)

G_1，管理员级别命令组；G_2，操作员级别命令组

#### [参数说明](#ZH-CN_MMLREF_0000001172225067)
...
| IPT | IP地址类型 | 可选必选说明：可选参数<br>参数含义：该参数用于表示CG的IP地址类型。<br>数据来源：全网规划<br>取值范围：<br>- “IPV4(IPV4地址)”<br>- “IPV6(IPV6地址)”<br>默认值：“IPV4(IPV4地址)” |
| CGIP1 | CG1的IPV4地址 | 可选必选说明：条件必选参数<br>参数含义：该参数用于表示CG1的IPV4地址。<br>前提条件：该参数在<br>“IPT(IP地址类型)”<br>配置为<br>“IPV4(IPV4地址)”<br>后生效。<br>数据来源：整网规划<br>取值范围：0.0.0.0～255.255.255.255<br>默认值：无<br>说明：“0.0.0.0”<br>和<br>“255.255.255.255”<br>是无效的IP地址。 |
...
| CG1IPV6 | CG1的IPV6地址 | 可选必选说明：条件必选参数<br>参数含义：该参数用于表示CG1的IPV6地址。<br>前提条件：该参数在<br>“IPT(IP地址类型)”<br>配置为<br>“IPV6(IPV6地址)”<br>后生效。<br>数据来源：全网规划<br>取值范围：::～FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF<br>默认值：无<br>配置原则：<br>- IPV6地址必须是全球单播地址，不能为::、FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF、环回地址(::1)、链路本地地址(FE80::/10)和组播地址(FF00::/8)。 |
...
#### [使用实例](#ZH-CN_MMLREF_0000001172225067)

为PLMN为123001配置优先发送话单的CG IPV4地址，两个CG IP地址分别为“192.168.66.5”和“192.168.177.18”：

ADD CHGPLMNCG: MCC="123", MNC="001", IPT=IPV4, CGIP1="192.168.66.5", CGIP2="192.168.177.18";
