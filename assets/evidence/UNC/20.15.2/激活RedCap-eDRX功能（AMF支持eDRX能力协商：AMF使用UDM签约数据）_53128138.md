# 激活RedCap eDRX功能（AMF支持eDRX能力协商：AMF使用UDM签约数据）

- [操作场景](#ZH-CN_OPI_0000001553128138__1.3.1)
- [必备事项](#ZH-CN_OPI_0000001553128138__1.3.2)
- [操作步骤](#ZH-CN_OPI_0000001553128138__1.3.3)
- [任务示例](#ZH-CN_OPI_0000001553128138__1.3.4)

## [操作场景](#ZH-CN_OPI_0000001553128138)
...
## [必备事项](#ZH-CN_OPI_0000001553128138)
...
| [**ADD NGM2MPLCY**](../../../../../OM参考/命令/UNC MML命令/业务服务管理/5G M2M管理/5G M2M策略参数配置/增加5G M2M策略参数（ADD NGM2MPLCY）_85251937.md) | NR模式EDRX寻呼周期签约优先（NREDRXSUBPRI） | YES | 全网规划 | 当UDM没有签约eDRX参数时，AMF优先使用参数“NRECL“和“NRPTW“的值。 |
...
## [操作步骤](#ZH-CN_OPI_0000001553128138)
...
## [任务示例](#ZH-CN_OPI_0000001553128138)

任务描述

支持eDRX功能的UE接入核心网时，AMF与gNodeB协商使用同一时间UTC，并且AMF提前默认值3s发起寻呼。AMF优先使用UDM下发的eDRX参数。

脚本

//配置AMF和gNodeB使用同一时间类型UTC。

```
SET HSFNTIME: TIMETYPE=UTC, DATE=2010/01/01, TIME=00：00：00;
```

//关闭eDRX短周期使能开关，NR模式寻呼窗口差值配置为3s。

```
SET NGM2MCTRL: SHORTECLSW=NO, NREDRXPAGDIFF=MAX_DIFF_3_SECOND, REACHEVENTNSSW=YES, INTRAUSRPLANOPT=YES, INTRAUSRPLANOPT=YES;
```

//配置5G M2M终端省电策略：AMF优先使用UDM签约的eDRX参数。

```
ADD NGM2MPLCY: SUBRANGE=HOME_USER, FEATURECOND=DNN, DNNNI=huawei.com, EDRXSW=ON, NREDRXSUBPRI=YES;
```
