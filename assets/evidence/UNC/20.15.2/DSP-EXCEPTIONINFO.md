# 显示RU异常信息（DSP EXCEPTIONINFO）

- [命令功能](#ZH-CN_CONCEPT_0259104276__1.3.1.1)
- [注意事项](#ZH-CN_CONCEPT_0259104276__1.3.2.1)
- [操作用户权限](#ZH-CN_CONCEPT_0259104276__1.3.3.1)
- [参数说明](#ZH-CN_CONCEPT_0259104276__1.3.4.1)
- [使用实例](#ZH-CN_CONCEPT_0259104276__1.3.5.1)
- [输出结果说明](#ZH-CN_CONCEPT_0259104276__1.3.6.1)

#### [命令功能](#ZH-CN_CONCEPT_0259104276)

该命令用于显示当前是否存在异常信息。

#### [注意事项](#ZH-CN_CONCEPT_0259104276)

无。

#### [操作用户权限](#ZH-CN_CONCEPT_0259104276)

G_1，管理员级别命令组

#### [参数说明](#ZH-CN_CONCEPT_0259104276)
...
| BEGININDEX | 记录起始位置 | 可选必选说明：可选参数<br>参数含义：该参数用于表示每个资源单元从第几条记录开始显示。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围为0～19。<br>默认值：无<br>配置原则：当不输入时，若是某个资源单元的实际数据量达到此参数的最大值，则显示的数据量为此参数最大值，否则按实际数据量显示。当输入某个值时，若是某个资源单元的实际数据量达到此参数值，则显示的数据量为此参数值，否则按实际数据量显示。 |
...
#### [使用实例](#ZH-CN_CONCEPT_0259104276)

显示所有详细的异常信息：

```
DSP EXCEPTIONINFO: ISVERBOSE=YES
...
结果如下:
--------
异常类型ID    异常子类型ID    异常名称      RU名称                  进程ID    进程名称            异常发生时间           CPU Tick时间高8位    CPU Tick时间低8位    Fenix版本                     Dopra版本                      补丁版本          调用栈              寄存器信息                                                  转储栈
11            1               SIGSEGV       VNODE_CSLB_VNFC_OMU_0002 1011      APPLocation10001    2016-04-15 10:11:33    0x0000083e           0xF3C2329B           FENIXV100R005C00B090          DOPRA SSP V300R002C50SPC070    V100R005MOD000    
...
11            1               SIGSEGV       VNODE_CSLB_VNFC_OMU_0002 1010      APPLocation10001    2016-04-15 10:11:22    0x0000083E           0xD7428535           FENIXV100R005C00B090    DOPRA SSP V300R002C50SPC070    V100R005MOD000    
...
#### [输出结果说明](#ZH-CN_CONCEPT_0259104276)
...
| 进程名称 | 用于表示进程名称。 |
| 异常发生时间 | 用于表示异常发生的时间。 |
| CPU Tick时间高8位 | 用于表示CPU TICK时间的高八位。 |
| CPU Tick时间低8位 | 用于表示CPU TICK时间的低八位。 |
| Fenix版本 | 用于表示FENIX版本。 |
| Dopra版本 | 用于表示DOPRA版本。 |
| 补丁版本 | 用于表示补丁版本。 |
| 调用栈 | 用于表示调用栈信息。 |
