# 删除SM原因值映射配置（RMV SMCAUSEMAP）

- [命令功能](#ZH-CN_MMLREF_0209651423__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0209651423__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0209651423__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0209651423__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0209651423)

**适用NF：SMF、PGW-C、SGW-C、GGSN**

该命令用于删除一个原因值映射配置记录。原因值映射就是将原接口消息中的原因值直接映射到目标接口消息中下发的原因值。

## [注意事项](#ZH-CN_MMLREF_0209651423)

- 该命令执行后立即生效。

- 执行该命令，可能会改变通过该映射配置进行原因值控制的异常场景中下发给终端的拒绝原因值，从而导致相应拒绝类话统指标的变化。

#### [操作用户权限](#ZH-CN_MMLREF_0209651423)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0209651423)
...
## [使用实例](#ZH-CN_MMLREF_0209651423)

删除CAUSEGRPID（原因值组标识）为126，CAUSERANGE（原因值范围）为DEFAULT（缺省）的原因值映射记录：

```
RMV SMCAUSEMAP: CAUSEGRPID=126, CAUSERANGE=DEFAULT;
```
