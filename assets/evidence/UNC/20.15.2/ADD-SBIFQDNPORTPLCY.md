# 增加FQDN端口策略（ADD SBIFQDNPORTPLCY）

- [命令功能](#ZH-CN_MMLREF_0000001378027898__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001378027898__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001378027898__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000001378027898__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000001378027898)

**适用NF：AMF、SMF、NSSF、SMSF、NCG**

该命令用于添加FQDN使用端口号的策略。

## [注意事项](#ZH-CN_MMLREF_0000001378027898)

- 该命令执行后只对新激活用户生效。

- 无配置记录时，默认FQDN不使用端口号。
- 如果对端NF支持FQDN使用端口号，可以配置策略为使用端口号。
- 同一个对端NF如果有多个PLMN，多个PLMN的策略应保持一致。

- 最多可输入256条记录。

#### [操作用户权限](#ZH-CN_MMLREF_0000001378027898)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0000001378027898)
...
## [使用实例](#ZH-CN_MMLREF_0000001378027898)

- 增加根据对端FQDN组装对端Uri场景下，PLMN间默认策略为FQDN使用端口号。
  ```
  ADD SBIFQDNPORTPLCY:SCENE=TARGETADDR,SCOPE=INTERPLMNDEFAULT,PORTPLCY=USEPORT;
  ```
- 增加根据对端FQDN组装对端Uri场景下，对端PLMN为"12303"时，策略为FQDN不使用端口号。
  ```
  ADD SBIFQDNPORTPLCY:SCENE=TARGETADDR,SCOPE=BYPLMN,PLMN="12303",PORTPLCY=NOTUSEPORT;
  ```
