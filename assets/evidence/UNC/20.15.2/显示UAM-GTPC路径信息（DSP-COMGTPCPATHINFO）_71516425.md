# 显示UAM GTPC路径信息（DSP COMGTPCPATHINFO）

- [命令功能](#ZH-CN_MMLREF_0000001171516425__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001171516425__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001171516425__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000001171516425__1.3.5)
- [输出结果说明](#ZH-CN_MMLREF_0000001171516425__1.3.6)

## [命令功能](#ZH-CN_MMLREF_0000001171516425)

**适用NF：SGSN、MME、AMF**

该命令用于显示UAM GTP-C路径信息。

## [注意事项](#ZH-CN_MMLREF_0000001171516425)

- 如需要进行MME/SGSN单进程的查询，则需要通过DSP GTPCPATH命令查询。
- 当"RU名称"未输入时，默认只向AMF查询。
- 查询的GTP-C路径数目，最大可显示MME/SGSN以及AMF共5000条，大于5000条需要独立查询原子表。

#### [操作用户权限](#ZH-CN_MMLREF_0000001171516425)

G_1，管理员级别命令组；G_2，操作员级别命令组；G_3，用户级别命令组

## [参数说明](#ZH-CN_MMLREF_0000001171516425)
...
## [使用实例](#ZH-CN_MMLREF_0000001171516425)
...
## [输出结果说明](#ZH-CN_MMLREF_0000001171516425)

| 输出项名称 | 输出项解释 |
| --- | --- |
| IP地址类型 | 该参数用于指定GTP-C路径的IP地址类型。<br>取值说明：<br>- IPV4（IPV4）<br>- IPV6（IPV6） |
| 进程类型 | 该参数用于指定进程类型。<br>取值说明：<br>- SPP（SPP）<br>- UPP（UPP）<br>- NULL0（NULL）<br>- NULL255（NULL） |
| 进程号 | 该参数用于指定进程号。 |
| GTP-C路径数目 | 该参数用于显示查询的GTP-C路径数目，最大可显示MME/SGSN以及AMF各5000条，大于5000条需要独立查询原子表。 |
| GTP-C路径网络类型 | 该参数用于指定GTP-C路径网络类型。<br>取值说明：<br>- GPRS/UMTS（2G或3G网络）<br>- EPC（4G网络）<br>- TEST（测试）<br>- INTF_BUTT（空）<br>- CORE5G（5G网络）<br>- NULL255（空） |
| 发送Echo Request开关 | 该参数用于指定MME/SGSN是否发送Echo Request。<br>取值说明：<br>- OFF（关闭）<br>- ON（开启）<br>- NULL255（空） |
| 接口类型 | 该参数用于指定GTP-C路径的接口类型。<br>取值说明：<br>- UNKNOWN（unknown）<br>- Gn（Gn）<br>- Gp（Gp）<br>- S3（S3）<br>- S4（S4）<br>- S10（S10）<br>- S11（S11）<br>- S16（S16）<br>- Sm（Sm）<br>- Sv（Sv）<br>- S101（S101）<br>- Sn（Sn）<br>- N26（N26）<br>- S5（S5）<br>- S8（S8）<br>- ALL（All） |
| 路径断去激活会话时间 | 该参数用于指定当GTP-C路径持续故障后，使用该GTP-C路径在该时间之前接入的会话会被去激活，仅适用于AMF。 |
| 对端Recovery变化去激活会话时间 | 该参数用于指定当对端Recovery变化后，在该时间之前和对端创建的会话会被去激活，仅适用于AMF。 |
| 对端Recovery值 | 该参数用于指定该GTP-C路径指示的对端网元的Recovery值。 |
| Prn标志 | 该参数用于指定该GTP-C路径指示的对端网元是否支持PRN功能，仅适用于AMF。<br>取值说明：<br>- Enable（Enable）<br>- Disable（Disable） |
| Ntsr标志 | 该参数用于指定该GTP-C路径指示的对端网元是否支持NTSR功能，仅适用于AMF。<br>取值说明：<br>- Enable（Enable）<br>- Disable（Disable） |
| RU名称 | 该参数用于指定RU名称。 |
| 路径状态 | 该参数用于指定GTP-C路径的路径状态。<br>取值说明：<br>- NORMAL（正常）<br>- ERROR（故障）<br>- IDLE（Idle）<br>- DETECT（探测）<br>- NULL（空）<br>- NULL255（null） |
| GTP版本 | 该参数用于指定GTP-C路径的GTP版本号。<br>取值说明：<br>- GTPV0（GTP V0）<br>- GTPV1（GTP V1）<br>- GTPV2（GTP V2）<br>- UNKNOWN（未知）<br>- NULL255（Null） |
| 本端IP地址 | 该参数用于指定GTP-C路径的本端IP地址。 |
| 对端IP地址 | 该参数用于指定GTP-C路径的对端IP地址。 |
