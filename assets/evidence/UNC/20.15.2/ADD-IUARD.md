# 增加Iu模式接入限制参数(ADD IUARD)

- [命令功能](#ZH-CN_MMLREF_0000001172345073__1.3.1.1)
- [注意事项](#ZH-CN_MMLREF_0000001172345073__1.3.2.1)
- [本地用户权限](#ZH-CN_MMLREF_0000001172345073__1.3.3.1)
- [网管用户权限](#ZH-CN_MMLREF_0000001172345073__1.3.4.1)
- [参数说明](#ZH-CN_MMLREF_0000001172345073__1.3.5.1)
- [使用实例](#ZH-CN_MMLREF_0000001172345073__1.3.6.1)
- [参考信息](#ZH-CN_MMLREF_0000001172345073__1.3.7.1)

#### [命令功能](#ZH-CN_MMLREF_0000001172345073)
...
#### [注意事项](#ZH-CN_MMLREF_0000001172345073)

- 本表的最大记录数为2048。
- 当本表记录中存在用户的IMSI对应的IMSI前缀或者所在的IMSI范围，且用户的属性（APNNI/CSI/卡类型）至少有一项与IUARD配置的属性匹配时，按照该配置的控制类型（CTRLTYPE）对用户接入进行控制。
- 当本表记录中存在用户的IMSI对应的IMSI前缀或者所在的IMSI范围，且用户的属性（APNNI/CSI/卡类型）与IUARD配置的属性均不匹配时，按照与该配置相反的控制类型（CTRLTYPE）对用户接入进行控制。
...
#### [本地用户权限](#ZH-CN_MMLREF_0000001172345073)

manage-ug；system-ug

#### [网管用户权限](#ZH-CN_MMLREF_0000001172345073)

G_1，管理员级别命令组；G_2，操作员级别命令组

#### [参数说明](#ZH-CN_MMLREF_0000001172345073)
...
| ENDIMSI | 终止IMSI | 可选必选说明：条件必选参数<br>参数含义：该参数用于指定用户的终止IMSI。<br>前提条件：该参数在<br>“SUBRANGE(用户范围)”<br>设置为<br>“SPECIAL_IMSI_RANGE(指定IMSI范围)”<br>时生效。<br>数据来源：整网规划<br>取值范围：1~15位数字<br>默认值：无<br>配置原则：参见<br>[参考信息](#ZH-CN_MMLREF_0000001172345073__re1)<br>。 |
...
| CARDTYPE | 卡类型 | 可选必选说明：可选参数<br>参数含义：该参数用于指定用户的卡类型。<br>数据来源：整网规划<br>取值范围：<br>- “SIM(SIM)”<br>- “USIM(USIM)”<br>默认值：无<br>配置原则：<br>- 如果选中全部清空，表示不根据用户签约的SIM卡或USIM卡作为用户的附加属性进行接入限制判断条件。<br>- 如果选中其中之一或全选，表示SIM卡和/或USIM卡将作为用户的附加属性作为接入限制的判断条件，不符合条件的用户将采取与“控制类型”相反的控制策略。<br>- 如果选中“SIM”卡，控制类型为“REJECT(拒绝)”，表示非SIM卡的用户都将被允许接入。<br>- 如果选中“USIM”卡，控制类型为“ALLOW(允许)”，表示非“USIM”卡的用户都将被拒绝接入。<br>- 如果选中“SIM”卡和“USIM”卡，控制类型为“ALLOW(允许)”，表示不根据用户卡类型控制用户接入。<br>- 如果选中“SIM”卡和“USIM”卡，控制类型为“REJECT(拒绝)”，表示该记录中指定的用户范围内的用户都不能接入。<br>- 如果用户不配置鉴权(可执行[**LST IUAUTHCIPH**](../../../业务安全管理/用户安全管理/Iu模式用户安全参数/查询Iu模式用户安全参数(LST IUAUTHCIPH)_26145648.md)命令查看)，本命令中关于卡类型限制的判断不生效，此时不会对不鉴权的用户做卡类型的接入限制。 |
...
| CTRLTYPE | 控制类型 | 可选必选说明：必选参数<br>参数含义：该参数用于指定是否允许用户接入。<br>数据来源：整网规划<br>取值范围：<br>- “ALLOW(允许)”：表示符合该规则的用户是允许接入。<br>- “REJECT(拒绝)”：表示符合该规则的用户是拒绝接入。<br>默认值：无<br>配置原则：当只使用ARD的功能时，该参数建议设置为<br>“ALLOW(允许)”<br>，以防止收到APNNI默认值的影响。 |
...
#### [使用实例](#ZH-CN_MMLREF_0000001172345073)

1. 增加一条Iu模式接入限制参数记录，用户范围为ALL_USER，卡类型为SIM-1，控制类型为拒绝，原因值为GPRS_SERVICE_NOT_ALLOWED：
  ADD IUARD: SUBRANGE=ALL_USER, CARDTYPE=SIM-1, CTRLTYPE=REJECT, CAUSE=GPRS_SERVICE_NOT_ALLOWED;
2. 增加一条Iu模式接入限制参数记录，用户范围为SPECIAL_IMSIPRE，IMSIPRE为123456，卡类型为SIM-1，控制类型为拒绝，原因值为GPRS_SERVICE_NOT_ALLOWED：
  ADD IUARD: SUBRANGE=SPECIAL_IMSIPRE, IMSIPRE="123456", CARDTYPE=SIM-1, CTRLTYPE=REJECT, CAUSE=GPRS_SERVICE_NOT_ALLOWED;

#### [参考信息](#ZH-CN_MMLREF_0000001172345073)

- 输入的起始IMSI必须小于或者等于终止IMSI。判断起始IMSI和终止IMSI大小的原则是：对于输入IMSI的长度小于系统规定IMSI长度时，将该IMSI补足0到规定长度后进行大小比较，且起始IMSI必须小于终止IMSI。只有输入IMSI长度等于系统规定长度时，起始IMSI才能等于终止IMSI。对于系统规定IMSI长度为15的情况，如[表1](#ZH-CN_MMLREF_0000001172345073__tab1)所示：
  *表1 IMSI限定范围*
