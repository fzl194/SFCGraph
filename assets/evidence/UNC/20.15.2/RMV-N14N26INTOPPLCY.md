# 删除N14N26接口跨运营商交互策略（RMV N14N26INTOPPLCY）

- [命令功能](#ZH-CN_MMLREF_0000001875982868__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001875982868__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001875982868__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000001875982868__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000001875982868)

**适用NF：AMF**

该命令用于删除本AMF和指定运营商的AMF/MME之间的跨运营商交互策略。

## [注意事项](#ZH-CN_MMLREF_0000001875982868)

下一次移动性流程生效。

#### [操作用户权限](#ZH-CN_MMLREF_0000001875982868)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0000001875982868)

| 参数标识 | 参数名称 | 参数说明 |
| --- | --- | --- |
| PEERMCC | 对端运营商移动国家码 | 可选必选说明：必选参数<br>参数含义：该参数用于指定需要控制交互策略的对端运营商的Serving PLMN中的移动国家码。<br>数据来源：全网规划<br>取值范围：字符串类型，输入长度是3。<br>默认值：无<br>配置原则：无 |
| PEERMNC | 对端运营商移动网号 | 可选必选说明：必选参数<br>参数含义：该参数用于指定需要控制交互策略的对端运营商的Serving PLMN中的移动网号。<br>数据来源：全网规划<br>取值范围：字符串类型，输入长度范围是2~3。<br>默认值：无<br>配置原则：无 |

## [使用实例](#ZH-CN_MMLREF_0000001875982868)

删除本AMF和运营商A之间的跨运营商交互策略，执行如下命令：

```
RMV N14N26INTOPPLCY:PEERMCC="123",PEERMNC="45";
```
