...
- [命令功能](#ZH-CN_CONCEPT_0129627067__1.3.1.1)
- [注意事项](#ZH-CN_CONCEPT_0129627067__1.3.2.1)
- [操作用户权限](#ZH-CN_CONCEPT_0129627067__1.3.3.1)
- [参数说明](#ZH-CN_CONCEPT_0129627067__1.3.4.1)
- [使用实例](#ZH-CN_CONCEPT_0129627067__1.3.5.1)
- [输出结果说明](#ZH-CN_CONCEPT_0129627067__1.3.6.1)

#### [命令功能](#ZH-CN_CONCEPT_0129627067)

查询业务VNFC的某个服务实例地址信息。用于核对业务VNFC的服务实例地址、CSLB的服务实例以及VNRS的路由是否一致。 CSLB查询到的一条服务实例地址对应VNRS上查询到的一条路由。

#### [注意事项](#ZH-CN_CONCEPT_0129627067)

该命令批量下发可能导致执行超时。

#### [操作用户权限](#ZH-CN_CONCEPT_0129627067)

G_1，管理员级别命令组；G_2，操作员级别命令组；G_3，用户级别命令组

#### [参数说明](#ZH-CN_CONCEPT_0129627067)
...
#### [输出结果说明](#ZH-CN_CONCEPT_0129627067)

| 输出项名称 | 输出项解释 |
| --- | --- |
| 服务VNFC ID | 参数含义：服务VNFC ID。<br>取值范围：整数类型，取值范围为0~4294967296。 |
| 服务实例ID | 参数含义：服务实例ID。<br>取值范围：整数类型，取值范围为0~4294967296。 |
| IP类型 | 参数含义：IP地址的类型。<br>取值范围：“IPV4(IPV4) ” “IPV6(IPV6) ” |
| 业务IP类型 | 参数含义：业务IP地址的类型。<br>取值范围：1~3 |
| 目的IPV4 | 参数含义：目的IPV4地址。<br>取值范围：0.0.0.0~255.255.255.255。 |
| 目的IPV6 | 参数含义：目的IPV6地址。<br>取值范围：::~FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF。 |
| IPV4掩码 | 参数含义：目的IPV4的地址掩码。<br>取值范围：0.0.0.0~255.255.255.255。 |
| IPV6掩码 | 参数含义：目的IPV6的地址掩码。<br>取值范围：::~FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF。 |
| VPN名称 | 参数含义：VPN的名称。<br>取值范围：32个字符 |
| V4路由下一跳 | 参数含义：目的IPV4地址路由的下一跳。<br>取值范围：0.0.0.0~255.255.255.255。 |
| V6路由下一跳 | 参数含义：目的IPV6地址路由的下一跳。<br>取值范围：::~FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF。 |
| 路由发布策略 | 参数含义：路由发布的策略。路由有生产者，也有消费者，路由是否发布，是路由生产者打上的标记，打上发布标记的路由，可以供消费者使用。<br>取值范围：0为不发布，1为发布。 |
| 路由类型 | 参数含义：路由的类型。<br>取值范围：0~3。 |
| 路由优先级 | 参数含义：路由优先级，值越小优先级越高。<br>取值范围：0~255。 |
| 容灾组ID | 参数含义：容灾组ID。<br>取值范围：0~4294967295。 |
