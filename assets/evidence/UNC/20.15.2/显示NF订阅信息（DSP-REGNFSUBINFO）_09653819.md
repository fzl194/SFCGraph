# 显示NF订阅信息（DSP REGNFSUBINFO）

- [命令功能](#ZH-CN_MMLREF_0209653819__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0209653819__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0209653819__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0209653819__1.3.5)
- [输出结果说明](#ZH-CN_MMLREF_0209653819__1.3.6)

## [命令功能](#ZH-CN_MMLREF_0209653819)
...
#### [操作用户权限](#ZH-CN_MMLREF_0209653819)

G_1，管理员级别命令组；G_2，操作员级别命令组；G_3，用户级别命令组

## [参数说明](#ZH-CN_MMLREF_0209653819)
...
## [输出结果说明](#ZH-CN_MMLREF_0209653819)
...
| 订阅条件 | 该字段表示订阅消息中的订阅条件。 |
| 订阅有效期 | 该字段表示订阅消息的有效期。 |
| 通知事件类型 | 该字段表示订阅消息中的通知事件类型。 |
| 目标NF的PLMN | 该字段表示订阅消息中目标NF的PLMN。 |
| 通知条件 | 该字段表示订阅消息中的通知条件。 |
| 请求的NF类型 | 该参数表示订阅请求消息中的源NF类型。<br>取值说明：<br>- NRF（NRF）<br>- UDM（UDM）<br>- AMF（AMF）<br>- SMF（SMF）<br>- AUSF（AUSF）<br>- NEF（NEF）<br>- PCF（PCF）<br>- SMSF（SMSF）<br>- NSSF（NSSF）<br>- UDR（UDR）<br>- LMF（LMF）<br>- GMLC（GMLC）<br>- EIR_5G（EIR_5G）<br>- SEPP（SEPP）<br>- UPF（UPF）<br>- N3IWF（N3IWF）<br>- AF（AF）<br>- UDSF（UDSF）<br>- BSF（BSF）<br>- CHF（CHF）<br>- NWDAF（NWDAF）<br>- CUSTOM_OCS（CUSTOM_OCS）<br>- DRA（DRA）<br>- SCP（SCP） |
| 请求NF的FQDN | 该参数表示订阅消息中请求NF的FQDN信息。 |
| NF支持的NRF特性 | 该参数表示发起订阅请求的NF支持的NRF特性。 |
| 请求NF的PLMN列表 | 该参数表示订阅请求消息中的源NF的PLMN列表。 |
| UserAgent携带的NF类型 | 该参数用于表示NRF收到的订阅请求消息中User-Agent字段携带的NF类型。<br>取值说明：<br>- NRF（NRF）<br>- UDM（UDM）<br>- AMF（AMF）<br>- SMF（SMF）<br>- AUSF（AUSF）<br>- NEF（NEF）<br>- PCF（PCF）<br>- SMSF（SMSF）<br>- NSSF（NSSF）<br>- UDR（UDR）<br>- LMF（LMF）<br>- GMLC（GMLC）<br>- EIR_5G（EIR_5G）<br>- SEPP（SEPP）<br>- UPF（UPF）<br>- N3IWF（N3IWF）<br>- AF（AF）<br>- UDSF（UDSF）<br>- BSF（BSF）<br>- CHF（CHF）<br>- NWDAF（NWDAF）<br>- CUSTOM_OCS（CUSTOM_OCS）<br>- DRA（DRA）<br>- SCP（SCP） |
| 订阅生成的连接模式 | 该参数表示订阅记录生成时，NRF与NF之间的连接模式。<br>取值说明：<br>- “NONPLMNDIRECT（NONPLMNDIRECT）”：订阅请求属于本PLMN内请求，与对端NF直接通信，不通过SEPP。<br>- “PLMNDIRECT（PLMNDIRECT）”：订阅请求不属于本PLMN内的请求，与对端NF直接通信，不通过SEPP。<br>- “SBI_TARGET_APIROOT（SBI_TARGET_APIROOT）”：与对端NF通过SEPP交互，在http头中携带对端NF的地址信息。<br>- “HTTP_PROXY（HTTP_PROXY）”：与对端NF通过SEPP交互，在URI中携带对端NF的地址信息。 |
| 订阅来源分类 | 该参数表示被订阅的订阅来源分类。若不输入，则表示查询所有类型的记录。<br>取值说明：<br>- EXTERNAL（外部订阅记录）<br>- INNER（内部订阅记录）<br>- ALL（全部订阅记录） |
