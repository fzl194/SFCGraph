...
- [命令功能](#ZH-CN_MMLREF_0000001168320981__1.3.1)
- [注意事项](#ZH-CN_MMLREF_0000001168320981__1.3.2)
- [参数说明](#ZH-CN_MMLREF_0000001168320981__1.3.4)
- [使用实例](#ZH-CN_MMLREF_0000001168320981__1.3.5)

## [命令功能](#ZH-CN_MMLREF_0000001168320981)

![](增加IPsec Ipv6策略（ADD IPSECPOLICY6）_68320981.assets/notice_3.0-zh-cn_2.png)

该命令影响协商及加解密性能，命令中的部分配置可能是不安全的。

该命令用于增加IPsec Ipv6策略。

## [注意事项](#ZH-CN_MMLREF_0000001168320981)
...
#### [操作用户权限](#ZH-CN_MMLREF_0000001168320981)

G_1，管理员级别命令组；G_2，操作员级别命令组

## [参数说明](#ZH-CN_MMLREF_0000001168320981)
...
| TRAFFSADISFLG | 去使能SA以流量计长 | 可选必选说明：可选参数<br>参数含义：流量方式触发SA过期。<br>数据来源：本端规划<br>取值范围：<br>- FALSE（FALSE）<br>- TRUE（TRUE）<br>默认值：FALSE<br>配置原则：<br>大流量环境该值建议设置为TRUE。 |
...
| ANTIREPLAYENUM | 抗重放 | 可选必选说明：可选参数<br>参数含义：抗重放。<br>数据来源：本端规划<br>取值范围：<br>- “NotConfigured（未配置）”：未配置<br>- “Enable（使能）”：使能<br>- “Disable（未使能）”：未使能<br>默认值：NotConfigured<br>配置原则：无 |
...
| DFBITCLEAR | 清除分片标记 | 可选必选说明：可选参数<br>参数含义：清除分片标记。<br>数据来源：本端规划<br>取值范围：<br>- TRUE(TRUE)<br>- FALSE(FALSE)<br>默认值：FALSE<br>配置原则：无 |
| FRAGBEFOREENCR | 加密前分片报文 | 可选必选说明：可选参数<br>参数含义：加密前分片报文。<br>数据来源：本端规划<br>取值范围：<br>- TRUE(TRUE)<br>- FALSE(FALSE)<br>默认值：FALSE<br>配置原则：无 |
...
| LOGENABLE | 日志使能 | 可选必选说明：可选参数<br>参数含义：日志使能。<br>数据来源：本端规划<br>取值范围：<br>- TRUE(TRUE)<br>- FALSE(FALSE)<br>默认值：FALSE<br>配置原则：无 |
| WORKMODE | 工作模式 | 可选必选说明：可选参数<br>参数含义：工作模式，如轮循模式、主备模式。<br>数据来源：本端规划<br>取值范围：<br>- “Round_robin（轮询）”：轮询工作模式<br>- “Master_standby（主备）”：主备隧道工作模式<br>默认值：Round_robin<br>配置原则：<br>配置为轮循模式或者主备模式时，SA协商方式为自动协商。推荐使用主备工作模式。 |
...
| ESN | 扩展序列号 | 可选必选说明：可选参数<br>参数含义：该参数用于表示扩展序列号是否使能。<br>数据来源：本端规划<br>取值范围：<br>- “Disable（未使能）”：未使能<br>- “Enable（使能）”：使能<br>默认值：Disable<br>配置原则：<br>该参数必须在“ANTIREPLAYENUM”配置为“Enable”时才可配置为"Enable"。只有该参数配置为“Disable”时，“ANTIREPLAYENUM”才可配置为“Disable”或“NotConfigured”。 |
| TFCENABLE | 数据流可信使能 | 可选必选说明：可选参数<br>参数含义：数据流可信使能。<br>数据来源：本端规划<br>取值范围：<br>- TRUE(TRUE)<br>- FALSE(FALSE)<br>默认值：FALSE<br>配置原则：无 |

## [使用实例](#ZH-CN_MMLREF_0000001168320981)

增加名称为“pol2”的IPsec策略：

```
ADD IPSECPOLICY6:POLICYNAME="pol2",SEQUENCENUMBER=1,POLICYMODE=Isakmp,TEMPLATEMODE=None,DSCPINSELECT=EnterDSCPCode,DSCPINBOUNDVAL=1,DSCPOUTSELECT=EnterDSCPCode,DSCPOUTBOUNDVAL=1,ACLTYPE=AclIPv6;
```
