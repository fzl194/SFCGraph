---
id: UDG@20.15.2@MMLCommand@ADD GLBTRUNKREMARK
type: MMLCommand
name: ADD GLBTRUNKREMARK（添加整机Trunk Remark配置）
nf: UDG
version: 20.15.2
verb: ADD
...
# ADD GLBTRUNKREMARK（添加整机Trunk Remark配置）

## 功能

**适用NF：SGW-U、PGW-U**

该命令用于添加整机的Trunk Remark配置。

## 注意事项

- 该命令执行后立即生效。
- 系统最多支持配置500条GLBTRUNKREMARK。
...
- [[configobject/UDG/20.15.2/GLBTRUNKREMARK]] · 整机Trunk Remark配置（GLBTRUNKREMARK）

## 使用实例

添加一条整机的Trunk Remark配置。设置QCI为1，ARP优先级为1，Remark方式为TOS，TOS值为3：

```
ADD GLBTRUNKREMARK: QCI=1, ARPPL=1, REMARKTYPE=TOS, TOSVALUE=3;
```

## 证据

- 原始手册：`evidence/UDG/20.15.2/ADD-GLBTRUNKREMARK.md`
