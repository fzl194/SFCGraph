---
id: UDG@20.15.2@MMLCommand@DSP TCPSTATUS
type: MMLCommand
name: DSP TCPSTATUS（查询TCP状态）
nf: UDG
version: 20.15.2
verb: DSP
object_keyword: TCPSTATUS
...
| 参数标识 | 参数名称 | 参数说明 |
| --- | --- | --- |
| IPVERSION | IP版本 | 可选必选说明：可选参数<br>参数含义：该参数用于指定TCP socket实例的IP版本。<br>数据来源：本端规划<br>取值范围：枚举类型。<br>- IPv4：IPv4协议族。<br>- IPv6：IPv6协议族。<br>默认值：IPv4 |
...
- [TCP状态（TCPSTATUS）](configobject/UDG/20.15.2/TCPSTATUS.md)

## 使用实例

- 显示当前系统IPv4协议下的TCP状态信息：
  ```
  DSP TCPSTATUS: APPCID="0x8093042C", SOCKETFD=1;
  ```
  ```
      ...
      ---    END
  ```
- 显示当前系统IPv6协议下的TCP状态信息：
  ```
  DSP TCPSTATUS:IPVERSION=IPv6,APPCID="0x80130080",SOCKETFD=1;
  ```
  ```
...
- 原始手册：`evidence/UDG/20.15.2/查询TCP状态（DSP-TCPSTATUS）_50121074.md`
