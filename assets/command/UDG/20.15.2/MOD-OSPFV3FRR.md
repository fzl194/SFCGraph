---
id: UDG@20.15.2@MMLCommand@MOD OSPFV3FRR
type: MMLCommand
name: MOD OSPFV3FRR（修改OSPFv3 IP FRR配置）
nf: UDG
version: 20.15.2
verb: MOD
...
# MOD OSPFV3FRR（修改OSPFv3 IP FRR配置）

## 功能

该命令用于修改OSPFv3 IP FRR（快速重路由）配置。
...
| ISLFAENABLE | LFA是否使能 | 可选必选说明：可选参数<br>参数含义：LFA是否使能。<br>数据来源：本端规划<br>取值范围：枚举类型。<br>- FALSE：不使能。<br>- TRUE：使能。<br>默认值：无<br>配置原则：使能此参数后，OSPFv3 IP FRR功能才会生效，生成无环备份路由。 |
...
修改OSPFv3进程号为1的IP FRR（快速重路由）配置，去使能LFA：

```
MOD OSPFV3FRR: PROCID=1, ISLFAENABLE=FALSE;
```

## 证据

- 原始手册：`evidence/UDG/20.15.2/修改OSPFv3-IP-FRR配置（MOD-OSPFV3FRR）_49961986.md`
