---
id: UDG@20.15.2@MMLCommand@ADD OSPFDNSUMRTID
type: MMLCommand
name: ADD OSPFDNSUMRTID（创建特定路由器禁止检查Summary-LSA的DN比特位配置）
nf: UDG
version: 20.15.2
verb: ADD
object_keyword: OSPFDNSUMRTID
command_category: 配置类
effect_mode: 立即生效
is_dangerous: false
max_records: 48000
category_path:
- 平台服务管理
- VNRS功能管理
- IP服务
- 路由管理
- OSPF管理
- OSPF特定路由器禁止检查Summary-LSA的DN比特位配置
status: active
---

# ADD OSPFDNSUMRTID（创建特定路由器禁止检查Summary-LSA的DN比特位配置）

## 功能

该命令用于配置特定路由器禁止对Summary LSA的DN位检查。

## 注意事项

- 该命令执行后立即生效。
- 该命令最大记录数为48000。
- 只有在执行ADD OSPF配置了OSPF进程后才能使用此命令。
- 仅支持在OSPF私网进程下配置，并且只在PE上生效。

## 权限

G_1，管理员级别命令组；G_2，操作员级别命令组

## 参数

| 参数标识 | 参数名称 | 参数说明 |
| --- | --- | --- |
| PROCID | OSPF进程号 | 可选必选说明：必选参数<br>参数含义：OSPF进程号。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围为1～4294967295。<br>默认值：无<br>配置原则：OSPF进程必须已经存在。请使用LST OSPF命令查看可用的OSPF进程。 |
| ROUTERID | 路由器标识 | 可选必选说明：必选参数<br>参数含义：路由器标识。<br>数据来源：全网规划<br>取值范围：IPv4地址类型。点分十进制格式。<br>默认值：无 |

## 操作的配置对象

- [创建特定路由器禁止检查Summary-LSA的DN比特位配置（OSPFDNSUMRTID）](configobject/UDG/20.15.2/OSPFDNSUMRTID.md)

## 使用实例

配置10.1.1.1的路由器禁止对Summary LSA的DN位检查：

```
ADD OSPFDNSUMRTID:PROCID=1,ROUTERID="10.1.1.1";
```

## 证据

- 原始手册：`evidence/UDG/20.15.2/创建特定路由器禁止检查Summary-LSA的DN比特位配置（ADD-OSPFDNSUMRTID）_49802630.md`
