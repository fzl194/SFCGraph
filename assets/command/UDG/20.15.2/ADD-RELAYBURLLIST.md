---
id: UDG@20.15.2@MMLCommand@ADD RELAYBURLLIST
type: MMLCommand
name: ADD RELAYBURLLIST（增加媒体中继黑名单URL列表）
nf: UDG
version: 20.15.2
verb: ADD
...
# ADD RELAYBURLLIST（增加媒体中继黑名单URL列表）
...
| URL | URL | 可选必选说明：必选参数<br>参数含义：该参数用于设定黑名单URL。<br>数据来源：本端规划<br>取值范围：字符串类型，输入长度范围为1～119。转义后的长度不超过127。<br>默认值：无<br>配置原则：无 |
...
```
ADD RELAYBURLLIST: RELAYURLLISTNM="list01", RELAYURLNAME="url01",URL="www.huawei.com",PRIORITY=1;
```

## 证据

- 原始手册：`evidence/UDG/20.15.2/ADD-RELAYBURLLIST.md`
