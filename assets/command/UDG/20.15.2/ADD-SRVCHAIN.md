---
id: UDG@20.15.2@MMLCommand@ADD SRVCHAIN
type: MMLCommand
name: ADD SRVCHAIN（添加业务链）
nf: UDG
version: 20.15.2
verb: ADD
...
该命令用于新增业务链配置，配置上下行业务链ID，为Rule提供业务链转发策略。
...
- [[UDG@20.15.2@ConfigObject@SRVCHAIN]] · 业务链（SRVCHAIN）

## 使用实例

如果运营商想要配置业务链名称为TestSrvChain，上行业务链ID为101，下行业务链ID为201的流量分类策略。命令如下：

```
ADD SRVCHAIN:SRVCHAINNAME="TestSrvChain",UPSRVCHAINID=101,DNSRVCHAINID=201;
```

## 证据

- 原始手册：`evidence/UDG/20.15.2/ADD-SRVCHAIN.md`
