---
id: UDG@20.15.2@MMLCommand@MOD RELAYBURLLIST
type: MMLCommand
name: MOD RELAYBURLLIST（修改媒体中继黑名单URL列表）
nf: UDG
version: 20.15.2
verb: MOD
object_keyword: RELAYBURLLIST
...
# MOD RELAYBURLLIST（修改媒体中继黑名单URL列表）
...
- 对于报文携带Referer头域值为相对路径的场景，需要配置黑名单URL格式为*/Path，其中Path部分可匹配报文携带的Referer头域值。
...
| URL | URL | 可选必选说明：可选参数<br>参数含义：该参数用于设定黑名单URL。<br>数据来源：本端规划<br>取值范围：字符串类型，输入长度范围为1～119。转义后的长度不超过127。<br>默认值：无<br>配置原则：无 |
| PRIORITY | 优先级 | 可选必选说明：可选参数<br>参数含义：该参数用于设置优先级。<br>数据来源：本端规划<br>取值范围：整数类型。取值范围为1～65535。值越小优先级越高。<br>默认值：无<br>配置原则：不允许重复，以免同时匹配中多个。 |
...
- [媒体中继黑名单URL列表（RELAYBURLLIST）](configobject/UDG/20.15.2/RELAYBURLLIST.md)
...
```
MOD RELAYBURLLIST: RELAYURLLISTNM="list01", RELAYURLNAME="url01",URL="www.huawei.com",PRIORITY=1;
```
