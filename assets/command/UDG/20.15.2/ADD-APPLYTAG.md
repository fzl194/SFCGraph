---
id: UDG@20.15.2@MMLCommand@ADD APPLYTAG
type: MMLCommand
name: ADD APPLYTAG（增加路由标签值设置）
nf: UDG
version: 20.15.2
verb: ADD
...
# ADD APPLYTAG（增加路由标签值设置）

## 功能

该命令用于添加路由标签设置，路由标签可将路由按实际需求分类，同类路由打上相同的Tag，在路由策略中根据Tag对路由进行灵活的控制和管理。
...
- [路由标签值设置（APPLYTAG）](configobject/UDG/20.15.2/APPLYTAG.md)

## 使用实例

路由策略a节点10，添加设置路由标签值10：

```
ADD APPLYTAG:POLICYNAME="a",NODESEQUENCE=10,TAGVALUE=10;
```

## 证据

- 原始手册：`evidence/UDG/20.15.2/增加路由标签值设置（ADD-APPLYTAG）_00841529.md`
