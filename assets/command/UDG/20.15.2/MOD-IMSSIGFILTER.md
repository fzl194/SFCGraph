---
id: UDG@20.15.2@MMLCommand@MOD IMSSIGFILTER
type: MMLCommand
name: MOD IMSSIGFILTER（修改IMS分类器）
nf: UDG
version: 20.15.2
verb: MOD
...
# MOD IMSSIGFILTER（修改IMS分类器）

## 功能

**适用NF：PGW-U、UPF**

![](修改IMS分类器（MOD IMSSIGFILTER）_82837818.assets/notice_3.0-zh-cn.png)

本命令属于高危命令，请确认修改规则后合法的IMS信令报文都能命中其中一条规则。

该命令用于修改系统整机的IMS信令专用上下文的分类器配置。当运营商规划IMS网络时，使用该命令修改IMS专用上下文的分类器。IMS网络可以为用户提供丰富多彩的基于Internet应用统一的多媒体业务和应用，如语音业务、实时多媒体业务和视频会议等。

## 注意事项

- 该命令执行后只对新激活用户生效。
- 当APN下没有配置ApnImsSigFltr规则时，该配置作为缺省配置。缺省规则需要保证所有合法IMS业务能够命中，配置修改不当会造成语音不通。
...
| IPVERSION | IP地址版本 | 可选必选说明：可选参数<br>参数含义：该参数用于指定过滤规则的地址类型。<br>数据来源：本端规划<br>取值范围：枚举类型。<br>- IPV4：IPv4地址类型。<br>- IPV6：IPv6地址类型。<br>默认值：无<br>配置原则：无 |
...
| PROTOCOL | 协议类型 | 可选必选说明：条件必选参数<br>前提条件：该参数在“PROSPECTYPE”配置为“SPECIFY”时为必选参数。<br>参数含义：该参数用于设置协议类型。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围为1～255。<br>默认值：无<br>配置原则：当指定了端口的时候，取值只能是6或者17。 |

## 操作的配置对象

- [[UDG@20.15.2@ConfigObject@IMSSIGFILTER]] · IMS分类器（IMSSIGFILTER）

## 使用实例

当运营商规划使用IMS网络，需要修改IPV4地址类型的IMS分配器信息，举例：

```
MOD IMSSIGFILTER:PRIORITY=5,IPVERSION=IPV4,SRCIPV4SPECTYPE=SPECIFY,SRCIPV4ADDR="10.11.11.11",SRCIPV4MASKLEN=32;
```

## 证据

- 原始手册：`evidence/UDG/20.15.2/MOD-IMSSIGFILTER.md`
