---
id: UDG@20.15.2@MMLCommand@ADD SFIPSRVIP
type: MMLCommand
name: ADD SFIPSRVIP（添加SFIP业务IP）
nf: UDG
version: 20.15.2
verb: ADD
...
| IPVERSION | IP版本 | 可选必选说明：必选参数<br>参数含义：该参数用于指定SFIP业务IP的版本信息。<br>数据来源：本端规划<br>取值范围：<br>- IPV4：表示地址类型为IPv4。<br>- IPV6：表示地址类型为IPv6。<br>- IPV4_IPV6：同时支持IPv4和IPv6地址。<br>默认值：无<br>配置原则：无 |
...
- [[UDG@20.15.2@ConfigObject@SFIPSRVIP]] · SFIP业务IP（SFIPSRVIP）

## 使用实例

添加SFIP业务IP地址，业务类型名称为dns，IP地址类型为IPv4，IPv4地址为192.168.1.1：

```
ADD SFIPSRVIP: SERVICETYPENAME="dns", IPVERSION=IPV4, IPV4ADDR="192.168.1.1";
```

## 证据

- 原始手册：`evidence/UDG/20.15.2/ADD-SFIPSRVIP.md`
