---
id: UDG@20.15.2@MMLCommand@MOD RELAYWURLLIST
type: MMLCommand
name: MOD RELAYWURLLIST（修改媒体中继白名单URL列表）
nf: UDG
version: 20.15.2
verb: MOD
...
# MOD RELAYWURLLIST（修改媒体中继白名单URL列表）
...
| URL | URL | 可选必选说明：可选参数<br>参数含义：该参数用于设定白名单URL。<br>数据来源：本端规划<br>取值范围：字符串类型，输入长度范围为1～119。转义后的长度不超过127。<br>默认值：无<br>配置原则：无 |
...
- [[configobject/UDG/20.15.2/RELAYWURLLIST]] · 媒体中继白名单URL列表（RELAYWURLLIST）
...
```
MOD RELAYWURLLIST: RELAYURLLISTNM="list01", RELAYURLNAME="url01",URL="www.huawei.com",PRIORITY=1;
```

## 证据

- 原始手册：`evidence/UDG/20.15.2/修改媒体中继白名单URL列表（MOD-RELAYWURLLIST）_43992614.md`
