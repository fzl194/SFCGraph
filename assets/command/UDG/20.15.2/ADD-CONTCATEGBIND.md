---
id: UDG@20.15.2@MMLCommand@ADD CONTCATEGBIND
type: MMLCommand
name: ADD CONTCATEGBIND（增加内容分类组绑定关系）
nf: UDG
version: 20.15.2
verb: ADD
...
| ACTION | 动作 | 可选必选说明：可选参数<br>参数含义：该参数用于设置动作。<br>数据来源：本端规划<br>取值范围：<br>- PASS：报文转发。<br>- BLOCK：报文丢弃。<br>- REDIRECT：报文重定向。<br>默认值：PASS<br>配置原则：无 |
...
- [[UDG@20.15.2@ConfigObject@CONTCATEGBIND]] · 内容分类组绑定关系（CONTCATEGBIND）
...
假如运营商希望在名为“cf_profile1”的内容过滤策略中，绑定名为“cf_contcategrprange1”的内容分类组，并且设定使用该内容过滤策略的用户，其报文若匹配上该内容分类组，则动作为BLOCK：

```
ADD CONTCATEGBIND: CFPROFILENAME="cf_profile1", CONTCATEGNAME="cf_contcategrprange1", ACTION=BLOCK;
```
