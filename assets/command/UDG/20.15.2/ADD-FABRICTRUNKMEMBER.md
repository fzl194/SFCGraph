---
id: UDG@20.15.2@MMLCommand@ADD FABRICTRUNKMEMBER
type: MMLCommand
name: ADD FABRICTRUNKMEMBER（增加Fabric-Trunk成员接口）
nf: UDG
version: 20.15.2
verb: ADD
...
# ADD FABRICTRUNKMEMBER（增加Fabric-Trunk成员接口）

## 功能

该命令用于增加NP卡Fabric-Trunk成员接口。在非NP卡基础上扩容NP卡的异构场景下，将NP卡中的多个物理接口作为一个逻辑接口使用。
...
- [[UDG@20.15.2@ConfigObject@FABRICTRUNKMEMBER]] · Fabric-Trunk成员接口（FABRICTRUNKMEMBER）

## 使用实例

增加一个Fabric-Trunk成员接口，该接口是1号框，8号槽位中的3号端口：

```
%%ADD FABRICTRUNKMEMBER: SUBRACK=Subrack_1, SLOTID=Slot_8, PORTID=P3;%%
RETCODE = 0  操作成功
---    END
```
...
- 原始手册：`evidence/UDG/20.15.2/ADD-FABRICTRUNKMEMBER.md`
