---
id: UNC@20.15.2@MMLCommand@RMV GTPCINTFATTR
type: MMLCommand
name: RMV GTPCINTFATTR（删除GTP-C IP地址接口属性）
nf: UNC
version: 20.15.2
verb: RMV
...
# RMV GTPCINTFATTR（删除GTP-C IP地址接口属性）

## 功能

**适用网元：SGSN、MME、AMF**

该命令用于删除指定GTPC IP地址在特定范围内的使用配置。

删除特定接口或者特定用户范围的GTPC IP地址后，该范围内用户的业务流程将使用 [**ADD GTPCLE**](../Gtpc本端实体管理/增加GTP-C本地实体(ADD GTPCLE)_26145966.md) 配置的0号组中的GTPC IP地址。

## 注意事项

- 该命令执行后立即生效。
- {“所有接口”+“所有用户”}为系统初始记录，不支持删除。
- 删除特定接口或者特定用户范围的GTPC IP地址后，如果在线用户中已经使用了对应GTPC IP地址，并且该GTPC IP地址也不在[**ADD GTPCLE**](../Gtpc本端实体管理/增加GTP-C本地实体(ADD GTPCLE)_26145966.md)配置的0号组中存在，在线用户需要重新激活才能进行业务。
...
| INDEX | 记录索引 | 可选必选说明：条件必选参数<br>参数含义：该参数用于指定本配置记录的索引。<br>前提条件：该参数在"删除条件"参数配置为"BYINDEX(根据记录索引)"后生效。<br>数据来源：本端规划<br>取值范围：0-4294967295<br>默认值：无<br>配置原则：无。<br>说明：使用<br>[**LST GTPCINTFATTR**](查询GTP-C IP地址接口属性(LST GTPCINTFATTR)_26145902.md)<br>查询记录的索引。 |
| SUBRANGE | 用户范围 | 可选必选说明：条件必选参数。<br>参数含义：该参数用于指定GTPC IP地址适用的用户范围。<br>前提条件：该参数在“删除条件”参数配置为“BYINTFSUB(根据接口类型和用户范围)”后生效。<br>数据来源：全网规划<br>取值范围：<br>- “ALL_USER(所有用户)”<br>- “SPECIAL_IMSIPRE(指定IMSI前缀)”<br>- “SPECIAL_NOID(指定运营商)”<br>默认值：无 |
...
- [[UNC@20.15.2@ConfigObject@GTPCINTFATTR]] · GTP-C IP地址接口属性（GTPCINTFATTR）

## 使用实例

将删除类型为BYINDEX、INDEX为1的记录删除：

RMV GTPCINTFATTR: DELTYPE=BYINDEX,INDEX=1;

## 证据

- 原始手册：`evidence/UNC/20.15.2/RMV-GTPCINTFATTR.md`
