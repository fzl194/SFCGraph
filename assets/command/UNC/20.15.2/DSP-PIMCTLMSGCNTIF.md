---
id: UNC@20.15.2@MMLCommand@DSP PIMCTLMSGCNTIF
type: MMLCommand
name: DSP PIMCTLMSGCNTIF（查询PIM接口报文统计计数）
nf: UNC
version: 20.15.2
verb: DSP
object_keyword: PIMCTLMSGCNTIF
command_category: 查询类
effect_mode: ''
is_dangerous: false
category_path:
- 平台服务管理
- VNRS功能管理
- IP服务
- IP组播
- PIM配置
- PIM报文统计计数
status: active
---

# DSP PIMCTLMSGCNTIF（查询PIM接口报文统计计数）

## 功能

该命令用于显示PIM接口报文统计计数。

## 注意事项

该命令仅适用于非NP卡场景和NP卡非加速模式场景。

## 权限

G_1，管理员级别命令组；G_2，操作员级别命令组；G_3，用户级别命令组

## 参数

| 参数标识 | 参数名称 | 参数说明 |
| --- | --- | --- |
| ADDRESSFAMILY | 地址族 | 可选必选说明：必选参数<br>参数含义：该参数用于表示地址族类型。<br>数据来源：本端规划<br>取值范围：枚举类型。<br>- ipv4unicast：IPv4单播。<br>默认值：无 |
| VRFNAME | VPN实例名称 | 可选必选说明：可选参数<br>参数含义：该参数用于表示VPN实例名称。<br>数据来源：本端规划<br>取值范围：字符串类型，输入长度范围为1～31。<br>默认值：_public_<br>配置原则：通过LST L3VPNINST查看当前已存在的VPN实例。 |
| IFNAME | 接口名称 | 可选必选说明：可选参数<br>参数含义：该参数用于表示接口名称。<br>数据来源：本端规划<br>取值范围：字符串类型，输入长度范围为1～63。<br>默认值：无<br>配置原则：请使用LST INTERFACE命令查看可用接口。 |

## 操作的配置对象

- [[UNC@20.15.2@ConfigObject@PIMCTLMSGCNTIF]] · PIM接口报文统计计数（PIMCTLMSGCNTIF）

## 使用实例

显示PIM接口报文统计计数：

```
DSP PIMCTLMSGCNTIF:VRFNAME="_public_",ADDRESSFAMILY=ipv4unicast,IFNAME="Ethernet64/0/3";
```

```
RETCODE = 0  操作成功。

结果如下
--------
       VPN实例名称  =  _public_
            地址族  =  IPv4单播
          接口名称  =  Ethernet64/0/3
收到Assert报文数目  =  0
发送Assert报文数目  =  0
无效Assert报文数目  =  0
过滤Assert报文数目  =  0
 收到Hello报文数目  =  0
 发送Hello报文数目  =  0
 无效Hello报文数目  =  0
 过滤Hello报文数目  =  0
    收到JP报文数目  =  0
    发送JP报文数目  =  0
    无效JP报文数目  =  0
    过滤JP报文数目  =  0
   收到BSR报文数目  =  0
   发送BSR报文数目  =  0
   无效BSR报文数目  =  0
   过滤BSR报文数目  =  0
  收到Pass报文数目  =  0
  发送Pass报文数目  =  0
  无效Pass报文数目  =  0
  过滤Pass报文数目  =  0
  收到宣告报文数目  =  0
  发送宣告报文数目  =  0
  无效宣告报文数目  =  0
  过滤宣告报文数目  =  0
  收到发现报文数目  =  0
  发送发现报文数目  =  0
  无效发现报文数目  =  0
  过滤发现报文数目  =  0
(结果个数 = 1)
---    END
```

## 证据

- 原始手册：`evidence/UNC/20.15.2/DSP-PIMCTLMSGCNTIF.md`
