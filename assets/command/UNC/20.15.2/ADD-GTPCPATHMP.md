---
id: UNC@20.15.2@MMLCommand@ADD GTPCPATHMP
type: MMLCommand
name: ADD GTPCPATHMP（增加GTP-C路径管理策略）
nf: UNC
version: 20.15.2
verb: ADD
...
- 最多可输入1024条记录。
- 如果GTP-C路径信息和该命令配置一样，则按照该配置进行路径状态管理；如果不一致，则Echo请求消息发送开关、Echo请求发送间隔、Echo请求重发间隔以及Echo请求发送次数参考SET GTPCFUNCPARA命令配置。
...
| ECHOSW | Echo请求发送开关 | 可选必选说明：可选参数<br>参数含义：该参数用于指定GTP-C路径是否发送Echo请求消息，探测对方通信是否正常。<br>数据来源：本端规划<br>取值范围：<br>- ON（ON）<br>- OFF（OFF）<br>默认值：OFF<br>配置原则：无 |
| EI | Echo请求发送间隔 | 可选必选说明：该参数在"ECHOSW"配置为"ON"时为条件可选参数。<br>参数含义：该参数用于指定GTP-C路径发送Echo请求消息的时间间隔。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围是60~3600，单位是秒。<br>默认值：60<br>配置原则：无 |
| T3 | Echo请求重发时间间隔 | 可选必选说明：该参数在"ECHOSW"配置为"ON"时为条件可选参数。<br>参数含义：该参数用于指定等待一条Echo响应消息的最大时长，超出时长后重发本Echo请求消息。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围是1~20，单位是秒。<br>默认值：3<br>配置原则：<br>T3与N3的乘积应该小于EI。 |
| N3 | Echo请求最大发送次数 | 可选必选说明：该参数在"ECHOSW"配置为"ON"时为条件可选参数。<br>参数含义：该参数用于指定发送Echo请求消息的最大尝试次数。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围是1~6。<br>默认值：5<br>配置原则：<br>T3与N3的乘积应该小于EI。 |
| ALMTHRESH | 告警门限 | 可选必选说明：该参数在"ECHOSW"配置为"ON"时为条件可选参数。<br>参数含义：该参数用于控制连续多少次未收到Echo请求消息的响应才会上报GTP-C路径故障告警。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围是1~20。<br>默认值：1<br>配置原则：无 |
| DESC | 描述信息 | 可选必选说明：可选参数<br>参数含义：该参数用于指定本GTP-C属性记录的描述信息。<br>数据来源：本端规划<br>取值范围：字符串类型，输入长度范围是0~32。<br>默认值：无<br>配置原则：无 |

## 操作的配置对象

- [[UNC@20.15.2@ConfigObject@GTPCPATHMP]] · GTP-C路径管理策略（GTPCPATHMP）

## 使用实例

增加GTP-C路径管理策略，设置到对端IPv4地址为10.1.12.3，掩码为255.255.255.255的GTP-C路径不进行Echo探测：

```
ADD GTPCPATHMP:RANGE=E_SPECI_IP,IPTYPE=IPv4,IPV4ADDR="10.1.12.3",IPV4MASK="255.255.255.255",ECHOSW=OFF;
```

## 证据

- 原始手册：`evidence/UNC/20.15.2/ADD-GTPCPATHMP.md`
