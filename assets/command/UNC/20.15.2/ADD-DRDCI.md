---
id: UNC@20.15.2@MMLCommand@ADD DRDCI
type: MMLCommand
name: ADD DRDCI（增加DC间通信通道信息）
nf: UNC
version: 20.15.2
verb: ADD
...
# ADD DRDCI（增加DC间通信通道信息）

## 功能

![](增加DC间通信通道信息（ADD DRDCI）_23554762.assets/notice_3.0-zh-cn_2.png)

该命令为增加容灾控制通道信息，可能导致业务中断，需要按照激活主备容灾指导书进行操作，请勿自行执行该命令。
...
| DATABKMODE | 数据备份模式 | 可选必选说明：可选参数<br>参数含义：该参数用于配置数据备份模式。<br>数据来源：全网规划<br>取值范围：<br>- Active-Standby（主备（冷备）容灾模式，无数据备份）<br>- Active-Standby_Hot_Backup（主备（热备）容灾模式，有数据备份）<br>默认值：Active-Standby<br>配置原则：<br>参数为预留参数，当前无实际作用。 |
| HBINTERVAL | 心跳探测时长(100ms) | 可选必选说明：可选参数<br>参数含义：该参数用于配置容灾控制通道的心跳探测时长，单位为100ms。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围是1~100。<br>默认值：10<br>配置原则：无 |
| HBTIMES | 心跳重传次数 | 可选必选说明：可选参数<br>参数含义：该参数用于配置容灾控制通道的心跳重传次数。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围是1~30。<br>默认值：6<br>配置原则：无 |

## 操作的配置对象

- [DC间通信通道信息（DRDCI）](configobject/UNC/20.15.2/DRDCI.md)
...
```
%%ADD DRDCI: DRGROUPID=1, IPVERSION=IPV4, LDCIP="172.16.3.4", PDCIP="172.16.5.6", VPNINSTANCE="12345", DATABKMODE=Active-Standby;%%
RETCODE = 0  操作成功
...
- 原始手册：`evidence/UNC/20.15.2/增加DC间通信通道信息（ADD-DRDCI）_23554762.md`
