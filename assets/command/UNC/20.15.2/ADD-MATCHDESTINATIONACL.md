---
id: UNC@20.15.2@MMLCommand@ADD MATCHDESTINATIONACL
type: MMLCommand
name: ADD MATCHDESTINATIONACL（增加目的IP地址基础ACL匹配路由策略）
nf: UNC
version: 20.15.2
verb: ADD
...
# ADD MATCHDESTINATIONACL（增加目的IP地址基础ACL匹配路由策略）
...
- 配置该命令前，必须已经通过ADD ACLRULEBAS4或ADD ACLRULEADV4配置了ACL过滤器，对于命名型ACL，配置过滤规则时，只有指定的源地址范围对配置规则有效。
...
- [目的IP地址基础ACL匹配路由策略（MATCHDESTINATIONACL）](configobject/UNC/20.15.2/MATCHDESTINATIONACL.md)

## 使用实例

添加路由策略a节点10的匹配条件，ACL过滤器abc匹配IPv4目的地址：

```
ADD MATCHDESTINATIONACL:POLICYNAME="a",NODESEQUENCE=10,ACLNAMEORNUM="abc";
```
