---
id: UNC@20.15.2@MMLCommand@ADD APNPAGINGPLCY
type: MMLCommand
name: ADD APNPAGINGPLCY（增加APN寻呼策略参数配置）
nf: UNC
version: 20.15.2
verb: ADD
...
# ADD APNPAGINGPLCY（增加APN寻呼策略参数配置）
...
| PAGINGPRIORITY | 寻呼优先级 | 可选必选说明：可选参数<br>参数含义：该参数用于指定S1接口Paging消息中的寻呼先级。<br>数据来源：整网规划<br>取值范围：0~7<br>默认值：7<br>配置原则：优先级数值越小，代表优先级越高。0～7优先级逐渐降低。 |

## 操作的配置对象

- [APN寻呼策略参数配置（APNPAGINGPLCY）](configobject/UNC/20.15.2/APNPAGINGPLCY.md)

## 使用实例

配置PTT业务的APN NI为“huawei.com”的用户寻呼优先级为3，使PTT业务在系统拥塞时，可以被优先处理：

ADD APNPAGINGPLCY: APNNI="huawei.com", PAGINGPRIORITY=3;
