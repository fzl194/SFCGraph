---
id: UNC@20.15.2@MMLCommand@INNERPING
type: MMLCommand
name: INNERPING（INNER IP Ping）
nf: UNC
version: 20.15.2
verb: INNERPING
object_keyword: ''
...
- 当前存在三类PING调测命令：**[PING](../../../../../VNRS功能管理/系统管理/系统维护/Ping和Tracert/Ping/IP Ping（PING）_16499685.md)**、**[INNERPING](INNER IP Ping（INNERPING）_65890802.md)**、**[NGPING](../../../../../IP管理/IP维护/NGPING（NGPING）_09587930.md)**，差异如下。
    ...
    - **INNERPING、NGPING**：该命令用于排查本网元**业务IP**与对端设备之间是否可以PING通。INNERPING适用场景如下表所示，其他场景均使用NGPING。
      *表1 INNERPING适用场景*
      ...
      | NF形态 | 接口 | 业务IP配置方式 | “服务实例”参数取值 |
      | --- | --- | --- | --- |
      | **SGSN/MME** | Ga、Gn-U、S11-U | **ADD SERVICEIP** | USN_VNFC_999 |
      | **SGSN/MME** | Gb | **ADD SERVICEIP** | GB_VNFC_999 |
      | **SGSN/MME** | Sdup | **ADD SERVICEIPMCR** | MCR_VNFC_999 |
      | **SGSN/MME** | 其他：如Gn、Sv、S10、S11等 | **ADD SERVICEIP** | LINK_VNFC_999 |
      | **AMF** | N2 | **ADD SERVICEIP** | LINK_VNFC_999 |
      | **AMF** | DNS接口 | **ADD SERVICEIP** | LINK_VNFC_999 |
      | **SMSF** | MAP接口 | **ADD SERVICEIP** | LINK_VNFC_999 |
      | **CHF** | Ga（内部）Server | **ADD IPRESOURCE** | CG_VNFC_999 |
      | **CHF** | BI | **ADD IPRESOURCE** | CG_VNFC_999 |
...
- 原始手册：`evidence/UNC/20.15.2/INNER-IP-Ping（INNERPING）_65890802.md`
