---
id: UNC@20.15.2@MMLCommand@MOD PDPNUMBERARP
type: MMLCommand
name: MOD PDPNUMBERARP（修改基于PDP数的ARP控制）
nf: UNC
version: 20.15.2
verb: MOD
...
# MOD PDPNUMBERARP（修改基于PDP数的ARP控制）

## 功能

**适用NF：SGW-C、PGW-C、GGSN**

当需要修改指定用户级别和业务级别对应的拒绝、恢复告警门限时，使用此命令。
...
- [基于PDP数的ARP控制（PDPNUMBERARP）](configobject/UNC/20.15.2/PDPNUMBERARP.md)

## 使用实例

当运营商需要修改不同的用户级别，业务级别的拒绝告警门限和恢复告警门限信息时，使用如下命令：

```
MOD PDPNUMBERARP: QOSPROFILENAME="test", USERPRIORITY=HIGH, SERVICELEVEL=GENERAL, REJECTLIMIT=60, RESTORELIMIT=30;
```
