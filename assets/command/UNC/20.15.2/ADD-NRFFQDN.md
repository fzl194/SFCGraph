---
id: UNC@20.15.2@MMLCommand@ADD NRFFQDN
type: MMLCommand
name: ADD NRFFQDN（增加NRF的FQDN）
nf: UNC
version: 20.15.2
verb: ADD
...
| FQDN | FQDN | 可选必选说明：必选参数<br>参数含义：该参数用于描述NRF的FQDN。<br>数据来源：全网规划<br>取值范围：字符串类型，输入长度范围是0~255。该参数只能由字母（A-Z或者a-z）、数字（0-9）、连字符（-）和点（.）组成，大小写不敏感，FQDN不能以“.”开始，也不能以“.”结束。<br>默认值：无<br>配置原则：无 |
| DESC | 描述 | 可选必选说明：可选参数<br>参数含义：该参数用于描述信息。<br>数据来源：本端规划<br>取值范围：字符串类型，输入长度范围是0~255。<br>默认值：无<br>配置原则：无 |

## 操作的配置对象

- [[UNC@20.15.2@ConfigObject@NRFFQDN]] · NRF的FQDN（NRFFQDN）

## 使用实例

假如运营商希望增加一条INDEX为1、MCC为"460"、MNC为"03"、SST为1、FQDN为"TAC-LB01.TAC-HB71.TAC.EPC.MNC001.MCC308.3GPPNETWORK.ORG"的记录，执行下列命令。

```
ADD NRFFQDN: INDEX=1, MCC="460", MNC="03", SST=1, FQDN="TAC-LB01.TAC-HB71.TAC.EPC.MNC001.MCC308.3GPPNETWORK.ORG";
```

## 证据

- 原始手册：`evidence/UNC/20.15.2/ADD-NRFFQDN.md`
