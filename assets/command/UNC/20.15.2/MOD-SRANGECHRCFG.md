---
id: UNC@20.15.2@MMLCommand@MOD SRANGECHRCFG
type: MMLCommand
name: MOD SRANGECHRCFG（修改小范围CHR生成规则）
nf: UNC
version: 20.15.2
verb: MOD
...
# MOD SRANGECHRCFG（修改小范围CHR生成规则）

## 功能

**适用网元：SGSN、MME**

该命令用于修改小范围CHR单据的生成规则。

## 注意事项

- 该命令执行后立即生效。
- 小范围CHR单据只进行本地存储，不发送给外部CloudUDN。
- 设置用户范围时，如果所设置范围内的用户数量过多，或设置的采集失效时间过长，会导致系统生成的小范围CHR单据数量过多，会引发系统负荷加重问题。
- OMU本地硬盘上小范围CHR单据和异常CHR单据合并的存储空间为4G，存储空间满后，会以最新的小范围CHR单据覆盖最旧的小范围CHR单据，此时，旧的单据会丢失。同时会影响异常CHR单据的存储时长。
...
| IMSIPRE | IMSI前缀 | 可选必选说明：条件必选参数<br>参数含义：该参数用于指定IMSI前缀。<br>前提条件：该参数在<br>“用户范围”<br>设置为<br>“E_SPECIAL_IMSI”<br>后生效。<br>数据来源：全网规划<br>取值范围：11～15位十进制数字字符串<br>默认值：无 |
| IMEIPRE | IMEI前缀 | 可选必选说明：条件必选参数<br>参数含义：该参数用于指定IMEI前缀。<br>前提条件：该参数在<br>“用户范围”<br>设置为<br>“E_SPECIAL_IMEI”<br>后生效。<br>数据来源：全网规划<br>取值范围：11～15位十进制数字字符串<br>默认值：无 |
...
| ENODEBID | eNodeB标识 | 可选必选说明：条件必选参数<br>参数含义：该参数用于指定eNodeB的ID。<br>前提条件：该参数在<br>“用户范围”<br>设置为<br>“E_SPECIAL_ENB”<br>后生效。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围为0~268435455。<br>默认值：无 |
...
MOD SRANGECHRCFG: SUBRANGE=E_SPECIAL_APNQCI, APNNI="Huawei.com", QCI=1, PROCMESSAGE=SUCCESS-0;
...
MOD SRANGECHRCFG: SUBRANGE=E_SPECIAL_APNQCI, APNNI="Huawei.com", QCI=2, PROCMESSAGE=SUCCESS-0;

## 证据

- 原始手册：`evidence/UNC/20.15.2/MOD-SRANGECHRCFG.md`
