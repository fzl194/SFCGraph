---
id: UNC@20.15.2@MMLCommand@MOD PNFDNAI
type: MMLCommand
name: MOD PNFDNAI（修改对端NF的DNAI信息）
nf: UNC
version: 20.15.2
verb: MOD
...
# MOD PNFDNAI（修改对端NF的DNAI信息）

## 功能

**适用NF：AMF、SMF、NSSF、SMSF、NCG、PGW-C**

该命令用于修改本地配置的对端NF实例支持的数据网络接入标识信息。
...
| PRISWITCH | 优先级配置功能 | 可选必选说明：可选参数<br>参数含义：该参数用于指定对端UPF实例支持的DNAI与其归属DNN为条件的优先级选择功能是否启用。<br>数据来源：本端规划<br>取值范围：<br>- “INHERIT（继承）”：继承PNFPROFILE中相同字段的内容<br>- “SPECIFIC（指定）”：使用本配置记录中相同字段的内容<br>默认值：无<br>配置原则：无 |
...
| CAPSWITCH | 权重配置功能 | 可选必选说明：可选参数<br>参数含义：该参数用于指定对端UPF实例支持的DNAI与其归属DNN为条件的权重选择功能是否启用。<br>数据来源：本端规划<br>取值范围：<br>- “INHERIT（继承）”：继承PNFPROFILE中相同字段的内容<br>- “SPECIFIC（指定）”：使用本配置记录中相同字段的内容<br>默认值：无<br>配置原则：<br>当配置的对端网元是UPF，且CAPSWITCH为SPECIFIC时，CAPACITY生效的前提条件是配置UPSELECTFLAG的PRIORITYFLAG为ENABLE。 |
...
修改对端NF的DNAI信息，NF实例标识为UPF_Instance_0，DNAI为huawei.com，绑定的DNN索引为1。

```
MOD PNFDNAI: INDEX=1,NFINSTANCEID="UPF_Instance_0",DNAI="huawei.com",PNFDNNINDEX=1;
```
