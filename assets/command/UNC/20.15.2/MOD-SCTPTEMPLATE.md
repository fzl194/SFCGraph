---
id: UNC@20.15.2@MMLCommand@MOD SCTPTEMPLATE
type: MMLCommand
name: MOD SCTPTEMPLATE（修改SCTP模板）
nf: UNC
version: 20.15.2
verb: MOD
...
# MOD SCTPTEMPLATE（修改SCTP模板）

## 功能

**适用NF：PGW-C、SMF**

此命令用于修改SCTP模板，根据现网规划，当UNC使用SCTP承载逻辑接口信令时，可以使用此命令修改SCTP协议参数。
...
| DELIVERYMODE | SCTP Data Chunk传输模式 | 可选必选说明：可选参数<br>参数含义：该参数用户设置SCTP协议Data Chunk的传输模式。<br>数据来源：对端协商<br>取值范围：枚举类型。<br>- ORDERED：SCTP Data Chunk传输模式为顺序传输。<br>- UNORDERED：SCTP Data Chunk传输模式为非顺序传输。<br>默认值：无<br>配置原则：<br>- 设置UNC发送SCTP协议DATA chunk的传输模式为非顺序传输时，Diameter对端需要对非顺序到达的消息能够正常处理，否则将会对业务造成影响。<br>- UNC支持对Diameter对端按顺序模式或非顺序模式发送SCTP协议DATA chunk，且不需要配置控制。 |
...
```
MOD SCTPTEMPLATE: SCTPTEMPLNAME="sctp_tp1",HEARTBEATINTVAL=20000;
```

## 证据

- 原始手册：`evidence/UNC/20.15.2/MOD-SCTPTEMPLATE.md`
