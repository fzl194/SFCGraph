---
id: UNC@20.15.2@MMLCommand@ADD OSPFINTERFACEAUTH
type: MMLCommand
name: ADD OSPFINTERFACEAUTH（创建OSPF接口认证配置）
nf: UNC
version: 20.15.2
verb: ADD
...
# ADD OSPFINTERFACEAUTH（创建OSPF接口认证配置）
...
- 该命令执行后立即生效。
- 该命令最大记录数为8000。
- 创建OSPF接口认证，请确保和邻居认证配置相同，否则会导致邻接关系中断，影响业务。
- 只有执行ADD OSPFINTERFACE配置了OSPF接口后才能使用此命令。
...
| AUTHTEXTTYPE | 密文口令类型 | 可选必选说明：条件可选参数<br>前提条件：该参数在“AUTHENMODE”配置为“hmac-sha256”、“simple”、“md5” 或 “hmac-md5”时为可选参数。<br>参数含义：认证密码类型。<br>数据来源：本端规划<br>取值范围：枚举类型。<br>- cipher：密文类型。<br>默认值：cipher<br>配置原则：cipher类型可以键入简单口令或密文口令，但在查看配置文件时均以密文方式显示口令。 |
...
| KEYID | 密文验证字标识符 | 可选必选说明：条件可选参数<br>前提条件：该参数在“AUTHENMODE”配置为“hmac-sha256”、“md5” 或 “hmac-md5”时为可选参数。<br>参数含义：密文验证字标识符。<br>数据来源：本端规划<br>取值范围：整数类型，取值范围为1～255。<br>默认值：无<br>配置原则：<br>- 密文验证字标识符和加密认证密码必须同时配置。<br>- KeyID需要跟对端保持一致。<br>- 如果不输入该参数，则表示密文验证字标识符不参与认证加密。 |
...
- [OSPF接口认证配置（OSPFINTERFACEAUTH）](configobject/UNC/20.15.2/OSPFINTERFACEAUTH.md)

## 使用实例

在接口Ethernet64/0/4配置OSPF keychain认证模式：

```
ADD OSPFINTERFACEAUTH:PROCID=1,AREAID="0.0.0.0",IFNAME="Ethernet64/0/4",AUTHENMODE=keychain,KEYCHAINNAME="KcName1";
```

## 证据

- 原始手册：`evidence/UNC/20.15.2/创建OSPF接口认证配置（ADD-OSPFINTERFACEAUTH）_00865817.md`
