---
id: UNC@20.15.2@MMLCommand@ADD LDPAUTHGROUP
type: MMLCommand
name: ADD LDPAUTHGROUP（添加LDP认证组）
nf: UNC
version: 20.15.2
verb: ADD
...
| AUTHPEERGROUPNAME | 认证组名称 | 可选必选说明：必选参数<br>参数含义：该参数用于指定认证组名称。<br>数据来源：本端规划<br>取值范围：字符串类型，输入长度范围为1～169。<br>默认值：无<br>配置原则：<br>- 所用的IP前缀名需提前配好。<br>- 只能配置一个LDP认证组。 |
...
- [LDP认证组（LDPAUTHGROUP）](configobject/UNC/20.15.2/LDPAUTHGROUP.md)
...
```
ADD LDPAUTHGROUP:VRFNAME="_public_",AUTHPEERGROUPNAME="bb",AUTHENTYPE=KEYCHAIN,KEYCHAINNAME="key1";
```
