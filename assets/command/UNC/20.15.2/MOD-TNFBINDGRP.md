---
id: UNC@20.15.2@MMLCommand@MOD TNFBINDGRP
type: MMLCommand
name: MOD TNFBINDGRP（修改目标NF实例绑定组）
nf: UNC
version: 20.15.2
verb: MOD
...
| PRIORITY | 优先级 | 可选必选说明：可选参数<br>参数含义：本参数用于指定目标NF的优先级（与其他同类型NF实例相比）。在NF选择过程中，NF会选择高优先级的NF，如果两个或多个NF的优先级一样，NF则会根据“权重”做进一步的判断。<br>数据来源：全网规划<br>取值范围：整数类型，取值范围是1~255。值越小优先级越高。<br>默认值：无<br>配置原则：无 |
...
- [[UNC@20.15.2@ConfigObject@TNFBINDGRP]] · 目标NF实例绑定组（TNFBINDGRP）

## 使用实例

运营商A需要对索引为0的目标NF组中索引1的目标NF进行修改，将优先级修改为10，权重修改为20。
...
- 原始手册：`evidence/UNC/20.15.2/MOD-TNFBINDGRP.md`
