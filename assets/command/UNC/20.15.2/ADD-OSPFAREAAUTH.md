---
id: UNC@20.15.2@MMLCommand@ADD OSPFAREAAUTH
type: MMLCommand
name: ADD OSPFAREAAUTH（创建OSPF区域认证配置）
nf: UNC
version: 20.15.2
verb: ADD
...
# ADD OSPFAREAAUTH（创建OSPF区域认证配置）

## 功能

该命令用于创建OSPF区域认证配置。

![](创建OSPF区域认证配置（ADD OSPFAREAAUTH）_00600897.assets/notice_3.0-zh-cn_2.png)
...
- 该命令执行后立即生效。
- 该命令最大记录数为8016。
- 创建OSPF区域认证，请确保和邻居认证配置相同，否则会导致邻接关系中断，影响业务。
...
- [OSPF区域认证配置（OSPFAREAAUTH）](configobject/UNC/20.15.2/OSPFAREAAUTH.md)

## 使用实例

配置OSPF进程1区域0.0.0.0下配置keychain认证：

```
ADD OSPFAREAAUTH:PROCID=1,AREAID="0.0.0.0",AUTHENMODE=keychain,KEYCHAINNAME="KcName1";
```

## 证据

- 原始手册：`evidence/UNC/20.15.2/创建OSPF区域认证配置（ADD-OSPFAREAAUTH）_00600897.md`
