---
id: UNC@20.15.2@MMLCommand@MOD UPCMPTCUSIE
type: MMLCommand
name: MOD UPCMPTCUSIE（修改UP节点定制信元兼容性配置）
nf: UNC
version: 20.15.2
verb: MOD
...
# MOD UPCMPTCUSIE（修改UP节点定制信元兼容性配置）

## 功能

**适用NF：SMF、SGW-C、PGW-C、GGSN**

该命令用于修改UP节点定制信元兼容性配置。
...
修改UP节点定制信元配置，UPF实例标识为upf_instance_1，设置Extend FSEID 企业ID为2011，类型值为37101，执行如下命令：

```
MOD UPCMPTCUSIE: UPFINSTANCE="upf_instance_1", IENAME=EFSEID, CUSTOMTYPE=37101, CUSTOMEPID=2011
```
