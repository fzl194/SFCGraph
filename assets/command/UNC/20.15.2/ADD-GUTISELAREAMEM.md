---
id: UNC@20.15.2@MMLCommand@ADD GUTISELAREAMEM
type: MMLCommand
name: ADD GUTISELAREAMEM（增加GUTI选网功能区域成员）
nf: UNC
version: 20.15.2
verb: ADD
...
# ADD GUTISELAREAMEM（增加GUTI选网功能区域成员）
...
- GUTISELAREAMEM重叠是指两条GUTISELAREAMEM记录中MCC和MNC都相等，且TAC范围有重叠。
- 不同配置记录的GUTISELAREAMEM记录之间不能重叠。
- 最多可输入4096条记录。
...
- [[configobject/UNC/20.15.2/GUTISELAREAMEM]] · GUTI选网功能区域成员（GUTISELAREAMEM）

## 使用实例

为编码为“GUTISelZone”的区域增加一个跟踪区（MCC="123", MNC="45", TAC="120101"），执行如下命令：

```
ADD GUTISELAREAMEM: GUTISELAREACODE="GUTISelZone", MCC="123", MNC="45", BGNTAC="120101";
```

## 证据

- 原始手册：`evidence/UNC/20.15.2/ADD-GUTISELAREAMEM.md`
