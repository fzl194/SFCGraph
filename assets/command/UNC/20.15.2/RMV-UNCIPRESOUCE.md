---
id: UNC@20.15.2@MMLCommand@RMV UNCIPRESOUCE
type: MMLCommand
name: RMV UNCIPRESOUCE（删除UNC接口IP地址）
nf: UNC
version: 20.15.2
verb: RMV
...
# RMV UNCIPRESOUCE（删除UNC接口IP地址）

## 功能

**适用NF：SGSN、MME、SGW-C、PGW-C、AMF、SMF、NRF、NSSF、GGSN、SMSF、NCG**

该命令用于删除已经上报至网管北向的UNC网元业务或者管理接口的IP地址。
...
- [UNC接口IP地址（UNCIPRESOUCE）](configobject/UNC/20.15.2/UNCIPRESOUCE.md)

## 使用实例

- 删除已经上报至网管北向的UNC网元业务或者管理接口的IP地址，其接口为N4，IPv4地址为10.1.3.11，执行以下命令：
  ```
  RMV UNCIPRESOUCE: INTFTYPE="N4", IPTYPE=IPv4, IPV4="10.1.3.11";
  ```
- 删除已经上报至网管北向的UNC网元业务或者管理接口的IP地址，其接口为N4，IPv6地址为2001:0DB8:0:0:0:800:200C:510A，执行以下命令：
  ```
  RMV UNCIPRESOUCE: INTFTYPE="N4", IPTYPE=IPv6, IPV6="2001:0DB8:0:0:0:800:200C:510A";
  ```
