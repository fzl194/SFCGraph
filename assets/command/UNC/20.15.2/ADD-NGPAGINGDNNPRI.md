---
id: UNC@20.15.2@MMLCommand@ADD NGPAGINGDNNPRI
type: MMLCommand
name: ADD NGPAGINGDNNPRI（增加基于DNN的Paging消息在流控期间放通的优先级）
nf: UNC
version: 20.15.2
verb: ADD
...
# ADD NGPAGINGDNNPRI（增加基于DNN的Paging消息在流控期间放通的优先级）
...
- 配置的DNN需为系统中已配置的DNN。
- 当用户不配置NGPAGINGDNNPRI时，则默认为低优先级。
...
- [基于DNN的Paging消息在流控期间放通的优先级（NGPAGINGDNNPRI）](configobject/UNC/20.15.2/NGPAGINGDNNPRI.md)
