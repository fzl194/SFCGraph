---
id: UNC@20.15.2@MMLCommand@MOD CPCGBINDING
type: MMLCommand
name: MOD CPCGBINDING（修改抄送CG绑定关系）
nf: UNC
version: 20.15.2
verb: MOD
...
| CGIPVERSION | CG IP版本 | 可选必选说明：必选参数<br>参数含义：该参数用于指定CG IP类型。<br>数据来源：全网规划<br>取值范围：枚举类型。<br>- IPV4：IPv4。<br>- IPV6：IPv6。<br>默认值：无<br>配置原则：无 |
...
- [[UNC@20.15.2@ConfigObject@CPCGBINDING]] · 抄送CG绑定关系（CPCGBINDING）

## 使用实例

修改抄送CG绑定关系，抄送CG组ID为1，CG的IP地址为192.168.0.2，CG的端口号为25009，CG优先级修改为2：

```
MOD CPCGBINDING: CPCGGRPID=1, CGIPVERSION=IPV4, CGIPV4ADDR="192.168.0.2", CGPORT=25009, PRIORITY=2;
```

## 证据

- 原始手册：`evidence/UNC/20.15.2/MOD-CPCGBINDING.md`
