---
id: UNC@20.15.2@MMLCommand@ADD GTPPLOCINFO
type: MMLCommand
name: ADD GTPPLOCINFO（增加GTPP本端信息）
nf: UNC
version: 20.15.2
verb: ADD
...
# ADD GTPPLOCINFO（增加GTPP本端信息）

## 功能

**适用NF：SGW-C、PGW-C、SMF**

该命令用来增加GTPP（GTP'协议）本端信息。
...
| REALMNAME | 本端域名 | 可选必选说明：必选参数<br>参数含义：该参数用于指定本端域名。<br>数据来源：本端规划<br>取值范围：字符串类型，输入长度范围为1～63。不支持空格。<br>默认值：无<br>配置原则：无 |
...
- [GTPP本端信息（GTPPLOCINFO）](configobject/UNC/20.15.2/GTPPLOCINFO.md)

## 使用实例

增加GTPP本端信息，本端主机名为“test”， 本端域名为“test”， 业务上下文标识为“test”， 逻辑接口名称为“gaif1/0/1”：

```
ADD GTPPLOCINFO: HOSTNAME="test", REALMNAME="test", SERVICECONTEXT="test", LOCINTERFACE="gaif1/0/1";
```
