---
id: UNC@20.15.2@MMLCommand@ADD RRCINACTTAI
type: MMLCommand
name: ADD RRCINACTTAI（增加RRC Inactive生效的TAI范围）
nf: UNC
version: 20.15.2
verb: ADD
...
# ADD RRCINACTTAI（增加RRC Inactive生效的TAI范围）

## 功能

**适用NF：AMF**

本命令用于增加RRC Inactive生效的TAI范围。

## 注意事项

- 该命令执行后立即生效。

- 该命令在ADD RRCINACTPLCY中的参数“其他RRC Inactive开启条件” 配置为TAI后生效。
- 起始TAI和结束TAI配置相同则表示一个具体的号码。
- 结束TAI大于起始TAI则表示一个TAI区间。
- 不同的TAI区间不能交叉重叠。
...
- [RRC Inactive生效的TAI范围（RRCINACTTAI）](configobject/UNC/20.15.2/RRCINACTTAI.md)

## 使用实例

增加一条RRC Inactive生效的TAI范围，“移动国家代码”为“123”，“移动网号”为“00”，“跟踪区编码起始值”为“000112”，“跟踪区编码结束值”为“000350”，执行如下命令：

```
ADD RRCINACTTAI: MCC="123", MNC="00", BGNTAC="000112", ENDTAC="000350";
```
