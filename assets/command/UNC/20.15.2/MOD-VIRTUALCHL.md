---
id: UNC@20.15.2@MMLCommand@MOD VIRTUALCHL
type: MMLCommand
name: MOD VIRTUALCHL（修改虚拟通道映射）
nf: UNC
version: 20.15.2
verb: MOD
...
- 该命令执行后立即生效。
- 修改虚拟通道后，本地备份和本地分发的目的路径的旧虚拟通道目录下如果存在文件，不会自动删除，会占用NCG的磁盘空间。
...
- [[UNC@20.15.2@ConfigObject@VIRTUALCHL]] · 虚拟通道映射（VIRTUALCHL）

## 使用实例

修改接入分组PS1的"98/OFFLINE"通道的虚拟通道映射为“100/OFFLINE”，配置举例如下：

```
MOD VIRTUALCHL: SOURCECHL="98/OFFLINE", AGID="PS1", VIRTUALCHL="100/OFFLINE";
```

## 证据

- 原始手册：`evidence/UNC/20.15.2/MOD-VIRTUALCHL.md`
