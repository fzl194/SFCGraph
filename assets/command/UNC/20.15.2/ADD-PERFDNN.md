---
id: UNC@20.15.2@MMLCommand@ADD PERFDNN
type: MMLCommand
name: ADD PERFDNN（增加DNN性能统计对象）
nf: UNC
version: 20.15.2
verb: ADD
...
- 网络切片已支持的DNN（ADD NSDNN配置）会自动添加到PERFDNN中，不需要人工配置。执行ADD PERFDNN命令时只能配置ADD NSDNN命令没有配置过的DNN。若PERFDNN记录数已达到最大规格，则随后执行ADD NSDNN命令添加的新切片信息不会再自动同步到PERFDNN中。
...
- [DNN性能统计对象（PERFDNN）](configobject/UNC/20.15.2/PERFDNN.md)

## 使用实例

当运营商希望增加一个PERFDNN配置记录，其中DNN为"huawei.com"，PLMN为12303，网络切片为“1-010101”作为性能指标对象时，执行如下命令：

```
ADD PERFDNN: DNN="huawei.com", MCC="123", MNC="03", SNSSAI="1-010101";
```
