---
id: UNC@20.15.2@MMLCommand@ADD AUTOSCALINGBGPLF
type: MMLCommand
...
- 该命令执行后立即生效。
- 该命令最大记录数为64。
- 相同VPN的同一个地址族下，最多允许配置四个IP对等体地址的BGP入不转板自动化配置模板。
...
| UDADVCOM | 发布的团体属性 | 可选必选说明：该参数在“UDADVCOMENABLE”配置为“TRUE”时为条件必选参数。<br>参数含义：该参数用于表示给对端网关发布路由时携带的团体属性。<br>数据来源：对端协商<br>取值范围：字符串类型，输入长度范围为1～51。默认值：无<br>配置原则：格式是aa<0-65535>:nn<0-65535>或取值为internet、no-export-subconfed、no-advertise、no-export，以及0-4294967295的整数。 |

## 操作的配置对象

- [BGP入不转板自动化配置模板（AUTOSCALINGBGPLF）](configobject/UNC/20.15.2/AUTOSCALINGBGPLF.md)
