---
id: UNC@20.15.2@MMLCommand@ADD UERCAPUNCSEL
type: MMLCommand
name: ADD UERCAPUNCSEL（增加UE无线能力选择UNC参数）
nf: UNC
version: 20.15.2
verb: ADD
object_keyword: UERCAPUNCSEL
...
# ADD UERCAPUNCSEL（增加UE无线能力选择UNC参数）
...
| UNCSELSW | UNC重选开关 | 可选必选说明：必选参数<br>参数含义：该参数用于指定是否开启UNC重选功能。<br>数据来源：整网规划<br>取值范围：<br>- OFF：关闭<br>- ON：开启<br>默认值：无 |
| UERCLEN | 长度阈值(KB) | 可选必选说明：条件必选参数<br>参数含义：该参数用于指定根据UE无线能力重选UNC的UE无线能力长度， UE无线能力长度大于<br>“长度阈值(KB)”<br>的用户可以迁移到UNC。<br>前提条件：该参数在<br>“UNC重选开关”<br>参数配置为<br>“ON(开启)”<br>后生效。<br>数据来源：整网规划<br>取值范围：1~63 |
| TMMEGPIDX | 目标MME组索引 | 可选必选说明：条件必选参数<br>参数含义：该参数用于指定根据UE无线能力进行UNC选择时的目标MME群组的标识。<br>前提条件：该参数在<br>“UNC重选开关”<br>参数配置为<br>“ON(开启)”<br>后生效。<br>数据来源：整网规划<br>取值范围：0~63的整数类型。<br>默认值：无<br>配置原则：该参数必须已经在<br>[**ADD MMEGP**](../../MME群组管理/MME群组配置/增加MME群组(ADD MMEGP)_72225301.md)<br>配置。 |

## 操作的配置对象

- [[UNC@20.15.2@ConfigObject@UERCAPUNCSEL]] · UE无线能力选择UNC参数（UERCAPUNCSEL）

## 使用实例

增加一条 “用户范围” 为 “所有用户” ， “UNC重选开关” 为 “ON（开启）” ， “长度阈值(KB)” 为 “3” ， “目标MME组索引” 为 “0” 的记录；

```
ADD UERCAPUNCSEL: SUBRANGE=ALL_USER, UNCSELSW=ON, UERCLEN=3, TMMEGPIDX=0;
```

## 证据

- 原始手册：`evidence/UNC/20.15.2/ADD-UERCAPUNCSEL.md`
