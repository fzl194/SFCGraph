---
id: UNC@20.15.2@MMLCommand@MOD PGWESTNSR
type: MMLCommand
name: MOD PGWESTNSR（修改E-STN-SR和PGW关系配置表）
nf: UNC
version: 20.15.2
verb: MOD
...
# MOD PGWESTNSR（修改E-STN-SR和PGW关系配置表）

## 功能

**适用网元：MME**

该命令用于修改SRVCC的紧急会话转移号码和P-GW关系配置表。通过本命令配置的PGW IP来找到P-GW对应的“SRVCC的紧急会话转移号码”。与华为MSC设备对接需要配置本命令。
...
| ESTNSR | SRVCC的紧急会话转移号码 | 可选必选说明：必选参数<br>参数含义：该参数用于设置SRVCC的紧急会话转移号码。<br>数据来源：运营商规划<br>取值范围： 1～15位数字<br>默认值： 无 |

## 操作的配置对象

- [[UNC@20.15.2@ConfigObject@PGWESTNSR]] · E-STN-SR和PGW关系配置表（PGWESTNSR）

## 使用实例

修改SRVCC的紧急会话转移号码和P-GW关系配置表， 修改记录 “PGW的IP地址类型” 为 “IPV4” ， “PGW的IPV4地址” 为 “10.10.10.10” ， “PGW的IPV4地址掩码” 为 “255.255.0.0” 的 “ESTNSR” 为 “11111111111” ：

MOD PGWESTNSR: IPT=IPV4, PGWIPV4="10.10.10.10", IPV4MSK="255.255.0.0", ESTNSR="11111111111";

## 证据

- 原始手册：`evidence/UNC/20.15.2/MOD-PGWESTNSR.md`
