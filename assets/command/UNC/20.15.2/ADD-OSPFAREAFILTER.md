---
id: UNC@20.15.2@MMLCommand@ADD OSPFAREAFILTER
type: MMLCommand
name: ADD OSPFAREAFILTER（创建区域过滤策略配置）
nf: UNC
version: 20.15.2
verb: ADD
...
| INCAGGRFLAG | 包括ABR Summary LSA | 可选必选说明：条件可选参数<br>前提条件：该参数在“FILTERTYPE”配置为“import”时为可选参数。<br>参数含义：包括ABR Summary LSA。<br>数据来源：本端规划<br>取值范围：布尔类型，输入格式为“TRUE”或者“FALSE”。<br>默认值：FALSE |
...
在进程1的区域0.0.0.1下配置出方向的类型为ip-prefix的过滤策略：

```
ADD OSPFAREAFILTER:PROCID=1,AREAID="0.0.0.1",TOPONAME=base,FILTERTYPE=export,TYPE=ip_prefix,IPPREFIX="ip-prefix";
```
