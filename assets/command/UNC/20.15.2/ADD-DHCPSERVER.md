---
id: UNC@20.15.2@MMLCommand@ADD DHCPSERVER
type: MMLCommand
name: ADD DHCPSERVER（增加DHCP服务器）
nf: UNC
version: 20.15.2
verb: ADD
...
- 执行该命令之前，需要先执行ADD DHCPSERVERGRP创建DHCP服务器组。
- 一个DHCP服务器组包含两个DHCP服务器，一主一备。配置的时候必须先配主DHCP服务器，后配备DHCP服务器。
...
- [DHCP服务器（DHCPSERVER）](configobject/UNC/20.15.2/DHCPSERVER.md)

## 使用实例

当需要为DHCP服务器组group1添加主DHCP服务器，配置其IP地址为10.1.1.1时，使用该命令：
