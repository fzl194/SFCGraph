---
id: UNC@20.15.2@MMLCommand@ADD NFABILITY
type: MMLCommand
name: ADD NFABILITY（增加配置NF的能力信息）
nf: UNC
version: 20.15.2
verb: ADD
...
# ADD NFABILITY（增加配置NF的能力信息）

## 功能

**适用NF：NRF**

该命令用于新增配置NF的能力信息。NRF可以根据NF能力的不同，做一些差异化的控制。

## 注意事项

- 该命令执行后立即生效。

- 如果NF的客户端IP地址是IPv4&IPv6双栈则IPv4和IPv6都需要配置。
- 主备或双活组网的场景下，如果需要配置此命令，则两个NRF上均需执行此命令，且配置参数一致。
- MB-SMF请求AMF场景下，该命令在NRFFUNCSW下的MBSMFFILTERSW开关为“FUNC_ON”时不生效。
...
- [[UNC@20.15.2@ConfigObject@NFABILITY]] · 配置NF的能力信息（NFABILITY）

## 使用实例

运营商希望在NRF上给NF实例标识为123e4567-e89b-12d3-a456-426655440000的NF配置能力信息时，执行此命令。

```
ADD NFABILITY: ABILITY=INCCACHE-0&NOCACHE-1, NFIDTYPE=NF_ID, NFINSTANCEID="123e4567-e89b-12d3-a456-426655440000";
```

## 证据

- 原始手册：`evidence/UNC/20.15.2/ADD-NFABILITY.md`
