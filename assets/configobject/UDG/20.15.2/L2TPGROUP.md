---
id: UDG@20.15.2@ConfigObject@L2TPGROUP
type: ConfigObject
name: L2TPGROUP（指定L2TP组）
nf: UDG
version: 20.15.2
object_name: L2TPGROUP
object_kind: entity
applicable_nf:
- PGW-U
- UPF
status: active
---

# L2TPGROUP（指定L2TP组）

## 说明

**适用NF：PGW-U、UPF**

该命令用于配置L2TP组，当需要使用本地配置的L2TP信息接入L2TP用户时，使用此命令。

## 操作本对象的命令

- [ADD L2TPGROUP](command/UDG/20.15.2/ADD-L2TPGROUP.md)
- [LST L2TPGROUP](command/UDG/20.15.2/LST-L2TPGROUP.md)
- [MOD L2TPGROUP](command/UDG/20.15.2/MOD-L2TPGROUP.md)
- [RMV L2TPGROUP](command/UDG/20.15.2/RMV-L2TPGROUP.md)

## 证据

- 原始手册：`evidence/UDG/20.15.2/修改指定L2TP组（MOD-L2TPGROUP）_35373525.md`
- 原始手册：`evidence/UDG/20.15.2/创建L2TP组（ADD-L2TPGROUP）_35373524.md`
- 原始手册：`evidence/UDG/20.15.2/删除指定L2TP组（RMV-L2TPGROUP）_35373526.md`
- 原始手册：`evidence/UDG/20.15.2/查询指定L2TP组（LST-L2TPGROUP）_35373527.md`
