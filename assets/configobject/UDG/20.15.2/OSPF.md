---
id: UDG@20.15.2@ConfigObject@OSPF
type: ConfigObject
name: OSPF（OSPF进程配置）
nf: UDG
version: 20.15.2
object_name: OSPF
object_kind: entity
status: active
---

# OSPF（OSPF进程配置）

## 说明

该命令用于增加OSPF特性的进程。

## 操作本对象的命令

- [[command/UDG/20.15.2/ADD-OSPF]] · ADD OSPF
- [[command/UDG/20.15.2/LST-OSPF]] · LST OSPF
- [[command/UDG/20.15.2/MOD-OSPF]] · MOD OSPF
- [[command/UDG/20.15.2/RMV-OSPF]] · RMV OSPF
- [[command/UDG/20.15.2/RST-OSPF]] · RST OSPF

## 关联对象

- [[configobject/UDG/20.15.2/L3VPNINST]] · L3VPNINST

## 证据

- 原始手册：`evidence/UDG/20.15.2/修改OSPF进程配置（MOD-OSPF）_50121158.md`
- 原始手册：`evidence/UDG/20.15.2/创建OSPF进程配置（ADD-OSPF）_00866089.md`
- 原始手册：`evidence/UDG/20.15.2/删除OSPF进程配置（RMV-OSPF）_00841109.md`
- 原始手册：`evidence/UDG/20.15.2/查询OSPF进程配置（LST-OSPF）_49802194.md`
- 原始手册：`evidence/UDG/20.15.2/重置OSPF进程（RST-OSPF）_49802458.md`
