---
id: UDG@20.15.2@ConfigObject@LOGICINF
type: ConfigObject
name: LOGICINF（逻辑接口）
nf: UDG
version: 20.15.2
object_name: LOGICINF
object_kind: entity
applicable_nf:
- SGW-U
- PGW-U
- UPF
status: active
---

# LOGICINF（逻辑接口）

## 说明

**适用NF：SGW-U、PGW-U、UPF**

该命令用于创建指定的逻辑接口。逻辑接口指的是物理上不存在，需要通过配置建立的接口，能够实现数据交换功能。

## 操作本对象的命令

- [ADD LOGICINF](command/UDG/20.15.2/ADD-LOGICINF.md)
- [LST LOGICINF](command/UDG/20.15.2/LST-LOGICINF.md)
- [MOD LOGICINF](command/UDG/20.15.2/MOD-LOGICINF.md)
- [RMV LOGICINF](command/UDG/20.15.2/RMV-LOGICINF.md)

## 关联对象

- [IPFARM](configobject/UDG/20.15.2/IPFARM.md)
- [VPNINST](configobject/UDG/20.15.2/VPNINST.md)

## 证据

- 原始手册：`evidence/UDG/20.15.2/修改逻辑接口（MOD-LOGICINF）_86526348.md`
- 原始手册：`evidence/UDG/20.15.2/删除逻辑接口（RMV-LOGICINF）_82835380.md`
- 原始手册：`evidence/UDG/20.15.2/增加逻辑接口（ADD-LOGICINF）_82835378.md`
- 原始手册：`evidence/UDG/20.15.2/查询逻辑接口（LST-LOGICINF）_86526672.md`
