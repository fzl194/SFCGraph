---
id: UNC@20.15.2@ConfigObject@REASTIMEOUT
type: ConfigObject
name: REASTIMEOUT（IPv4报文重组超时配置）
nf: UNC
version: 20.15.2
object_name: REASTIMEOUT
object_kind: global_setting
status: active
---

# REASTIMEOUT（IPv4报文重组超时配置）

## 说明

为了提高路由设备的性能，防止攻击，使用该命令设置IPv4分片报文适当的重组超时时间，可以使长时间等待重组完成的重组队列及时被老化。缺省情况下，IPv4分片报文的重组超时时间是30秒。

## 操作本对象的命令

- [LST REASTIMEOUT](command/UNC/20.15.2/LST-REASTIMEOUT.md)
- [SET REASTIMEOUT](command/UNC/20.15.2/SET-REASTIMEOUT.md)

## 证据

- 原始手册：`evidence/UNC/20.15.2/查询IPv4报文重组超时配置（LST-REASTIMEOUT）_50121726.md`
- 原始手册：`evidence/UNC/20.15.2/设置IPv4报文重组超时配置（SET-REASTIMEOUT）_50281006.md`
