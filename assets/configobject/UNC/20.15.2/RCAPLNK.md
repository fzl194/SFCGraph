---
id: UNC@20.15.2@ConfigObject@RCAPLNK
type: ConfigObject
name: RCAPLNK（注册中心链路）
nf: UNC
version: 20.15.2
object_name: RCAPLNK
object_kind: entity
applicable_nf:
- SMSF
status: active
---

# RCAPLNK（注册中心链路）

## 说明

**适用NF：SMSF**

此命令用于新增注册中心链路配置。注册中心的链路固定作为CLINET端。

## 操作本对象的命令

- [ADD RCAPLNK](command/UNC/20.15.2/ADD-RCAPLNK.md)
- [DSP RCAPLNK](command/UNC/20.15.2/DSP-RCAPLNK.md)
- [LST RCAPLNK](command/UNC/20.15.2/LST-RCAPLNK.md)
- [MOD RCAPLNK](command/UNC/20.15.2/MOD-RCAPLNK.md)
- [RMV RCAPLNK](command/UNC/20.15.2/RMV-RCAPLNK.md)

## 证据

- 原始手册：`evidence/UNC/20.15.2/修改注册中心链路-(MOD-RCAPLNK)_46028502.md`
- 原始手册：`evidence/UNC/20.15.2/删除注册中心链路-(RMV-RCAPLNK)_46187766.md`
- 原始手册：`evidence/UNC/20.15.2/增加注册中心链路-(ADD-RCAPLNK)_92188105.md`
- 原始手册：`evidence/UNC/20.15.2/显示注册中心链路(DSP-RCAPLNK)_63942114.md`
- 原始手册：`evidence/UNC/20.15.2/查询注册中心链路-(LST-RCAPLNK)_45868734.md`
