---
id: UNC@20.15.2@ConfigObject@APNTRAFFICDIST
type: ConfigObject
name: APNTRAFFICDIST（漫游地动态签约分流控制）
nf: UNC
version: 20.15.2
object_name: APNTRAFFICDIST
object_kind: entity
applicable_nf:
- SMF
status: active
---

# APNTRAFFICDIST（漫游地动态签约分流控制）

## 说明

**适用NF：SMF**

该命令用于添加漫游地动态签约分流控制。指定基于漫游地动态签约的分流策略控制特性的AMF带给SMF的Selected DNN、SMF发给CHF的消息中的Sponsor Identity、访地策略对应的计费用户模板名称，控制是否需要上报辅锚点UPF的用量以及控制园区内主锚点流量是否进行计费。

## 操作本对象的命令

- [ADD APNTRAFFICDIST](command/UNC/20.15.2/ADD-APNTRAFFICDIST.md)
- [LST APNTRAFFICDIST](command/UNC/20.15.2/LST-APNTRAFFICDIST.md)
- [MOD APNTRAFFICDIST](command/UNC/20.15.2/MOD-APNTRAFFICDIST.md)
- [RMV APNTRAFFICDIST](command/UNC/20.15.2/RMV-APNTRAFFICDIST.md)

## 证据

- 原始手册：`evidence/UNC/20.15.2/修改漫游地动态签约分流控制（MOD-APNTRAFFICDIST）_70021109.md`
- 原始手册：`evidence/UNC/20.15.2/删除漫游地动态签约分流控制（RMV-APNTRAFFICDIST）_70261217.md`
- 原始手册：`evidence/UNC/20.15.2/增加漫游地动态签约分流控制（ADD-APNTRAFFICDIST）_19141642.md`
- 原始手册：`evidence/UNC/20.15.2/查询漫游地动态签约分流控制（LST-APNTRAFFICDIST）_18982082.md`
