---
id: UNC@20.15.2@ConfigObject@BGPPEER
type: ConfigObject
name: BGPPEER（BGP对等体）
nf: UNC
version: 20.15.2
object_name: BGPPEER
object_kind: entity
status: active
---

# BGPPEER（BGP对等体）

## 说明

该命令用于增加IPv4或IPv6地址类型的对等体。

## 操作本对象的命令

- [ADD BGPPEER](command/UNC/20.15.2/ADD-BGPPEER.md)
- [LST BGPPEER](command/UNC/20.15.2/LST-BGPPEER.md)
- [MOD BGPPEER](command/UNC/20.15.2/MOD-BGPPEER.md)
- [RBL BGPPEER](command/UNC/20.15.2/RBL-BGPPEER.md)
- [RMV BGPPEER](command/UNC/20.15.2/RMV-BGPPEER.md)
- [SYN BGPPEER](command/UNC/20.15.2/SYN-BGPPEER.md)

## 证据

- 原始手册：`evidence/UNC/20.15.2/修改BGP对等体（MOD-BGPPEER）_00440489.md`
- 原始手册：`evidence/UNC/20.15.2/删除BGP对等体（RMV-BGPPEER）_00841537.md`
- 原始手册：`evidence/UNC/20.15.2/增加BGP对等体（ADD-BGPPEER）_49801602.md`
- 原始手册：`evidence/UNC/20.15.2/查询BGP对等体（LST-BGPPEER）_50120942.md`
- 原始手册：`evidence/UNC/20.15.2/刷新BGP对等体（SYN-BGPPEER）_00441157.md`
- 原始手册：`evidence/UNC/20.15.2/复位BGP邻居（RBL-BGPPEER）_49802162.md`
