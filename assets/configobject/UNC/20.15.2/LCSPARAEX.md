---
id: UNC@20.15.2@ConfigObject@LCSPARAEX
type: ConfigObject
name: LCSPARAEX（LCS扩展参数）
nf: UNC
version: 20.15.2
object_name: LCSPARAEX
object_kind: entity
applicable_nf:
- MME
status: active
---

# LCSPARAEX（LCS扩展参数）

## 说明

**适用网元：MME**

该命令用于基于运营商设置LCS扩展参数。包括VoLTE紧急呼叫触发的NI-LR流程的一些参数。

## 操作本对象的命令

- [ADD LCSPARAEX](command/UNC/20.15.2/ADD-LCSPARAEX.md)
- [LST LCSPARAEX](command/UNC/20.15.2/LST-LCSPARAEX.md)
- [MOD LCSPARAEX](command/UNC/20.15.2/MOD-LCSPARAEX.md)
- [RMV LCSPARAEX](command/UNC/20.15.2/RMV-LCSPARAEX.md)

## 证据

- 原始手册：`evidence/UNC/20.15.2/修改LCS扩展参数(MOD-LCSPARAEX)_26145816.md`
- 原始手册：`evidence/UNC/20.15.2/删除LCS扩展参数(RMV-LCSPARAEX)_72345415.md`
- 原始手册：`evidence/UNC/20.15.2/增加LCS扩展参数(ADD-LCSPARAEX)_26305624.md`
- 原始手册：`evidence/UNC/20.15.2/查询LCS扩展参数(LST-LCSPARAEX)_72225495.md`
