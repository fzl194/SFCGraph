---
id: UNC@20.15.2@ConfigObject@REALMBINDAPN
type: ConfigObject
name: REALMBINDAPN（APN与Diameter Realm关联关系）
nf: UNC
version: 20.15.2
object_name: REALMBINDAPN
object_kind: binding
applicable_nf:
- PGW-C
- SMF
status: active
---

# REALMBINDAPN（APN与Diameter Realm关联关系）

## 说明

**适用NF：PGW-C、SMF**

该命令用于DRA部署场景下，配置APN对应的Diameter域信息，或指定根据IMSI构造Diameter域信息。

该配置基于某种应用类型生效。

## 操作本对象的命令

- [ADD REALMBINDAPN](command/UNC/20.15.2/ADD-REALMBINDAPN.md)
- [LST REALMBINDAPN](command/UNC/20.15.2/LST-REALMBINDAPN.md)
- [MOD REALMBINDAPN](command/UNC/20.15.2/MOD-REALMBINDAPN.md)
- [RMV REALMBINDAPN](command/UNC/20.15.2/RMV-REALMBINDAPN.md)

## 证据

- 原始手册：`evidence/UNC/20.15.2/修改APN与Diameter-Realm关联关系（MOD-REALMBINDAPN）_09897286.md`
- 原始手册：`evidence/UNC/20.15.2/删除APN与Diameter-Realm关联关系（RMV-REALMBINDAPN）_09897287.md`
- 原始手册：`evidence/UNC/20.15.2/增加APN与Diameter-Realm关联关系（ADD-REALMBINDAPN）_09897285.md`
- 原始手册：`evidence/UNC/20.15.2/查询APN与Diameter-Realm关联关系（LST-REALMBINDAPN）_09897288.md`
