---
id: UNC@20.15.2@ConfigObject@NWDAFINFO
type: ConfigObject
name: NWDAFINFO（NWDAF信息）
nf: UNC
version: 20.15.2
object_name: NWDAFINFO
object_kind: entity
status: active
---

# NWDAFINFO（NWDAF信息）

## 说明

该命令用于配置NWDAF的实例信息，如NWDAF的名称。NWDAF实例信息属于基础配置。

## 操作本对象的命令

- [ADD NWDAFINFO](command/UNC/20.15.2/ADD-NWDAFINFO.md)
- [LST NWDAFINFO](command/UNC/20.15.2/LST-NWDAFINFO.md)
- [MOD NWDAFINFO](command/UNC/20.15.2/MOD-NWDAFINFO.md)
- [RMV NWDAFINFO](command/UNC/20.15.2/RMV-NWDAFINFO.md)

## 证据

- 原始手册：`evidence/UNC/20.15.2/修改NWDAF信息（MOD-NWDAFINFO）_70382357.md`
- 原始手册：`evidence/UNC/20.15.2/删除NWDAF信息（RMV-NWDAFINFO）_70462605.md`
- 原始手册：`evidence/UNC/20.15.2/增加NWDAF信息（ADD-NWDAFINFO）_23622914.md`
- 原始手册：`evidence/UNC/20.15.2/查询NWDAF信息（LST-NWDAFINFO）_70382329.md`
