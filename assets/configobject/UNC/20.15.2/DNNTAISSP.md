---
id: UNC@20.15.2@ConfigObject@DNNTAISSP
type: ConfigObject
name: DNNTAISSP（DNN的TAI和ServingScope映射）
nf: UNC
version: 20.15.2
object_name: DNNTAISSP
object_kind: entity
applicable_nf:
- AMF
status: active
---

# DNNTAISSP（DNN的TAI和ServingScope映射）

## 说明

**适用NF：AMF**

该命令用于增加DNN的TAI和ServingScope映射关系。通过本配置，AMF服务发现锚点SMF时可以根据用户所在TAI查询Serving Scope取值。

## 操作本对象的命令

- [ADD DNNTAISSP](command/UNC/20.15.2/ADD-DNNTAISSP.md)
- [LST DNNTAISSP](command/UNC/20.15.2/LST-DNNTAISSP.md)
- [MOD DNNTAISSP](command/UNC/20.15.2/MOD-DNNTAISSP.md)
- [RMV DNNTAISSP](command/UNC/20.15.2/RMV-DNNTAISSP.md)

## 证据

- 原始手册：`evidence/UNC/20.15.2/修改DNN的TAI和ServingScope映射（MOD-DNNTAISSP）_46201086.md`
- 原始手册：`evidence/UNC/20.15.2/删除DNN的TAI和ServingScope映射（RMV-DNNTAISSP）_45881874.md`
- 原始手册：`evidence/UNC/20.15.2/增加DNN的TAI和ServingScope映射（ADD-DNNTAISSP）_46041326.md`
- 原始手册：`evidence/UNC/20.15.2/查询DNN的TAI和ServingScope映射（LST-DNNTAISSP）_96506117.md`
