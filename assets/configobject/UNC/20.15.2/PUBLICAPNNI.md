---
id: UNC@20.15.2@ConfigObject@PUBLICAPNNI
type: ConfigObject
name: PUBLICAPNNI（公共APN NI配置）
nf: UNC
version: 20.15.2
object_name: PUBLICAPNNI
object_kind: entity
applicable_nf:
- SGSN
- MME
status: active
---

# PUBLICAPNNI（公共APN NI配置）

## 说明

**适用网元：SGSN、MME**

该命令用于配置公用APN NI(Access Point Name Network Identifier)信息，公用APN NI信息在激活请求信息纠正功能时使用。

## 操作本对象的命令

- [ADD PUBLICAPNNI](command/UNC/20.15.2/ADD-PUBLICAPNNI.md)
- [LST PUBLICAPNNI](command/UNC/20.15.2/LST-PUBLICAPNNI.md)
- [RMV PUBLICAPNNI](command/UNC/20.15.2/RMV-PUBLICAPNNI.md)

## 证据

- 原始手册：`evidence/UNC/20.15.2/删除公共APN-NI配置(RMV-PUBLICAPNNI)_72225361.md`
- 原始手册：`evidence/UNC/20.15.2/增加公共APN-NI配置(ADD-PUBLICAPNNI)_26145682.md`
- 原始手册：`evidence/UNC/20.15.2/查询公共APN-NI配置(LST-PUBLICAPNNI)_26305492.md`
