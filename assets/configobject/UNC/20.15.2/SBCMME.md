---
id: UNC@20.15.2@ConfigObject@SBCMME
type: ConfigObject
name: SBCMME（SBC MME实体）
nf: UNC
version: 20.15.2
object_name: SBCMME
object_kind: entity
applicable_nf:
- CBCF
status: active
---

# SBCMME（SBC MME实体）

## 说明

**适用网元：CBCF**

此命令用于增加SBC MME实体配置。

## 操作本对象的命令

- [[command/UNC/20.15.2/ADD-SBCMME]] · ADD SBCMME
- [[command/UNC/20.15.2/DSP-SBCMME]] · DSP SBCMME
- [[command/UNC/20.15.2/LST-SBCMME]] · LST SBCMME
- [[command/UNC/20.15.2/MOD-SBCMME]] · MOD SBCMME
- [[command/UNC/20.15.2/RMV-SBCMME]] · RMV SBCMME

## 证据

- 原始手册：`evidence/UNC/20.15.2/修改SBC-MME实体(MOD-SBCMME)_12596110.md`
- 原始手册：`evidence/UNC/20.15.2/删除SBC-MME实体(RMV-SBCMME)_47994769.md`
- 原始手册：`evidence/UNC/20.15.2/增加SBC-MME实体(ADD-SBCMME)_48075261.md`
- 原始手册：`evidence/UNC/20.15.2/显示SBC-MME链路状态(DSP-SBCMME)_48075273.md`
- 原始手册：`evidence/UNC/20.15.2/查询SBC-MME实体(LST-SBCMME)_12436342.md`
