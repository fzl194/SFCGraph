---
id: UNC@20.15.2@ConfigObject@SMFEMGCFG
type: ConfigObject
name: SMFEMGCFG（运营商紧急呼叫会话功能配置）
nf: UNC
version: 20.15.2
object_name: SMFEMGCFG
object_kind: entity
applicable_nf:
- SMF
status: active
---

# SMFEMGCFG（运营商紧急呼叫会话功能配置）

## 说明

**适用NF：SMF**

该命令用于增加运营商紧急呼叫会话相关功能配置。运营商需要使用紧急呼叫业务时，通过此命令配置对应MNO或MVNO下的紧急呼叫会话的ARP、5QI等信息。

## 操作本对象的命令

- [ADD SMFEMGCFG](command/UNC/20.15.2/ADD-SMFEMGCFG.md)
- [LST SMFEMGCFG](command/UNC/20.15.2/LST-SMFEMGCFG.md)
- [MOD SMFEMGCFG](command/UNC/20.15.2/MOD-SMFEMGCFG.md)
- [RMV SMFEMGCFG](command/UNC/20.15.2/RMV-SMFEMGCFG.md)

## 证据

- 原始手册：`evidence/UNC/20.15.2/修改运营商紧急呼叫会话功能配置（MOD-SMFEMGCFG）_37903905.md`
- 原始手册：`evidence/UNC/20.15.2/删除运营商紧急呼叫会话功能配置（RMV-SMFEMGCFG）_91284506.md`
- 原始手册：`evidence/UNC/20.15.2/增加运营商紧急呼叫会话功能配置（ADD-SMFEMGCFG）_91124570.md`
- 原始手册：`evidence/UNC/20.15.2/查询运营商紧急呼叫会话功能配置（LST-SMFEMGCFG）_90836434.md`
