---
id: UNC@20.15.2@Feature@IPFD-014002
type: Feature
name: 支持BGP
nf: UNC
version: 20.15.2
...
![](IPFD-014002 支持BGP特性概述_61317186.assets/zh-cn_image_0000001626424409_2.png)
...
| 相关特性 | 交互关系 |
| --- | --- |
| [BFD](../../IPFD-012003 BFD/IPFD-012003 BFD特性概述_61317254.md) | BFD for BGP能够为BGP邻居之间的链路提供快速检测功能，当邻居之间的链路出现故障时，加快BGP协议的收敛速度。 |
...
- Speaker：发送BGP消息的 UNC 称为BGP发言者（Speaker），它接收或产生新的路由信息，并发布（Advertise）给其它BGP Speaker。当BGP Speaker收到来自其它AS的新路由时，如果该路由比当前已知路由更优、或者当前还没有该路由，它就把这条路由发布给所有其他BGP Speaker（发布该路由的BGP Speaker除外）。
...
- AS_Path属性
  ...
  当BGP Speaker本地通告一条路由时：
    - 当BGP Speaker将这条路由通告到其他AS时，便会将本地AS号添加在AS_Path列表中，并通过Update消息通告给邻居 UNC 。
...
- Next_Hop属性
  BGP的下一跳属性和IGP的有所不同，不一定就是邻居 UNC 的IP地址。通常情况下，Next_Hop属性遵循下面的规则：
...
- MED
  ...
  MED属性相当于IGP使用的度量值（Metrics），它用于判断流量进入AS时的最佳路由。当一个运行BGP的 UNC 通过不同的EBGP对等体得到目的地址相同但下一跳不同的多条路由时，在其它条件相同的情况下，将优先选择MED值较小者作为最佳路由。
- Local_Pref属性
  Local_Pref属性仅在IBGP对等体之间有效，不通告给其他AS。它表明 UNC 的BGP优先级。
  Local_Pref属性用于判断流量离开AS时的最佳路由。当BGP的 UNC 通过不同的IBGP对等体得到目的地址相同但下一跳不同的多条路由时，将优先选择Local_Pref属性值较高的路由。

**BGP选择路由的策略**

在 UNC 的实现中，当到达同一目的地存在多条路由时，BGP采取如下策略进行路由选择：
...
- [IPFD-012003](feature/UNC/20.15.2/IPFD-012003.md)：BFD for BGP能够为BGP邻居之间的链路提供快速检测功能，当邻居之间的链路出现故障时，加快BGP协议的收敛速度。
- [IPFD-014005](feature/UNC/20.15.2/IPFD-014005.md)：通过应用路由策略，控制BGP发布、接收、引用的路由信息。

## 证据

- 原始手册：`evidence/UNC/20.15.2/IPFD-014002-支持BGP特性概述_61317186.md`
- 原始手册：`evidence/UNC/20.15.2/4字节AS号_61317299.md`
- 原始手册：`evidence/UNC/20.15.2/BFD-for-BGP_26342281.md`
- 原始手册：`evidence/UNC/20.15.2/BGP-ADD-PATH_61317352.md`
- 原始手册：`evidence/UNC/20.15.2/BGP-Auto-FRR_61317165.md`
- 原始手册：`evidence/UNC/20.15.2/BGP-Best-external_61317278.md`
- 原始手册：`evidence/UNC/20.15.2/BGP-GR_61317256.md`
- 原始手册：`evidence/UNC/20.15.2/BGP-NSR_61317283.md`
- 原始手册：`evidence/UNC/20.15.2/BGP基本原理_61317232.md`
- 原始手册：`evidence/UNC/20.15.2/BGP安全性_61317373.md`
- 原始手册：`evidence/UNC/20.15.2/BGP按组打包_61317221.md`
- 原始手册：`evidence/UNC/20.15.2/BGP联盟_61317259.md`
- 原始手册：`evidence/UNC/20.15.2/BGP负载分担_75943794.md`
- 原始手册：`evidence/UNC/20.15.2/BGP路由迭代下一跳_15257973.md`
- 原始手册：`evidence/UNC/20.15.2/MP-BGP_61317371.md`
- 原始手册：`evidence/UNC/20.15.2/团体属性_61317348.md`
- 原始手册：`evidence/UNC/20.15.2/对等体组_61317319.md`
- 原始手册：`evidence/UNC/20.15.2/活跃路由发布_61317399.md`
- 原始手册：`evidence/UNC/20.15.2/路由交叉_61317253.md`
- 原始手册：`evidence/UNC/20.15.2/路由反射器_61317315.md`
- 原始手册：`evidence/UNC/20.15.2/路由引入_61317218.md`
- 原始手册：`evidence/UNC/20.15.2/路由聚合_61317163.md`
- 原始手册：`evidence/UNC/20.15.2/路由衰减_61317311.md`
