...
- ✅ 范本（[业务感知域融合计费 CS](business/business-awareness/charging/ConfigurationSolution@charging-converged.md)）
...
3. **网元对接业务域待建**：APN 域 BD 边界划定中提到"网元对接业务域（N4 PFCP 偶联 / Diameter / SBI）"——但该 BD 还未建，APN 域已建好对它的引用（按 [BusinessDomain@network-element-docking.md](BusinessDomain@network-element-docking.md)（★此文件待建，APN 域边界划定引用） 引用，文件暂不存在）
...
构建知识源：仓库根路径 `business-graph/APN业务域/` 4 文件（开发期源，**未在 assets/ 内**，构建期引用，运行时通过 task/feature wiki）
