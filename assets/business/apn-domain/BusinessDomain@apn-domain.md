...
- 含场景（1 个）：[APN 接入与会话管理](business/apn-domain/NetworkScenario@apn-access.md)
- 含方案（4 个，AND 关系）：
  - [地址分配](business/apn-domain/ConfigurationSolution@apn-addr-allocation.md) — 核心（必选）
  - [鉴权 AAA](business/apn-domain/ConfigurationSolution@apn-auth.md) — 维度增强（4 选 1）
  - [隧道接入](business/apn-domain/ConfigurationSolution@apn-tunnel.md) — 维度增强（5 选 1，VPN 直通时可不启用）
  - [IP 类型治理](business/apn-domain/ConfigurationSolution@apn-ip-typing.md) — 维度增强（3 选 1）
- 不属于本域（与相邻域的边界）：
  ...
  - 网元对接开局（N4 PFCP 偶联 / Diameter 对接 / SBI 基础）— 属 [网元对接业务域](business/apn-domain/BusinessDomain@network-element-docking.md)（★待建，APN 域边界划定引用）
  - 运营级 KQI / 性能 / 告警 — 属运维域
...
- **下游场景**：[APN 接入与会话管理](business/apn-domain/NetworkScenario@apn-access.md)
- **下游方案**（4 个，AND 关系）：[地址分配](business/apn-domain/ConfigurationSolution@apn-addr-allocation.md) / [鉴权 AAA](business/apn-domain/ConfigurationSolution@apn-auth.md) / [隧道接入](business/apn-domain/ConfigurationSolution@apn-tunnel.md) / [IP 类型治理](business/apn-domain/ConfigurationSolution@apn-ip-typing.md)
