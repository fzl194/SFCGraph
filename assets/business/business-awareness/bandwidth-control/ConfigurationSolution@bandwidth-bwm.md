...
- **UDG 用户面（UPF/PGW-U）**：[BWM 2-00001](task/UDG/20.15.2/2-00001.md) 主 backbone——`SET LICENSESWITCH` + `ADD BWMSERVICE`（业务识别 TOS/NONTOS）+ `ADD BWMCONTROLLER`（CAR/Shaping + CIR/CBS/PIR/PBS + 三色）+ `ADD BWMUSERGROUP`（用户集合）+ `ADD BWMRULE`（绑定 Service+Controller，分用户级/用户组级/缺省）→ 绑接入域（APN 或切片）。
- **UDG 业务识别**：复用 [SA-Basic 2-00019](task/UDG/20.15.2/2-00019.md) 的特征库+解析库+L7 识别（HTTP/P2P/VoIP 七层识别能力）；[PCC基本功能 2-00018](task/UDG/20.15.2/2-00018.md) 的 PCCPOLICYGRP+RULE+USERPROFILE 骨架承载 BWM 策略组（POLICYTYPE=BWM）。
...
本方案跨网元编排 **5 个核心特性**：UDG 主 backbone [2-00001](task/UDG/20.15.2/2-00001.md) + UDG SA 识别 [2-00019](task/UDG/20.15.2/2-00019.md) + UDG PCC 通道 [2-00018](task/UDG/20.15.2/2-00018.md) + UNC BWM 承载 [2-00010](task/UNC/20.15.2/2-00010.md) + UNC PCC 通道 [2-00005](task/UNC/20.15.2/2-00005.md)。各特性未变化的配置走其**标准配置方法**（见各 feature task），以下仅说明本方案的**特性级变种/排除项**与**跨网元协同**。

### UDG 主 backbone：BWM 特性（[2-00001](task/UDG/20.15.2/2-00001.md)）
