...
| UDG 内容计费（[2-00003](task/UDG/20.15.2/2-00003.md)，GWFD-020301） | 基础（费率三件套结构） | 可选（配置重叠已含） | 费率三件套（URR/URRGROUP/PCCPOLICYGRP）结构**已内含于融合计费配置**——配融合计费时三件套已配，不需额外配内容计费特性；License LKV3G5BCBC01 必开 |
| UDG SA-Basic（[2-00019](task/UDG/20.15.2/2-00019.md)，GWFD-110101） | 基础（依赖前提） | 必配（License 前置） | 业务识别前提；融合计费依赖其识别业务流。配置不重叠（SA 配识别链，融合配 URR/CCT） |
