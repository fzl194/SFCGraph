...
本方案跨网元编排 **3 个特性**：UDG [2-00004 在线计费](task/UDG/20.15.2/2-00004.md) + [2-00006 融合计费](task/UDG/20.15.2/2-00006.md)；UNC [2-00003 融合计费](task/UNC/20.15.2/2-00003.md)。

**特性关系矩阵**（★回答"哪些必配 / 可选 / 包含 / 正交"，追溯原始文档「与其他特性的交互」段 + feature task 依赖声明）：

| 特性 | 角色 | 必配? | 关系说明 |
|---|---|---|---|
| [UDG 在线计费 2-00004](task/UDG/20.15.2/2-00004.md) | 触发链（在线场景） | 二选一 | 在线计费场景的配额耗尽触发链（Gy/DCC CCR-U → OCS Final-Unit-Action）；**与融合触发链互斥，按上层部署是纯在线还是融合选其一** |
| [UDG 融合计费 2-00006](task/UDG/20.15.2/2-00006.md) | 触发链（融合场景） | 二选一 | 融合计费场景的配额耗尽触发链（CHF Notify REAUTHORIZATION → 新配额）；**与在线触发链互斥**；融合路径多一步重授权 |
| [UNC 融合计费 2-00003](task/UNC/20.15.2/2-00003.md) | 跨网元对端（融合场景） | 条件必配 | 仅融合触发链时配（CCT Trigger + Final-Unit-Action 下发）；在线触发链无 UNC 侧独立特性（参考 CS-2 UNC/SMF CTF 角色） |

> 矩阵规则：①**核心**=方案主体（必配）；②**基础/依赖前提**=核心依赖的底层（必配，配置常与核心重叠）；③**维度增强**=正交维度可选叠加（按业务诉求）；④**跨网元对端**=另一网元对应特性（组合）；⑤**二选一**=互斥路径选其一。**配置重叠必须在「关系说明」注明**（防"额外配一遍"误解）。注：在线 OR 融合是**二选一触发链**（取决于上层是纯在线还是融合部署），不是都配。
