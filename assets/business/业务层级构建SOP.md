...
>
> **依赖前提特性必须进矩阵**（审查经验沉淀）：License 前置的依赖前提特性（如 SA-Basic、PCC基本功能）**必须作为矩阵行**，角色标「基础(依赖前提)」，必配列标「必配(License 前置)」，关系说明注明配置是否与核心重叠（识别/通道类通常**不重叠**——SA 配识别链、BWM 配限速对象，对象集不同）——**不只在约束段提**。避免不同 CS 矩阵行不一致。横切叠加型 CS（如配额降速/兜底，叠加于模式 CS 之上）也应在矩阵注明与模式的正交/叠加关系。
