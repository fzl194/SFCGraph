...
- **R2.4 ★ 调测混入检查（2026-07-11 APN 域新增，critical）**：feature/compound「配置流程」是否纯净？
  - grep 配置流程段内是否出现 `DSP`/`LST`/`EXP`/`STP`/`STR 探测` 命令 → 出现 = critical（调测混入配置蓝图，配置生成会误产出调测命令）
  - 配置概览表是否列「调测」行 → 出现 = warning（应剥离）
  - 关联段是否单列「调测 atom」子段 → 出现 = warning（调测应只在证据列表作链接，不单列）
  - 三场景范本（2-00001/03/06/10/18/19/26）配置流程 0 调测——这是基线。
  - **例外**：诊断/运维型特性（如 GWFD-010108，零配置命令）按 §3.7 骨架处置，不走本规则。
- **R2.5 ★ 平铺检查（2026-07-11 APN 域新增，critical）**：feature 配置流程是否把多命令阶段模块平铺成 atom 列表？
  - 找连续 ≥3 个 atom 引用且无 compound 承载的段 → 存在 = critical（违反 §3.5「跨特性复用预判」）
  - 重点查"接入域基础设施/可靠性体系/路由发布/隧道族"等多命令阶段是否抽了 compound
  - 跨特性重复：同族/同域 ≥2 个 feature 出现相同多命令步骤却各自平铺 = critical（应抽共享 compound）
