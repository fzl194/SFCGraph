...
## 概览
{方案是什么 + 跨网元/多特性协同骨架——UDG 用户面干啥、UNC 控制面干啥}。1-2 段。

## 配置与协同                         ← 镜像 feature task「配置流程」
{方案怎么配——编排哪些 feature task 2- + 可复用 compound 1- + 单命令 atom 0- + 方案特有额外步骤；
 跨网元协同顺序(UDG 用户面 vs UNC 控制面)。每步"干啥 + 引用哪个 task"，人不点链接就懂。}
1. **{步骤名}**（{一句话干啥}）：`{命令提示}` → [{task-id}](链接)
...
## 关联
- 上游场景：{NS}
- 下游 task：{feature/compound/atom task 链接，带命令提示}
- 证据：{evidence 链接}
```

> **CS「配置与协同」段是表达"怎么配"的方式**：镜像 feature task 的「配置流程」，每步"干啥 + 引用哪个 task"（含方案特有额外步骤）。不另设 solution task 对象。
...
- **证据自包含**：方案/业务专题 md 拷进 `assets/evidence/business/{场景}/`（CLAUDE.md §7 可剥离）。evidence 文件命名 `{描述名}_{原始文档ID}.md`；`source_evidence_ids` 用 `EV-BIZ-{场景slug}-{NN}`。证据同时在「关联」段以链接列出。
