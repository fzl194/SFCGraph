...
| `task_goal` | 为所有后续 task 提供统一的角色、接口和组网上下文 |
| `input_contract` | 部署角色、接口抽象策略、参考点理解 |
| `output_contract` | 部署角色判定、接口范围清单 |
| `command_refs` | `[]` |
...
| `task_scope_type` | `generic` |
| `task_goal` | 使目录后续章节所需能力可用 |
| `input_contract` | ESN、License文件、加载路径 |
| `output_contract` | License 已激活状态 |
...
| `task_scope_type` | `generic` |
| `task_goal` | 形成稳定可纳管、可对接、可转发的基础状态 |
| `input_contract` | NTP地址、ME/OMIP、公共参数值、MTU规划 |
| `output_contract` | 基础数据配置完成 |
...
| `task_scope_type` | `generic` |
| `task_goal` | 避免后续高危命令或证书相关操作被阻断 |
| `input_contract` | 授权用户、命令成员、证书需求 |
| `output_contract` | 安全接入前置完成 |
...
| `task_goal` | 使网元可被 MAE/U2020 纳管 |
| `input_contract` | 北向密码、SNMPv3 密钥、适配层版本、网元参数 |
| `output_contract` | 网元纳管成功 |
| `command_refs` | `["LST ME"]` |
| `source_evidence_ids` | `["EV-TK-02"]` |
...
| `task_goal` | 建立 N4 控制面入口 |
| `input_contract` | N4 地址、信令 VPN、HOSTNAME |
| `output_contract` | N4if 就绪 |
...
| `task_scope_type` | `feature_specific` |
| `task_goal` | 建立业务面承载路径 |
| `input_contract` | 接口清单、IP版本、接口地址 |
| `output_contract` | 业务逻辑接口就绪 |
...
| `task_scope_type` | `feature_specific` |
| `task_goal` | 处理需要 Nupf 或切片场景的分支接口能力 |
| `input_contract` | 是否启用 Nupf、切片参数 |
| `output_contract` | Nupf 或切片绑定完成 |
...
| `task_scope_type` | `feature_specific` |
| `task_goal` | 形成会话接入数据闭环 |
| `input_contract` | APN/DNN、地址池类型、地址段、分配规则 |
| `output_contract` | 会话接入数据可用 |
...
| `task_scope_type` | `generic` |
| `task_goal` | 为后续路由协议与隧道叠加提供容器和承载口 |
| `input_contract` | VPN名、地址族、外联口方案 |
| `output_contract` | 外联基础完成 |
...
| `task_scope_type` | `cross_feature` |
| `task_goal` | 让路由在当前组网模式下稳定收敛 |
| `input_contract` | 路由协议、IP版本、部署方式、组网模式 |
| `output_contract` | 路由协议生效 |
...
| `task_scope_type` | `cross_feature` |
| `task_goal` | 提供快速故障检测和隧道增强能力 |
| `input_contract` | BFD模式、隧道类型、MPLS需求 |
| `output_contract` | BFD和隧道侧生效 |
...
| `task_scope_type` | `cross_feature` |
| `task_goal` | 完成自动部署模板下发或硬件级联准备 |
| `input_contract` | 自动/手工、硬件类型、模板参数 |
| `output_contract` | 自动部署模板或级联口可用 |
...
| `task_scope_type` | `cross_feature` |
| `task_goal` | 给出可验证的端到端上线结果 |
| `input_contract` | 实例模板、调测路径、验收标准 |
| `output_contract` | 实例套用完成，FirstCall 验证通过 |
