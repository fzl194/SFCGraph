...
带宽控制场景三层图谱的证据源共分三类，合计 **58份知识资产**：

| 证据类型 | ID前缀 | 数量 | 路径根目录 |
|---------|--------|------|-----------|
| 特性知识文档 | `EV-FK-*` | 24 | `feature-knowledge/` |
| 主题知识批次 | `EV-TK-*` | 32 | `topic-knowledge/` |
| 跨层分析文档 | `EV-CA-*` | 2 | `cross-feature-analysis.md`、`cross-topic-analysis.md` |
| **合计** | — | **58** | — |
...
```
CS-BW-01 (SA-BWM方案)
  → source_evidence_ids: [EV-CA-02, EV-TK-19, EV-FK-04]
...
- [x] 24份特性知识全部登记并分配 Evidence ID
- [x] 32份主题知识全部登记并分配 Evidence ID
- [x] 2份跨层分析全部登记并分配 Evidence ID
- [x] 所有路径相对 `business-graph/带宽控制场景/` 准确
- [x] 核心方案 CS-BW-01~07 至少引用 1份direct证据 + 1份supporting证据
- [x] 所有Feature对象引用对应 EV-FK-*
- [x] 所有核心Command对象引用对应 EV-FK-* + EV-CA-01
...
1. 知识库扩展（新增特性/主题）→ 产出新知识文档
2. 在本索引对应分类下追加 `EV-FK-*` 或 `EV-TK-*` 条目
...
3. 最后采信主题知识批次（topic-knowledge/，综合归纳）
4. 冲突记录到 `conflicts/` 目录，并在图谱对象标注冲突引用

---

> 本索引为带宽控制场景三层图谱的"证据层"，确保图谱任何结论都能反查到权威事实源，满足 CLAUDE.md §16 质量门禁的"可信度门禁"要求。
