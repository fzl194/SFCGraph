...
| D6 | 执行态 Task 与 ProcedureStep 分离，通过 task_executed_by_step 映射 |
...
> 决策依据：D3（DS-02合并为DS-01的DecisionPoint分支）、D4（dependency_type替代primary/cross-cutting）、D6（Task与ProcedureStep分离）
...
**定位**：原子动作单元，可被多个 DS 或 Feature 复用。不再绑定单一 DS。
...
| command_role | string | 否 | 使用的命令角色（ADD/SET for execute, LST/EXP for verify, none for plan） |
| config_object_relation | string | 否 | 与 ConfigObject 的关系（creates_config / verifies_config） |
...
EngineeringTask --task_executed_by_step--> ProcedureStep (1:N, execute phase)
EngineeringTask --supported_by--> Evidence
```

> 决策依据：D5（phase 字段）、D6（Task 与 ProcedureStep 分离）、M:N 共享模型（Task 是原子单元，可被 DS 和 Feature 复用）
...
| command_role | ADD |
| config_object_relation | creates_config |
| source_evidence_ids | ["套餐1_93112148", "套餐2_94838085", "套餐3_94838086"] |
| status | active |

**涉及 ConfigObject**：FILTER

**命令示例**（来源：套餐1_93112148.md 步骤 2）：
...
**涉及 ConfigObject**：IPLIST

**命令示例**（来源：套餐1_93112148.md 步骤 1）：
...
**涉及 ConfigObject**：L7FILTER

**命令示例**（来源：套餐1_93112148.md 步骤 3）：
...
**涉及 ConfigObject**：FLOWFILTER、FLOWFILTERGRP（及绑定关系 FLTBINDFLOWF、PROTBINDFLOWF）

**命令示例**（来源：套餐1_93112148 步骤4）：
...
**涉及 ConfigObject**：URR、URRGROUP、PCCPOLICYGRP

**命令示例**（来源：套餐1_93112148.md 步骤 5）：
...
**涉及 ConfigObject**：PCCACTIONPROP、HEADEN、REDIRECT、PCCPOLICYGRP

**命令示例**（来源：套餐3_94838086.md 步骤 4）：
...
**涉及 ConfigObject**：CATEGORYPROP、BWMSERVICE、BWMCONTROLLER、BWMUSERGROUP、BWMRULE

**命令示例**（来源：套餐2_94838085.md 步骤 4）：

```
// 最低带宽保证 10Mbps（CAR + CIR）
...
**涉及 ConfigObject**：RULE（及可选 BLACKLISTRULE）

**命令示例**：

DS-01 计费（来源：套餐1_93112148.md 步骤 6）：
```
...
DS-02 访问限制（来源：套餐3_94838086.md 步骤 5）：
```
...
DS-03 带宽控制（来源：套餐2_94838085.md 步骤 5）：
```
...
**涉及 ConfigObject**：BLACKLISTRULE

**命令示例**（来源：套餐2_94838085.md 步骤 5）：
...
| command_role | ADD / SET |
| config_object_relation | creates_config |
| source_evidence_ids | ["套餐1_93112148", "套餐2_94838085", "套餐3_94838086"] |
| status | active |

**涉及 ConfigObject**：USERPROFILE、RULEBINDING、REFRESHSRV（可选 URRGRPBINDING）

**命令示例**（来源：套餐1_93112148.md 步骤 7-8）：
...
**涉及 ConfigObject**：RULEBINDING→RULE→FLOWFILTER→FILTER 全链路
...
LST PCCPOLICYGRP:PCCPOLICYGRPNM="pccgA";
LST FLOWFILTER:FLOWFILTERNAME="flowfilterA";
LST FLTBINDFLOWF:FLOWFILTERNAME="flowfilterA";
LST FILTER:FILTERNAME="filterA";
```
...
| command_role | LST（排查时使用） |
| config_object_relation | verifies_config |
| source_evidence_ids | ["调测内容计费_08957400"] |
| status | active |

**涉及 ConfigObject**：PFCP Session Report（N4 接口消息）
...
**排查命令**（URR ID 不匹配时逐层回查）：

```
LST RULEBINDING:USERPROFILENAME="up_charging";
LST RULE:RULENAME="ruleA";
LST PCCPOLICYGRP:PCCPOLICYGRPNM="pccgA";
LST URRGROUP:URRGROUPNAME="urrgA";
LST URR:URRNAME="urrA";
LST PROTBINDFLOWF:FLOWFILTERNAME="flowfilterA";
LST L7FILTER:L7FILTERNAME="l7filterA";
```
...
**涉及 ConfigObject**：RULEBINDING, RULE, PCCPOLICYGRP, PCCACTIONPROP, FLTBINDFLOWF, FILTER

**命令示例**：

```
LST RULEBINDING:USERPROFILENAME="up_policy";
LST RULE:RULENAME="ruleA";
LST PCCPOLICYGRP:PCCPOLICYGRPNM="pccgrpA";
LST PCCACTIONPROP:PCCACTPROPNAME="pccactA";
LST FLTBINDFLOWF:FLOWFILTERNAME="flowfilterA";
LST FILTER:FILTERNAME="filterA";
```
...
| command_role | LST / DSP |
| config_object_relation | verifies_config |
| source_evidence_ids | ["套餐3_94838086"] |
| status | active |

**涉及 ConfigObject**：RULEBINDING, RULE, PCCPOLICYGRP, PCCACTIONPROP, FLOWFILTER, PROTBINDFLOWF, L7FILTER, FLTBINDFLOWF, FILTER, SIGNATUREDB
...
**涉及 ConfigObject**：USERPROFILE, RULEBINDING, RULE, CATEGORYPROP, BWMSERVICE, BWMCONTROLLER, BWMUSERGROUP, BWMRULE

**命令示例**：

```
LST RULEBINDING:USERPROFILENAME="up_bwmcontrol",POLICYTYPE=BWM;
LST RULE:RULENAME="ruleA";
LST CATEGORYPROP:CATEPROPNAME="catropA";
LST BWMSERVICE:BWMSERVICENAME="bwmservA";
LST BWMCONTROLLER:BWMCNAME="bwmcontrolA";
LST BWMUSERGROUP:QRYUSRGRPTYPE=SPECIFIC_GROUP,USERGROUPNAME="bwmgrpA";
LST BWMRULE:USERGROUPTYPE=SPECIFIC_GROUP,USERGROUPNAME="bwmgrpA",BWMRULETYPE=SUBSCRIBER_SPECIFIC,BWMRULENAME="bwmruleA";
```
