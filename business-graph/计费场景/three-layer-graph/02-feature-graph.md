...
| WSFD-109101 (UNC PCC) | `depends_on` | GWFD-020351 (UDG PCC) | mandatory（跨产品） | 前置条件：UDG侧已完成GWFD-020351配置 |
...
> 详细Task定义见 `03-task-layer.md`。此处仅列出核心Feature的task_order。

### 6.1 GWFD-010171 离线计费（UDG侧配置链）

```
T-101(License) → T-006(URR/URRGROUP, OFFLINE) → T-007(PCCPOLICYGRP)
→ T-001(CATEGORYPROP/FILTER) → T-002(FLOWFILTER/FLTBINDFLOWF)
→ T-003(RULE) → T-004(USERPROFILE/RULEBINDING)
→ T-104(URRGRPBINDING/SPECTRAFURRGRP) → T-008(REFRESHSRV, must_be_last)
```

### 6.2 GWFD-020300 在线计费（UDG侧配置链）

```
T-101(License LKV3G5OLCH01) → T-006(URR/URRGROUP, ONLINE)
→ T-007(PCCPOLICYGRP) → T-001/T-002(过滤条件) → T-003(RULE)
→ T-004(USERPROFILE/RULEBINDING) → T-203(UPDEFAULTQUOTA容灾)
→ T-202(Tx定时器/异常处理) → T-204(QUOTAEXHAUSTACT配额耗尽)
→ T-008(REFRESHSRV)
```
...
### 6.5 WSFD-011206 融合计费（UNC侧配置链，18步）

```
T-101(License) → T-301(SET CHGMODE=NchfMode) → T-302(SET CHARGECTRL使能)
→ T-303(SET CHFINIT=SENDREQ + CHF选择链: TNFINS/TNFGRP/TNFBINDGRP/SELECTCHFGBYCC)
→ T-309(ADD CHARGECHAR CC属性) → T-304(ADD CCT模板 + SELECTCCTBYCC)
→ T-305(ADD PDUTRIGGER/RGTRIGGER) → T-306(SET N40APIVER)
→ T-307(SET FAILHANDLING/PDUSCACT/RGRCACT) → T-308(SET N40MSGSTG缓存)
→ T-006~T-004(URR三件套+规则+绑定) → T-005(USRPROFGROUP/UPBINDUPG/APNUSRPROFG)
→ T-310(费率切换, 可选) → T-204(QUOTAEXHAUSTACT, 可选)
```
