...
> **作用**：实例化28个ConfigTask + TaskRule + TaskCommandOrderEdge
> **数据来源**：`feature-knowledge/cross-feature-analysis.md`（附录D端到端配置流程）、`计费场景统一知识库.md`（K001-K261）
...
| feature_specific（特性专属） | 20 | T-101~T-311 | License/SA/在线/融合/CG/CC专属 |
| **合计** | **28** | — | — |
...
## 2. 特性专属Task（feature_specific，20个）
...
|-----------|-------------|-------------|--------------|--------------|------------|
| `TR-01` | 计费三件套必须完整链路 | `acceptance_rule` | T-006, T-007, T-003 | Rule → PCCPOLICYGRP → URRGROUP → URR 链路必须完整无断裂，任何一环缺失导致计费不生效 | critical |
| `TR-02` | URR ID空间分段分配 | `naming_rule` | T-006 | 同一PDU会话内URR ID不能重复（不同业务需分段分配ID空间，如1-99/100+） | critical |
| `TR-03` | REFRESHSRV必须最后执行 | `execution_rule` | T-008 | SET REFRESHSRV必须在所有ADD/SET完成后执行；PROTBINDFLOWF需等待60秒 | critical |
| `TR-04` | 融合计费三联前置约束 | `dependency_rule` | T-301, T-302, T-303 | 融合计费场景下CHGMODE=NchfMode + CHARGECTRL使能 + CHFINIT=SENDREQ 三项必须全部满足 | critical |
| `TR-05` | CHF选择链完整性 | `dependency_rule` | T-303 | TNFINS → TNFINSIP → TNFGRP → TNFBINDGRP → SELECTCHFGBYCC → GLBDFTCHFGROUP 必须顺序完成 | warning |
| `TR-06` | License前置门控 | `validation_rule` | T-101 | 计费特性配置前必须先SET LICENSESWITCH开启对应License；内容计费需UNC+UDG双License同时开启 | critical |
...
| edge_id | task_ref | from_command | to_command | relation_type | requiredness | propagated_context |
|---------|----------|--------------|------------|---------------|--------------|-------------------|
| TE-006-1 | T-006 | `ADD URR` | `ADD URRGROUP` | `precedes` | required | URRID → URRGROUP.UPURRNAME1 |
| TE-006-2 | T-006 | `ADD URRGROUP` | `SET URRGRPBINDING` | `precedes` | required | URRGROUPNAME → URRGRPBINDING |
| TE-006-3 | T-006 | `SET URRGRPBINDING` | `SET SPECTRAFURRGRP` | `precedes` | optional | URRGROUPNAME |

### 4.2 T-007 配置PCC策略组

| edge_id | task_ref | from_command | to_command | relation_type | requiredness | propagated_context |
|---------|----------|--------------|------------|---------------|--------------|-------------------|
| TE-007-1 | T-007 | `ADD URRGROUP` | `ADD PCCPOLICYGRP` | `precedes` | required | URRGROUPNAME → PCCPOLICYGRP |
| TE-007-2 | T-007 | `ADD PCCPOLICYGRP` | `ADD RULE` | `precedes` | required | PCCPOLICYGRPNAME → RULE.POLICYNAME |

### 4.3 T-003 配置PCC规则（内部链）

| edge_id | task_ref | from_command | to_command | relation_type | requiredness | propagated_context |
|---------|----------|--------------|------------|---------------|--------------|-------------------|
| TE-003-1 | T-003 | `ADD FLOWFILTER` | `ADD RULE` | `precedes` | required | FLOWFILTERNAME → RULE |
| TE-003-2 | T-003 | `ADD PCCPOLICYGRP` | `ADD RULE` | `precedes` | required | PCCPOLICYGRPNAME → RULE.POLICYNAME |

### 4.4 T-311 配置CG接口（离线CG侧链）

| edge_id | task_ref | from_command | to_command | relation_type | requiredness | propagated_context |
|---------|----------|--------------|------------|---------------|--------------|-------------------|
| TE-311-1 | T-311 | `ADD CG` | `SET CDRTRANSFER` | `precedes` | required | CGNAME |
| TE-311-2 | T-311 | `SET CDRTRANSFER` | `SET OFCTHRESHOLD` | `precedes` | required | — |
| TE-311-3 | T-311 | `SET OFCTHRESHOLD` | `SET CONTAINERTRIGGER` | `precedes` | optional | — |

### 4.5 T-303 配置CHF交互使能与选择链

| edge_id | task_ref | from_command | to_command | relation_type | requiredness | propagated_context |
|---------|----------|--------------|------------|---------------|--------------|-------------------|
| TE-303-1 | T-303 | `SET CHFINIT` | `ADD TNFINS` | `precedes` | required | CHFINIT=SENDREQ |
| TE-303-2 | T-303 | `ADD TNFINS` | `ADD TNFINSIP` | `precedes` | required | TNFNAME |
| TE-303-3 | T-303 | `ADD TNFINSIP` | `ADD TNFGRP` | `precedes` | required | TNFNAME → TNFGRP |
| TE-303-4 | T-303 | `ADD TNFGRP` | `ADD TNFBINDGRP` | `precedes` | required | TNFGRPNAME, TNFNAME |
| TE-303-5 | T-303 | `ADD TNFBINDGRP` | `ADD SELECTCHFGBYCC` | `precedes` | required | TNFGRPNAME → SELECTCHFGBYCC |
| TE-303-6 | T-303 | `ADD SELECTCHFGBYCC` | `SET GLBDFTCHFGROUP` | `precedes` | optional | TNFGRPNAME |

### 4.6 T-302 融合计费配置链（三联前置约束）

| edge_id | task_ref | from_command | to_command | relation_type | requiredness | propagated_context |
|---------|----------|--------------|------------|---------------|--------------|-------------------|
| TE-302-1 | T-302 | `SET CHGMODE` | `SET CHARGECTRL` | `precedes` | required | FORCED=NchfMode |
| TE-302-2 | T-302 | `SET CHARGECTRL` | `SET CHFINIT` | `precedes` | required | RGAPPLIED |
...
| 对象类型 | 数量 | 编号范围 |
|---------|------|---------|
| ConfigTask | 28 | T-001~T-008（generic 8）+ T-101~T-311（feature_specific 20） |
| TaskRule | 6 | TR-01~TR-06 |
