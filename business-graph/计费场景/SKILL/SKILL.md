...
在执行任何步骤之前，**必须先检查以下两个条件**：

1. **用户需求描述**：用户是否已说明计费配置需求？
2. **现网配置脚本**：用户是否已提供现网配置脚本文件路径？

> **阻塞检查（绝对不可跳过）**：
>
> 如果用户**未提供现网配置脚本文件路径**，**必须立即停止**，向用户明确提示：
>
> ```
...
> ```
>
> 在用户提供现网脚本之前：**不得执行步骤 1-4，不得进入 Phase 2，不得生成任何配置命令**。
>
> **文件验证**：收到路径后，先验证文件存在且可读取。如果文件不存在或无法读取，告知用户具体错误并要求重新提供。验证通过后告知用户："已收到现网配置脚本，开始分析。"
