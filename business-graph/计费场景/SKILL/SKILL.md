...
在执行 §4.2 任何步骤之前，**必须按顺序完成以下检查**：

**检查1：用户是否已提供现网配置脚本文件路径？**

- 如果用户**未提供**，**必须立即停止**，向用户提示：
...
**检查2：验证现网脚本文件实际存在且可读取。**

- 使用 **Glob 工具** 或 **Bash `ls`** 检查用户提供的每个文件路径是否存在
- 如果文件不存在：告知用户具体路径和错误，要求重新提供，**不得继续**
- 如果文件存在但为空或无法读取：告知用户具体错误，**不得继续**
- 验证通过后告知用户："已收到现网配置脚本（UPF: {文件名}, SMF: {文件名}），共 {n} 个文件，开始分析。"

> **在检查1和检查2都通过之前，绝对不得执行 §4.2 的任何步骤，不得进入 Phase 2，不得生成任何配置命令。**
...
> **阻塞检查（绝对不可跳过）**：进入 Phase 5 之前，**必须确认用户已在 Phase 4 明确确认参数表**。如果用户未确认参数，**不得开始生成任何 MML 配置命令**，必须退回 Phase 4。
...
核查通过后，**必须将核查报告和生成的配置脚本一并向用户展示，等待用户确认**。

> **阻塞检查（绝对不可跳过）**：用户确认配置脚本内容后，才可进入 Phase 7 正式交付。用户未确认前，**不得输出最终交付物**。

- 展示后告知用户："以上配置脚本已通过全量核查。请确认脚本内容无误后回复'确认'，我将输出最终交付物。如有问题请指出。"
- 用户确认 → 进入 Phase 7
- 用户要求修改 → 回到对应 Phase 修正
