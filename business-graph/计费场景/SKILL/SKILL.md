...
进入 Phase 1 后，**必须先使用 Read 工具读取以下文件，再进行任何业务逻辑（包括 §4.1 的检查）**：

```
必须读取（共4个文件）：
1. knowledge/ref-phase1-场景识别与解析.md   — Phase 1 的 pipeline 指令（含阶段执行约束）
...
**读取完成后告知用户："已加载 4 个 Phase 1 参考文件。"** 然后继续执行 §4.1。
